...
final class WP_Service_Worker_Custom_Header_Integration extends WP_Service_Worker_Base_Integration
{
    ...
    public function register(WP_Service_Worker_Scripts $scripts)
    {
        ...
        if ($attachment) {
            foreach ($this->get_attachment_image_urls($attachment->ID, [ $header->width, $header->height ]) as $image_url) {
                $scripts->precaching_routes()->register($image_url, [ 'revision' => $attachment->post_modified ]);
            }
