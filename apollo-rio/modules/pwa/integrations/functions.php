    ...
    foreach ($integrations as $slug => $integration_class) {
        ...
        switch ($scope) {
            case WP_Service_Workers::SCOPE_FRONT:
                add_action('wp_front_service_worker', [ $integration, 'register' ], $priority, 1);

                break;
            case WP_Service_Workers::SCOPE_ADMIN:
                add_action('wp_admin_service_worker', [ $integration, 'register' ], $priority, 1);
            ...
            case WP_Service_Workers::SCOPE_ALL:
                ...
                break;
            default:
                $valid_scopes = [ WP_Service_Workers::SCOPE_FRONT, WP_Service_Workers::SCOPE_ADMIN, WP_Service_Workers::SCOPE_ALL ];
                _doing_it_wrong(
                    __FUNCTION__,
                    sprintf(
