...
final class WP_Web_App_Manifest
{
     ...
     * @var string
     */
    public const FALLBACK_THEME_COLOR = '#fff';
     ...
     * @var string
     */
    public const REST_ROUTE = '/web-app-manifest';
     ...
     * @var string
     */
    public const SHORT_NAME_OPTION = 'short_name';
     ...
     * @var int
     */
    public const SHORT_NAME_MAX_LENGTH = 12;
    ...
    public function init()
    {
        ...
        add_filter('site_status_tests', [ $this, 'add_pwa_site_health_tests' ]);

        add_action('rest_api_init', [ $this, 'register_short_name_setting' ]);
        add_action('admin_init', [ $this, 'register_short_name_setting' ]);
        add_action('admin_init', [ $this, 'add_short_name_settings_field' ]);
    }
        ...
        switch ($display) :
            ...
            case 'standalone':
                ...
                foreach ($images as $image) {
                    if (is_array($image) && isset($image['href']) && esc_url($image['href'], [ 'http', 'https' ])) {
    ...
    public function get_theme_color()
    {
        ...
        if (current_theme_supports('custom-background')) {
            ...
            // This returns a hex value without the leading #, or an empty string.
            if ($background_color) {
    ...
    public function add_pwa_site_health_tests($tests)
    {
        ...
        $tests['direct']['web_app_manifest_short_name'] = [
            'label' => __('Short Name in Web App Manifest', 'pwa'),
            'test'  => [ $this, 'test_short_name_present_in_manifest' ],
        ];

        // Add tests for the site icon validation.
        $tests['direct']['pwa_site_icon_validation'] = [
            'label' => __('Site Icon', 'pwa'),
            'test'  => [ $this, 'test_site_icon' ],
        ];
    ...
    public function register_manifest_rest_route()
    {
        register_rest_route(
            ...
            [
                'methods'             => 'GET',
                'callback'            => [ $this, 'rest_serve_manifest' ],
    ...
    public function rest_permission(WP_REST_Request $request)
    {
        if ('edit' === $request['context']) {
            return new WP_Error('rest_forbidden_context', __('Sorry, you are not allowed to edit the manifest.', 'default'), [ 'status' => rest_authorization_required_code() ]);
        }
    ...
    public function register_short_name_setting()
    {
        register_setting(
            ...
            [
                ...
                'sanitize_callback' => [ $this, 'sanitize_short_name' ],
                'show_in_rest'      => true,
            ]
        );
    }
    ...
    public function add_short_name_settings_field()
    {
        add_settings_field(
            ...
            [ $this, 'render_short_name_settings_field' ],
            'general',
            'default',
            [
