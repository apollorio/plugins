...
final class WP_Service_Worker_Navigation_Routing_Component implements WP_Service_Worker_Component
{
     ...
     * @var string
     */
    public const CACHE_NAME = 'navigations';
     ...
     */
    public const STREAM_THEME_SUPPORT = 'service_worker_streaming';
    ...
    public function serve(WP_Service_Worker_Scripts $scripts)
    {
        ...
        if (! is_admin()) {
            ...
            $revision .= ';deps=' . md5(wp_json_encode([ wp_scripts()->queue, wp_styles()->queue ]));
            ...
            $offline_error_template_file  = pwa_locate_template([ 'offline.php', 'error.php' ]);
            ...
            $server_error_template_file  = pwa_locate_template([ '500.php', 'error.php' ]);
            ...
            $offline_error_precache_entry = [
                'url' => add_query_arg('code', 'offline', admin_url('admin-ajax.php?action=wp_error_template')),
                // Upon core merge, this would use admin_url( 'error.php' ).
                                    'revision' => $revision,
            // Upon core merge, this should be the core version.
            ];
            $server_error_precache_entry = [
                'url' => add_query_arg('code', '500', admin_url('admin-ajax.php?action=wp_error_template')),
                // Upon core merge, this would use admin_url( 'error.php' ).
                                    'revision' => $revision,
            // Upon core merge, this should be the core version.
            ];
        ...
        $scripts->register(
            ...
            [
                'src'  => [ $this, 'get_offline_post_request_script' ],
                'deps' => [ 'wp-base-config' ],
            ]
        );
        ...
        $scripts->register(
            ...
            [
                'src'  => [ $this, 'get_script' ],
                'deps' => [ 'wp-base-config', 'wp-offline-post-request-handling' ],
            ]
        );
    ...
    public function get_navigation_route_denylist_patterns()
    {
        ...
        if (! is_admin()) {
             ...
             */
            $denylist_patterns = apply_filters_deprecated('wp_service_worker_navigation_route_blacklist_patterns', [ $denylist_patterns ], '0.4', 'wp_service_worker_navigation_route_denylist_patterns');
    ...
    public function get_script()
    {
        ...
        return preg_replace_callback(
            ...
            [ $this, 'replace_exported_variable' ],
            $script
        );
    }
