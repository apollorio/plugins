...
function apollo_should_show_content($template_type)
{
	...
	// pagx_site & pagx_apolloapp: ALWAYS show content (public pages)
	if (in_array($template_type, ['pagx_site', 'pagx_apolloapp'], true)) {
		return true;
	}

	// pagx_app and pagx_appclean: Conditional logic
	if (in_array($template_type, ['pagx_app', 'pagx_appclean'], true)) {
...
function apollo_get_header($name = null)
{
	...
	$file = plugin_dir_path(__DIR__) . 'templates/' . $template;
