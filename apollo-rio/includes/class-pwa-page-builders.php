...
class Apollo_PWA_Page_Builders
{
	...
	public function load_template($template)
	{
		...
		if (file_exists($plugin_template)) {
			// Track PWA page view in Apollo Core Analytics
	...
	public function filter_enqueued_assets()
	{
		...
		if (isset($wp_styles) && is_object($wp_styles) && isset($wp_styles->queue)) {
			...
			foreach ($wp_styles->queue as $handle) {
				...
				if (strpos($src, '/apollo-') !== false || strpos($src, 'assets.apollo.rio.br') !== false || strpos($src, 'remixicon') !== false) {
					$is_apollo = true;
				}
		...
		if (isset($wp_scripts) && is_object($wp_scripts) && isset($wp_scripts->queue)) {
			...
			foreach ($wp_scripts->queue as $handle) {
				...
				// Check if src contains apollo plugin paths
				if (strpos($src, '/apollo-') !== false || strpos($src, 'assets.apollo.rio.br') !== false) {
					$is_apollo = true;
				}
	...
	public function enqueue_manifest()
	{
		...
		// Only enqueue manifest for App::rio / Apollo-APP pages
		if (array_key_exists($template, $apollo_templates) && in_array($template, ['pagx_app.php', 'pagx_apolloapp.php', 'pagx_appclean.php'], true)) {
	...
	/**
	 * ULTRA PRO: Enqueue all Apollo PWA assets with robust fallbacks
	 *
	 * Asset loading strategy:
	 * 1. apollo_ensure_base_assets() - CDN loader from apollo-social
	 * 2. Local fallbacks for critical CSS
	 * 3. Template-specific styles
	 * 4. PWA detection script
	 */
	public function enqueue_pwa_assets()
	{
		...
		// ========================================
		// LAYER 1: Apollo CDN Loader (primary)
		// ========================================
		if (function_exists('apollo_ensure_base_assets')) {
			apollo_ensure_base_assets();
		} else {
			// Fallback: Load CDN directly if apollo-social not available
			$this->enqueue_cdn_fallback();
		}

		// ========================================
		// LAYER 2: Critical CSS (always load)
		// ========================================
		$this->enqueue_critical_css();

		// ========================================
		// LAYER 3: Template-specific assets
		// ========================================
		wp_enqueue_style(
			'apollo-pwa-templates',
			plugins_url('assets/css/pwa-templates.css', __DIR__),
			[],
			defined('APOLLO_RIO_VERSION') ? APOLLO_RIO_VERSION : '1.0.0'
		);

		// ========================================
		// LAYER 4: PWA Detection Script
		// ========================================
		wp_enqueue_script(
			'apollo-pwa-detect',
	...
	/**
	 * FALLBACK: Enqueue Apollo CDN assets directly if apollo-social not available
	 *
	 * This ensures Apollo Rio templates work even without apollo-social plugin.
	 */
	private function enqueue_cdn_fallback()
	{
		// Apollo Assets CDN - Design System
		wp_enqueue_style(
			'apollo-uni-css',
	...
	/**
	 * Enqueue critical CSS for Apollo templates
	 *
	 * Inline critical CSS for instant paint on page load.
	 */
	private function enqueue_critical_css()
	{
		// Register empty style handle for inline CSS
		wp_register_style('apollo-rio-critical', false);
		wp_enqueue_style('apollo-rio-critical');

		$critical_css = "
/* Apollo Rio Critical CSS - Instant Paint */
:root {
	--apollo-primary: #EA580C;
	--apollo-primary-dark: #C2410C;
	--apollo-bg: #FFFFFF;
	--apollo-bg-dark: #0F172A;
	--apollo-text: #1E293B;
	--apollo-text-muted: #64748B;
...
.apollo-body { min-height: 100vh; background: var(--apollo-bg); color: var(--apollo-text); }
#apollo-wrapper { min-height: 100vh; display: flex; flex-direction: column; }
#apollo-header { position: sticky; top: 0; z-index: 50; }
#apollo-main, .apollo-content-wrapper { flex: 1; }
.apollo-container { width: 100%; max-width: 1280px; margin: 0 auto; padding: 0 1rem; }
@media (prefers-color-scheme: dark) {
	.apollo-body { background: var(--apollo-bg-dark); color: #F1F5F9; }
}
";
		wp_add_inline_style('apollo-rio-critical', $critical_css);
	}
}
...
function apollo_rio_register_templates(): void
{
	...
	apollo_rio_add_template('pagx_apolloapp.php', __('Apollo-APP', 'apollo-rio'));
