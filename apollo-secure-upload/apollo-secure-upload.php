...
if ( file_exists( WP_PLUGIN_DIR . '/apollo-core/includes/class-apollo-audit-log.php' ) ) {
...
function apollo_secure_upload_admin_page() {
	...
	if ( ! menu_page_url( 'apollo-core-hub', false ) ) {
		// Fallback: create standalone menu if apollo-core-hub doesn't exist
		add_menu_page(
		...
		add_submenu_page(
			'apollo-core-hub',
			__( 'Apollo Secure Upload Logs', 'apollo-secure-upload' ),
			__( 'Upload Logs', 'apollo-secure-upload' ),
			'manage_apollo_uploads',
			'apollo-secure-upload-logs',
			'apollo_secure_upload_logs_page'
		);
	...
	<div class="wrap">
		...
		<table class="widefat">
			...
			<tbody>
				...
				$logs = $wpdb->get_results( "SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE 'apollo_upload_%' ORDER BY option_id DESC LIMIT 50" );
				if ( is_array( $logs ) ) {
					foreach ( $logs as $log ) {
