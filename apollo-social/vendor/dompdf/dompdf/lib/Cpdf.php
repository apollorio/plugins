...
class Cpdf {
	 ...
	 */
	public $currentLineTransparency = array(
		'mode'    => 'Normal',
		'opacity' => 1.0,
	);
	 ...
	 */
	public $currentFillTransparency = array(
		'mode'    => 'Normal',
		'opacity' => 1.0,
	);
	 ...
	 */
	protected $imageAlphaList = array();
	 ...
	 */
	protected $currentPageSize = array(
		'width'  => 0,
		'height' => 0,
	);
			...
			case 'add':
				...
				foreach ( $options as $k => $v ) {
					switch ( $k ) {
						...
						case 'Duplex':
							if ( ! in_array( $v, array( 'None', 'Simplex', 'DuplexFlipShortEdge', 'DuplexFlipLongEdge' ) ) ) {
								break;
							}
							$o['info'][ $k ] = $v;
							break;
...
EOT;
		...
		switch ( $action ) {
			case 'new':
				...
				if ( ! isset( $options['type'] ) || $options['type'] === 'jpg' ) {
					...
					if ( $options['isMask'] ) {
						$info['ColorSpace'] = '/DeviceGray';
					} elseif ( mb_strlen( $options['pdata'], '8bit' ) ) {
	...
	protected function o_xobject( $id, $action, $options = '' ) {
		switch ( $action ) {
			...
			case 'font':
				$this->objects[ $id ]['fonts'][ $options['fontNum'] ] = array(
					'objNum'  => $options['objNum'],
					'fontNum' => $options['fontNum'],
				);
				break;
			...
			case 'xObject':
				$this->objects[ $id ]['xObjects'][] = array(
					'objNum' => $options['objNum'],
					'label'  => $options['label'],
				);
				break;
		...
		switch ( $action ) {
			...
			case 'add':
				if ( $id === 0 ) {
					...
					$this->o_indirect_references( $this->indirectReferenceId, 'add', array( 'EmbeddedFiles' => $id ) );
		...
		switch ( $action ) {
			...
			case 'out':
				...
				if ( $this->compressionReady ) {
					...
					$deflateContext          = deflate_init( ZLIB_ENCODING_DEFLATE, array( 'level' => 6 ) );
	...
	function output( $debug = false ) {
		...
		if ( $this->javascript ) {
			...
			$this->o_indirect_references( $this->indirectReferenceId, 'add', array( 'JavaScript' => $js_id ) );
		}
		...
		if ( count( $this->byteRange ) > 0 ) {
			foreach ( $this->byteRange as $k => $v ) {
				$tmp = 'o_' . $v['t'];
				$this->$tmp( $k, 'byterange', array( 'content' => &$content ) );
			}
		}
	...
	function selectFont( $fontName, $encoding = '', $set = true, $isSubsetting = true ) {
		...
		if ( ! isset( $this->fonts[ $fontName ] ) ) {
			...
			if ( isset( $this->fonts[ $fontName ] ) ) {
				...
				} elseif ( mb_strlen( $encoding, '8bit' ) ) {
	...
	function setLineTransparency( $mode, $opacity ) {
		...
		if ( is_null( $this->currentLineTransparency ) ) {
			$this->currentLineTransparency = array();
		}
	...
	function setFillTransparency( $mode, $opacity ) {
		...
		if ( is_null( $this->currentFillTransparency ) ) {
			$this->currentFillTransparency = array();
		}
	...
	function setXObjectResource( $numXObject, $type, $options ) {
		if ( in_array( $type, array( 'procset', 'font', 'xObject' ) ) ) {
	 ...
	 * @return int
	 */
	public function addFormField( $type, $name, $x0, $y0, $x1, $y1, $ff = 0, $size = 10.0, $color = array( 0, 0, 0 ) ) {
		if ( ! $this->numFonts ) {
			$this->selectFont( $this->defaultFont );
		}
		...
		$this->o_acroform(
			...
			'font',
			array(
				'objNum'  => $font[0]['objNum'],
				'fontNum' => $font[0]['fontNum'],
			)
		);
	...
	public function setFormFieldValue( $numFieldObj, $value ) {
		$this->o_field( $numFieldObj, 'set', array( 'value' => $value ) );
	}
	...
	public function setFormFieldRefValue( $numFieldObj, $numObj ) {
		$this->o_field( $numFieldObj, 'set', array( 'refvalue' => $numObj ) );
	}
	...
	public function setFormFieldAppearance( $numFieldObj, $normalNumObj, $rolloverNumObj = null, $downNumObj = null ) {
		...
		$this->o_field( $numFieldObj, 'set', array( 'appearance' => $appearance ) );
	}
	...
	public function setFormFieldOpt( $numFieldObj, $value ) {
		$this->o_field( $numFieldObj, 'set', array( 'options' => $value ) );
	}
	...
	protected function addImagePngAlpha( $file, $x, $y, $w, $h, $byte ) {
		...
		$this->imageAlphaList[ $file ] = array( $tempfile_alpha, $tempfile_plain );
