...
abstract class AbstractRenderer {
	...
	protected function _render_outline( Frame $frame, array $border_box, string $corner_style = 'bevel' ): void {
		...
		[$x, $y, $w, $h]     = $border_box;
		$d                   = $width + $offset;
		$outline_box         = array( $x - $d, $y - $d, $w + $d * 2, $h + $d * 2 );
	...
	protected function _background_image( $url, $x, $y, $width, $height, $style ) {
		...
		$key = implode( '_', array( $bg_width, $bg_height, $img_w, $img_h, $bg_x, $bg_y, $repeat ) );
	...
	protected function dashPattern( string $style, float $width, float $length ): array {
		...
		if ( $style === 'dotted' ) {
			...
			return array( array( 0, $s ), 'round' );
		}

		return array( array(), 'butt' );
	}
			...
			case 'top':
				if ( $cornerClip ) {
					$points = array(
						...
						$y,
						$x,
						$y - 1, // Extend outwards to avoid gaps
						$x + $length,
						$y - 1, // Extend outwards to avoid gaps
						$x + $length,
						$y,
			...
			case 'left':
				if ( $cornerClip ) {
					$points = array(
						...
						$y,
						$x - 1,
						$y, // Extend outwards to avoid gaps
						$x - 1,
						$y + $length, // Extend outwards to avoid gaps
						$x,
						$y + $length,
						$x + max( $width, $r2 ),
