		...
		switch ( $style->white_space ) {
			...
			case 'pre-wrap':
				...
				if ( $style->overflow_wrap === 'anywhere' ) {
					...
					$words   = preg_split( self::$_wordbreak_pattern, $text, -1, PREG_SPLIT_DELIM_CAPTURE );
					$lengths = array_map(
						function ( $chunk ) use ( $fontMetrics, $font, $size, $word_spacing, $letter_spacing ) {
			...
			case 'pre':
				...
				array_walk(
					$lines,
					function ( &$chunked_text_width, $chunked_text ) use ( $fontMetrics, $font, $size, $word_spacing, $letter_spacing ) {
