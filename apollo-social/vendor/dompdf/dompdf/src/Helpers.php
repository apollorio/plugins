...
class Helpers {
	...
	public static function imagecreatefrombmp( $filename, $context = null ) {
		...
		switch ( $meta['compression'] ) {
			case 1:
				$data = self::rle8_decode( $data, $meta['width'] );
				break;
			case 2:
				$data = self::rle4_decode( $data, $meta['width'] );
				break;
		}
			...
			if ( $is_local_path || ini_get( 'allow_url_fopen' ) || ! $can_use_curl ) {
				...
				if ( $maxlen > 0 ) {
					...
					curl_setopt(
						...
						function ( $res, $download_size_total, $download_size, $upload_size_total, $upload_size ) use ( $maxlen ) {
