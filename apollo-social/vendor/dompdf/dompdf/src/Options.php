...
class Options {
	 ...
	 * @var array
	 */
	private $allowedProtocols = array(
	...
	public function __construct( array $attributes = null ) {
		...
		$this->setAllowedProtocols( array( 'file://', 'http://', 'https://' ) );

		$this->setArtifactPathValidation( array( $this, 'validateArtifactPath' ) );
	...
	public function setAllowedProtocols( array $allowedProtocols ) {
		...
		foreach ( $allowedProtocols as $protocol => $config ) {
			if ( is_string( $protocol ) ) {
				...
				$protocols[ $config ] = array();
			}
		}
		$this->allowedProtocols = array();
		foreach ( $protocols as $protocol => $config ) {
			$this->addAllowedProtocol( $protocol, ...( $config['rules'] ?? array() ) );
		}
		return $this;
	}
	...
	public function addAllowedProtocol( string $protocol, callable ...$rules ) {
		...
		if ( empty( $rules ) ) {
			...
			switch ( $protocol ) {
				case 'file://':
					$rules[] = array( $this, 'validateLocalUri' );
					break;
				case 'http://':
				case 'https://':
					$rules[] = array( $this, 'validateRemoteUri' );
					break;
				case 'phar://':
					$rules[] = array( $this, 'validatePharUri' );
					break;
			}
		}
		$this->allowedProtocols[ $protocol ] = array( 'rules' => $rules );
		return $this;
	}
	...
	public function validateLocalUri( string $uri ) {
		...
		if ( $chrootValid !== true ) {
			return array( false, 'Permission denied. The file could not be found under the paths specified by Options::chroot.' );
		}
