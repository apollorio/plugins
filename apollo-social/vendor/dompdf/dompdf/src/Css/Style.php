...
class Style {
	 ...
	 */
	public const TEXT_ALIGN_KEYWORDS = array( 'left', 'right', 'center', 'justify' );

	/**
	 * List of valid vertical-align keywords.
	 */
	public const VERTICAL_ALIGN_KEYWORDS = array(
		'baseline',
		'bottom',
		'middle',
		'sub',
	 ...
	 */
	public const BLOCK_LEVEL_TYPES = array(
		'block',
		// "flow-root",
		'list-item',
		// "flex",
	 ...
	 */
	public const INLINE_LEVEL_TYPES = array(
		'inline',
		'inline-block',
		// "inline-flex",
		// "inline-grid",
	 ...
	 */
	public const TABLE_INTERNAL_TYPES = array(
		'table-row-group',
		'table-header-group',
		'table-footer-group',
		'table-row',
	 ...
	 */
	public const INLINE_TYPES = array( 'inline' );

	/**
	 * List of all block (inner) display types.
	 */
	public const BLOCK_TYPES = array( 'block', 'inline-block', 'table-cell', 'list-item' );

	/**
	 * List of all table (inner) display types.
	 */
	public const TABLE_TYPES = array( 'table', 'inline-table' );
	 ...
	 * @var array
	 */
	protected static $valid_display_types = array();

	/**
	 * List of all positioned types.
	 */
	public const POSITIONED_TYPES = array( 'relative', 'absolute', 'fixed' );

	/**
	 * List of valid border styles.
	 */
	public const BORDER_STYLES = array(
		'none',
		'hidden',
		'dotted',
		'dashed',
	 ...
	 */
	protected const OUTLINE_STYLES = array(
		'auto',
		'none',
		'dotted',
		'dashed',
	 ...
	 * @var array
	 */
	protected static $_props_shorthand = array(
		'background'    => array(
			'background_image',
			'background_position',
			'background_size',
	 ...
	 * @var array
	 */
	protected static $_props_alias = array(
	 ...
	 * @var array
	 */
	protected $non_final_used = array();
	 ...
	 * @var array
	 */
	protected static $_dependent_props = array();
	...
	public function __construct( Stylesheet $stylesheet, int $origin = Stylesheet::ORIG_AUTHOR ) {
		...
		if ( ! isset( self::$_defaults ) ) {
			...
			$d['background_position']         = array( '0%', '0%' );
			$d['background_repeat']           = 'repeat';
			$d['background']                  = '';
			$d['border_collapse']             = 'separate';
			$d['border_color']                = '';
			$d['border_spacing']              = array( 0.0, 0.0 );
			$d['border_style']                = '';
			$d['border_top']                  = '';
			$d['border_right']                = '';
			$d['border_bottom']               = '';
			...
			// CSS3
			$d['opacity']          = 1.0;
			$d['background_size']  = array( 'auto', 'auto' );
			$d['transform']        = 'none';
			...
			self::$valid_display_types = array(
				'none'                => true,
				'-dompdf-br'          => true,
				'-dompdf-image'       => true,
	...
	protected function set_quad_shorthand( string $prop, string $value ): array {
		...
		switch ( \count( $v ) ) {
			case 1:
				$values = array( $v[0], $v[0], $v[0], $v[0] );
				break;
			case 2:
				$values = array( $v[0], $v[1], $v[0], $v[1] );
				break;
			case 3:
				$values = array( $v[0], $v[1], $v[2], $v[1] );
				break;
			case 4:
				$values = array( $v[0], $v[1], $v[2], $v[3] );
				break;
			default:
				return array();
		}
	...
	protected function _compute_background_repeat( string $val ) {
		$keywords = array( 'repeat', 'repeat-x', 'repeat-y', 'no-repeat' );
	...
	protected function _set_font( string $value ): array {
		...
		$hasLineHeight     = $lineFamily !== array() && $lineFamily[0] === '/';
		$lineHeight        = $hasLineHeight ? \array_slice( $lineFamily, 1, 1 ) : array();
	...
	protected function parse_border_side( string $value, array $styles = self::BORDER_STYLES ): ?array {
		...
		return array( $width, $style, $color );
	}
	...
	protected function _get_transform_origin( $computed ) {
		...
		$values = array_map(
			function ( $value ) {
				if ( \in_array( $value, array( 'top', 'left' ), true ) ) {
					return 0;
				} elseif ( \in_array( $value, array( 'bottom', 'right' ), true ) ) {
					return '100%';
				} else {
					return $value;
				}
	...
	public function __toString(): string {
		...
		return print_r(
			array_merge(
				array( 'parent_font_size' => $parent_font_size ),
				$this->_props
			),
			true
		);
