...
class Cache {
	 ...
	 * @var array
	 */
	protected static $tempImages = array();
	 ...
	 * @var array
	 */
	protected static $svgRefs = array();
	...
	static function resolve_url( $url, $protocol, $host, $base_path, Options $options ) {
		...
		try {
			...
			if ( ( $width && $height && in_array( $type, array( 'gif', 'png', 'jpeg', 'bmp', 'svg', 'webp' ), true ) ) === false ) {
	...
	static function addTempImage( string $filePath, string $tempPath, string $key = 'default' ): void {
		if ( ! isset( self::$tempImages[ $filePath ] ) ) {
			self::$tempImages[ $filePath ] = array();
		}
