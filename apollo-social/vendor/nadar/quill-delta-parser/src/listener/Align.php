...
class Align extends BlockListener {
	 ...
	 */
	public $alignments = array( 'center', 'right', 'justify', 'left' );
	...
	public function process( Line $line ) {
		...
		if ( $alignment ) {
			$this->pick( $line, array( 'alignment' => $alignment ) );
			$line->setDone();
		}
	}
	...
	public function render( Lexer $lexer ) {
		...
		$this->wrapElement( '<p style="text-align: {alignment};">{__buffer__}</p>', array( 'alignment' ) );
	}
}
