...
class Lists extends BlockListener {
	...
	public function process( Line $line ) {
		...
		if ( $listType ) {
			$this->pick( $line, array( 'type' => $listType ) );
			$line->setDone();
		}
	}
	...
	public function render( Lexer $lexer ) {
		...
		foreach ( $this->picks() as $pick ) {
			...
			if ( ! $isEmpty ) {
				$first->while(
					static function ( &$index, Line $line ) use ( &$buffer, $pick ) {
						++$index;
			...
			$hasNextInside = false;
			$pick->line->whileNext(
				static function ( Line $line ) use ( &$hasNextInside ) {
			...
			$isCheck = in_array( $this->getPickType( $pick ), array( self::LIST_TYPE_CHECKED, self::LIST_TYPE_UNCHECKED ), true );
			...
			$nextIndent = 0;
			$pick->line->whileNext(
				static function ( Line $line ) use ( &$nextIndent ) {
	...
	protected function getListAttribute( Pick $pick ) {
		...
		if ( in_array( $type, array( self::LIST_TYPE_BULLET, self::LIST_TYPE_CHECKED, self::LIST_TYPE_UNCHECKED ), true ) ) {
			return 'ul';
		}
