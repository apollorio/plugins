...
class Video extends BlockListener {
	 ...
	 * @since 2.5.0
	 */
	public $allow = array( 'accelerometer', 'autoplay', 'encrypted-media', 'gyroscope', 'picture-in-picture' );
	...
	public function process( Line $line ) {
		...
		if ( $embedUrl ) {
			$line->output = str_replace( array( '{url}', '{allow}' ), array( $line->getLexer()->escape( $embedUrl ), implode( '; ', $this->allow ) ), $this->wrapper );
			$line->setDone();
		}
	}
}
