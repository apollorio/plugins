...
class Lexer {
	...
	protected $listeners = array(
		Listener::TYPE_INLINE => array(
			Listener::PRIORITY_EARLY_BIRD        => array(),
			Listener::PRIORITY_GARBAGE_COLLECTOR => array(),
		),
		Listener::TYPE_BLOCK  => array(
			Listener::PRIORITY_EARLY_BIRD        => array(),
			Listener::PRIORITY_GARBAGE_COLLECTOR => array(),
		),
	);
	...
	protected function opsToLines( array $ops ) {
		...
		foreach ( $ops as $delta ) {
			...
			if ( $insert == self::NEWLINE_EXPRESSION ) {
				$lines[ $i ] = new Line( $i, '', $delta['attributes'] ?? array(), $this, true, true );
				++$i;
			} else {
				...
				foreach ( $parts as $index => $value ) {
					...
					$lines[ $i ] = new Line( $i, $value, $delta['attributes'] ?? array(), $this, $hadEndNewline, $hasNewline );
					++$i;
				}
			}
		}
