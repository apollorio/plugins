...
abstract class BlockListener extends Listener {
	 ...
	 * @return void
	 */
	protected function wrapElement( $wrapper, array $options = array() ) {
		$search = array( '{__buffer__}' );
		...
		foreach ( $this->picks() as $pick ) {
			...
			$buffer = null;
			$first->while(
				static function ( &$index, Line $line ) use ( &$buffer, $pick, $first ) {
					++$index;
