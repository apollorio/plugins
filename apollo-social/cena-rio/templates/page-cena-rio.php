 ...
 *
 * ACCESS CONTROL:
 * - Requires 'view_industry_content' capability OR 'administrator' role
 * - Non-authorized users receive 403 wp_die()
 */
...
// PHASE 5: Gated industry access with proper capability check
if ( ! current_user_can( 'view_industry_content' ) && ! current_user_can( 'administrator' ) ) {
	wp_die(
		esc_html__( 'Acesso negado. Você não possui permissão para visualizar o conteúdo da indústria.', 'apollo-social' ),
	...
	<aside class="ap-dashboard-sidebar">
		...
		<nav class="ap-sidebar-nav">
			<a href="<?php echo esc_url( home_url( '/cena-rio' ) ); ?>"
				class="ap-nav-item <?php echo 'dashboard' === $current_tab ? 'active' : ''; ?>"
			...
			<a href="<?php echo esc_url( add_query_arg( 'tab', 'documents', home_url( '/cena-rio' ) ) ); ?>"
				class="ap-nav-item <?php echo 'documents' === $current_tab ? 'active' : ''; ?>"
			...
			<a href="<?php echo esc_url( add_query_arg( 'tab', 'plans', home_url( '/cena-rio' ) ) ); ?>"
				class="ap-nav-item <?php echo 'plans' === $current_tab ? 'active' : ''; ?>"
				data-ap-tooltip="Planos e cronogramas de eventos">
				<i class="ri-calendar-line"></i>
				<span>Planos de Evento</span>
			</a>

			<a href="<?php echo esc_url( add_query_arg( 'tab', 'vendors', home_url( '/cena-rio' ) ) ); ?>"
				class="ap-nav-item <?php echo 'vendors' === $current_tab ? 'active' : ''; ?>"
				data-ap-tooltip="<?php esc_attr_e( 'Visualizar fornecedores cadastrados', 'apollo-social' ); ?>">
				<i class="ri-store-2-line"></i>
				<span><?php esc_html_e( 'Fornecedores', 'apollo-social' ); ?></span>
			</a>
		...
		<div class="ap-dashboard-content">
			...
			switch ( $current_tab ) {
				...
				case 'plans':
					...
					break;
				case 'vendors':
					include APOLLO_SOCIAL_PLUGIN_DIR . 'cena-rio/templates/vendors-list.php';
