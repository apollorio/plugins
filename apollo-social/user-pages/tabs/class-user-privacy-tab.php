...
class Apollo_User_Privacy_Tab {
	...
	public static function init() {
		add_action( 'apollo_user_page_tabs', array( __CLASS__, 'add_privacy_tab' ), 20 );
		add_action( 'apollo_user_page_tab_content', array( __CLASS__, 'render_privacy_content' ), 20 );
		add_action( 'wp_ajax_apollo_save_privacy_settings', array( __CLASS__, 'ajax_save_privacy' ) );
	}
	...
	public static function ajax_save_privacy() {
		...
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => __( 'You must be logged in', 'apollo-social' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Privacy settings saved', 'apollo-social' ) ) );
	}
}
