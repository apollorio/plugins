...
(function ($) {
  ...
  // REST config - use multiple fallback sources.
  const getRestConfig = function () {
    const mainConfig = window.apolloBuilderConfig || {};
    const assetsConfig = window.apolloBuilderAssetsConfig || {};
    const wpSettings = window.wpApiSettings || {};

    return {
      namespace: mainConfig.restUrl || assetsConfig.restUrl || '/wp-json/apollo-social/v1/builder',
      nonce: mainConfig.restNonce || assetsConfig.restNonce || wpSettings.nonce || ''
    };
  };
  ...
  function init() {
    console.log('[Apollo Assets] Initializing...');
    ...
    loadAssets().then(() => {
      ...
      console.error('[Apollo Assets] Init failed:', err);
    });
  ...
  function loadAssets() {
    ...
    const url = config.namespace.includes('/wp-json/') ? config.namespace + '/assets' : '/wp-json' + config.namespace + '/assets';

    console.log('[Apollo Assets] Loading from:', url);
    ...
    return fetch(url, {
      ...
      .catch(err => {
        ...
        // Fallback to empty arrays - don't block the builder.
        ApolloAssets.isLoaded = true;
      });
  }
  ...
  function loadCurrentLayout() {
    ...
    const url = config.namespace.includes('/wp-json/') ? config.namespace + '/layout' : '/wp-json' + config.namespace + '/layout';
  ...
  function getApiUrl(endpoint) {
    const config = getRestConfig();
    const base = config.namespace.includes('/wp-json/') ? config.namespace : '/wp-json' + config.namespace;
  ...
  function saveBackground(backgroundId) {
    ...
    return fetch(getApiUrl('/background'), {
      method: 'POST',
      headers: {
  ...
  function addStickerToLayout(assetId, x, y) {
    ...
    return fetch(getApiUrl('/stickers'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-WP-Nonce': config.nonce
  ...
  function updateStickerPosition(instanceId, x, y) {
    ...
    return fetch(getApiUrl('/stickers/' + instanceId), {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'X-WP-Nonce': config.nonce
  ...
  function removeSticker(instanceId) {
    ...
    return fetch(getApiUrl('/stickers/' + instanceId) + '?user_id=' + userId, {
      method: 'DELETE',
      headers: {
        'X-WP-Nonce': config.nonce
      }
