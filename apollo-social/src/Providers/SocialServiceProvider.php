...
final class SocialServiceProvider {
	...
	public static function boot(): void {
		...
		add_action('apollo_daily_cron',[self::class,'dailyCron']);
		add_action('apollo_hourly_cron',[self::class,'hourlyCron']);
	...
	public static function registerShortcodes(): void {
		add_shortcode('apollo_members_directory',[self::class,'membersDirectoryShortcode']);
		add_shortcode('apollo_activity_feed',[self::class,'activityFeedShortcode']);
		add_shortcode('apollo_groups_directory',[self::class,'groupsDirectoryShortcode']);
		add_shortcode('apollo_leaderboard',[self::class,'leaderboardShortcode']);
		add_shortcode('apollo_online_users',[self::class,'onlineUsersShortcode']);
		add_shortcode('apollo_my_profile',[self::class,'myProfileShortcode']);
		add_shortcode('apollo_team_members',[self::class,'teamMembersShortcode']);
		add_shortcode('apollo_testimonials',[self::class,'testimonialsShortcode']);
		add_shortcode('apollo_map',[self::class,'mapShortcode']);
		add_shortcode('apollo_notices',[self::class,'noticesShortcode']);
	}

	public static function enqueueAssets(): void {
		if(is_user_logged_in()){
			wp_localize_script('jquery','apolloSocial',[
	...
	public static function enqueueAdminAssets(): void {
		wp_localize_script('jquery','apolloSocialAdmin',[
			'ajaxUrl'=>admin_url('admin-ajax.php'),
	...
	public static function membersDirectoryShortcode(array $atts=[]): string {
		$a=shortcode_atts(['limit'=>20,'show_search'=>'true'],$atts);
		$members=\Apollo\Modules\Members\MembersDirectoryRepository::search(['limit'=>(int)$a['limit']]);
		ob_start();
		...
		foreach($members as $m){
			...
			echo'<img src="'.esc_url(get_avatar_url($m['ID'],['size'=>80])).'" alt="'.esc_attr($m['display_name']).'">';
	...
	public static function activityFeedShortcode(array $atts=[]): string {
		$a=shortcode_atts(['limit'=>20,'user_id'=>null],$atts);
		$args=['limit'=>(int)$a['limit']];
		if($a['user_id'])$args['user_id']=(int)$a['user_id'];
	...
	public static function groupsDirectoryShortcode(array $atts=[]): string {
		$a=shortcode_atts(['limit'=>20],$atts);
		$groups=\Apollo\Modules\Groups\GroupsRepository::search(['limit'=>(int)$a['limit']]);
		ob_start();
	...
	public static function leaderboardShortcode(array $atts=[]): string {
		$a=shortcode_atts(['limit'=>10,'type'=>'points'],$atts);
	...
	public static function onlineUsersShortcode(array $atts=[]): string {
		$a=shortcode_atts(['limit'=>20],$atts);
	...
	public static function myProfileShortcode(array $atts=[]): string {
	...
	public static function teamMembersShortcode(array $atts=[]): string {
		$a=shortcode_atts(['department'=>''],$atts);
		...
		foreach($members as $m){
			...
			if($m['photo_url'])echo'<img src="'.esc_url($m['photo_url']).'" alt="'.esc_attr($m['name']).'">';
	...
	public static function testimonialsShortcode(array $atts=[]): string {
		$a=shortcode_atts(['limit'=>5,'featured'=>'false'],$atts);
		...
		foreach($items as $t){
			...
			if($t['author_title'])echo', '.esc_html($t['author_title']);
	...
	public static function mapShortcode(array $atts=[]): string {
		$a=shortcode_atts(['category'=>'','height'=>'400px'],$atts);
	...
	public static function noticesShortcode(array $atts=[]): string {
		...
		foreach($notices as $n){
			...
			if($n['is_dismissible'])echo'<button class="dismiss" data-notice="'.esc_attr($n['id']).'">×</button>';
			echo'</div>';
		}
		echo'</div>';
		return ob_get_clean();
