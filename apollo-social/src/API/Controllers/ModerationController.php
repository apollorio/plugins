...
class ModerationController
{
    ...
    {
        register_rest_route('apollo/v1', '/groups/(?P<id>\d+)/approve', [
            'methods' => 'POST',
        ...
        register_rest_route('apollo/v1', '/groups/(?P<id>\d+)/reject', [
            'methods' => 'POST',
        ...
        register_rest_route('apollo/v1', '/groups/(?P<id>\d+)/resubmit', [
            'methods' => 'POST',
        ...
        register_rest_route('apollo/v1', '/groups/(?P<id>\d+)/status', [
            'methods' => 'GET',
    ...
    public function approve_group(WP_REST_Request $request): WP_REST_Response
    {
        ...
        // Approve the group
        $result = $this->moderation->approveGroup($group_id, $moderator_id);
    ...
    public function reject_group(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $result = $this->moderation->rejectGroup($group_id, $moderator_id, $reason);
    ...
    public function get_group_status(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if ($group['status'] === 'rejected') {
            $rejection_notice = $this->moderation->getRejectionNotice($group_id);
            if ($rejection_notice) {
