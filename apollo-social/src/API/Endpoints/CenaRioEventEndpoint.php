...
class CenaRioEventEndpoint {
	...
	public function registerRoutes(): void {
		...
		register_rest_route(
			'apollo/v1',
			'cena-rio/agenda/(?P<id>\d+)/aprovar',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
	...
	/**
	 * Permission check (contributor role = cena::rio)
	 */
	public function permissionCheck( WP_REST_Request $request ): bool|WP_Error {
		...
		// Uses standard WP contributor role (cena::rio label)
		if ( ! in_array( 'contributor', $user->roles, true ) && ! current_user_can( 'manage_options' ) ) {
			return new WP_Error(
				'rest_forbidden',
