...
class WidgetsEndpoints {
	...
	public function register() {
		add_action( 'wp_ajax_apollo_save_widgets', [ $this, 'saveWidgets' ] );
		add_action( 'wp_ajax_apollo_add_depoimento', [ $this, 'addDepoimento' ] );
		add_action( 'wp_ajax_apollo_delete_widget', [ $this, 'deleteWidget' ] );
	}
	...
	public function saveWidgets() {
		...
		if ( ! $page_id ) {
			wp_send_json_error( [ 'message' => 'ID da página inválido' ] );
			return;
		}
		...
		if ( ! is_array( $widgets ) ) {
			wp_send_json_error( [ 'message' => 'Dados de widgets inválidos' ] );
			return;
		}
	...
	public function addDepoimento() {
		...
		if ( ! $user_page ) {
			wp_send_json_error( [ 'message' => 'Página do usuário não encontrada' ] );
			return;
		}
		...
		if ( ! $depoimentos_widget || ( $depoimentos_widget['config']['allow_comments'] ?? true ) === false ) {
			wp_send_json_error( [ 'message' => 'Comentários não permitidos' ] );
			return;
		}
		...
		if ( ! $comment_id ) {
			wp_send_json_error( [ 'message' => 'Erro ao criar depoimento' ] );
			return;
		}
	...
	public function deleteWidget() {
		...
		if ( ! is_array( $widgets ) ) {
			wp_send_json_error( [ 'message' => 'Widgets não encontrados' ] );
			return;
		}
		...
		wp_send_json_success( [ 'message' => 'Widget removido' ] );
	}
