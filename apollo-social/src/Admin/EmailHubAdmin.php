...
class EmailHubAdmin {
	...
	public static function registerSettings(): void {
		register_setting(
			'apollo_email_hub_group',
			self::OPTION_KEY,
			array(
				'type'              => 'array',
				'sanitize_callback' => array( self::class, 'sanitizeSettings' ),
				'default'           => array(),
			)
		);
	}
	...
	public static function sanitizeSettings( $input ): array {
		...
		// Checkbox fields (notifications)
		$checkboxes = array( 'welcome_email', 'membership_emails', 'event_reminders', 'weekly_digest', 'security_alerts' );
			...
			'events' => array(
				...
				'templates'   => array(
					...
					'event_response' => array(
						'name'                  => 'Resposta de Convidado',
						'description'           => 'Quando um convidado responde ao convite',
						'trigger'               => 'apollo_event_rsvp',
						'icon'                  => '💬',
						'required_placeholders' => array( '[display-name]', '[event-name]', '[guest-name]' ),
						'default_subject'       => '💬 [guest-name] respondeu ao evento [event-name]',
						'default_body'          => self::getDefaultEventResponseTemplate(),
					),
					'event_djs_update' => array(
						'name'                  => 'Atualização de Line-up',
						'description'           => 'Quando os DJs do evento são atualizados',
						'trigger'               => 'apollo_event_djs_updated',
						'icon'                  => '🎧',
						'required_placeholders' => array( '[display-name]', '[event-name]', '[event-djs]' ),
						'default_subject'       => '🎧 Novidade na programação da [event-name]!',
						'default_body'          => self::getDefaultEventDJsUpdateTemplate(),
					),
					'event_category_update' => array(
						'name'                  => 'Categoria Atualizada',
						'description'           => 'Quando a categoria do evento muda',
						'trigger'               => 'apollo_event_category_updated',
						'icon'                  => '📂',
						'required_placeholders' => array( '[display-name]', '[event-name]' ),
						'default_subject'       => '📂 Categoria atualizada: [event-name]',
						'default_body'          => self::getDefaultEventCategoryUpdateTemplate(),
					),
					'weekly_digest'   => array(
						'name'                  => 'Digest Semanal',
	 ...
	 *
	 * Uses Apollo Design System tokens from external CSS file.
	 * Replaces inline styles for better performance and consistency.
	 *
	 * @since 2.5.0 - Migrated to external CSS with Apollo tokens
	 */
	public static function enqueueAssets( string $hook ): void {
		...
		// Load Apollo Design System (uni.css) if available
		if ( function_exists( 'apollo_enqueue_global_assets' ) ) {
			apollo_enqueue_global_assets();
		}

		// Email Hub dedicated stylesheet with Apollo tokens
		wp_enqueue_style(
			'apollo-email-hub-admin',
			APOLLO_SOCIAL_PLUGIN_URL . 'assets/css/admin/email-hub.css',
			array(),
			defined( 'APOLLO_SOCIAL_VERSION' ) ? APOLLO_SOCIAL_VERSION : '2.5.0'
		);
		...
		// Email Hub JavaScript functionality
...
<body style="margin:0;padding:0;background:#1a1a2e;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;">
	...
	/**
	 * Get admin styles (DEPRECATED)
	 *
	 * @deprecated 2.5.0 Use external CSS file assets/css/admin/email-hub.css instead.
	 *             This method is kept for backwards compatibility only.
	 *             All styles have been migrated to use Apollo Design System tokens.
	 *
	 * @return string Empty string - styles now loaded via external CSS
	 */
	private static function getAdminStyles(): string {
		// Styles migrated to: assets/css/admin/email-hub.css
		// Using Apollo Design System tokens (--ap-*)
		// This method now returns empty to prevent duplicate styles
		return '';
	}
...
<p style="color:#888;font-size:12px;text-align:center;">
	...
	private static function getDefaultEventResponseTemplate(): string {
		return '<h2 style="color:#00d4ff;margin-top:0;">💬 Resposta de convidado</h2>

<p>Olá, [display-name]!</p>

<p><strong>[guest-name]</strong> respondeu ao convite do seu evento <strong>[event-name]</strong>.</p>

<div style="background:rgba(0,212,255,0.1);padding:20px;border-radius:8px;margin:20px 0;">
    <p style="margin:0;font-size:16px;">{{response_label}}</p>
</div>
...
<p style="color:#888;font-size:12px;text-align:center;">
	...
	private static function getDefaultEventDJsUpdateTemplate(): string {
		return '<h2 style="color:#667eea;margin-top:0;">🎧 Novidade na programação!</h2>

<p>E aí, [display-name]!</p>

<p>O line-up do evento <strong>[event-name]</strong> foi atualizado!</p>

<div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);padding:24px;border-radius:12px;margin:20px 0;color:#fff;">
    <p style="margin:0;font-size:18px;font-weight:bold;">Line-up:</p>
    <p style="margin:10px 0 0;font-size:16px;">[event-djs]</p>
</div>
...
<p style="color:#888;font-size:12px;text-align:center;">
	...
	private static function getDefaultEventCategoryUpdateTemplate(): string {
		return '<h2 style="color:#f0ad4e;margin-top:0;">📂 Categoria atualizada</h2>

<p>Olá, [display-name]!</p>

<p>O evento <strong>[event-name]</strong> que você tem interesse teve sua categoria alterada.</p>

<div style="background:rgba(240,173,78,0.1);padding:20px;border-radius:8px;margin:20px 0;">
    <p style="margin:0;"><strong>Nova categoria:</strong> {{new_categories}}</p>
</div>
...
<p style="color:#888;font-size:12px;text-align:center;">
	...
	private static function getDefaultCommunityInviteTemplate(): string {
		return '<h2 style="color:var(--primary);margin-top:0;">🤝 Convite para a Comunidade</h2>

<p>Olá!</p>

<p><strong>[inviter-name]</strong> está convidando você para fazer parte da nossa comunidade Apollo.</p>

<div style="background:var(--card);padding:24px;border-radius:var(--radius);margin:20px 0;border-left:4px solid var(--primary);">
	<p style="margin:0;font-size:16px;">Junte-se a produtores, DJs e amantes da cena eletrônica do Rio.</p>
</div>

<p style="text-align:center;margin-top:30px;">
	<a href="[invite-link]" style="display:inline-block;background:var(--primary);color:var(--white);padding:15px 30px;border-radius:var(--radius);text-decoration:none;font-weight:600;">
		Aceitar convite →
	</a>
</p>

<p style="color:var(--txt-muted);font-size:12px;text-align:center;">Este convite expira em 7 dias</p>';
	}

	private static function getDefaultNucleoInviteTemplate(): string {
		return '<h2 style="color:var(--accent-2);margin-top:0;">🔐 Convite para Núcleo Privado</h2>

<p>Olá, [display-name]!</p>

<p><strong>[inviter-name]</strong> te convidou para participar do núcleo <strong>[group-name]</strong>.</p>

<div style="background:var(--surface);padding:24px;border-radius:var(--radius);margin:20px 0;">
	<p style="margin:0 0 10px;"><i class="ri-team-line"></i> Núcleos são grupos privados de produtores</p>
	<p style="margin:0;color:var(--txt-muted);font-size:14px;">Acesse sua conta para aceitar o convite e colaborar com o time.</p>
</div>

<p style="text-align:center;margin-top:30px;">
	<a href="[site-url]/meu-perfil" style="display:inline-block;background:var(--accent-2);color:var(--white);padding:15px 30px;border-radius:var(--radius);text-decoration:none;font-weight:600;">
		Acessar minha conta →
	</a>
</p>

<p style="color:var(--txt-muted);font-size:12px;text-align:center;">
	<a href="[dashboard-url]" style="color:var(--txt-muted);">Gerenciar preferências de email</a>
</p>';
	}

	private static function getDefaultNucleoApprovalTemplate(): string {
		return '<h2 style="color:var(--accent-1);margin-top:0;">✅ Aprovado no Núcleo!</h2>

<p>Parabéns, [display-name]!</p>

<p>Sua solicitação para participar do núcleo <strong>[group-name]</strong> foi aprovada.</p>

<div style="background:linear-gradient(135deg,var(--accent-1) 0%,var(--primary) 100%);padding:24px;border-radius:var(--radius);margin:20px 0;color:var(--black);">
	<p style="margin:0;font-size:18px;font-weight:bold;"><i class="ri-check-double-line"></i> Você agora faz parte do time!</p>
	<p style="margin:10px 0 0;font-size:14px;">Acesse a plataforma para começar a colaborar.</p>
</div>

<p style="text-align:center;margin-top:30px;">
	<a href="[site-url]/meu-perfil" style="display:inline-block;background:var(--black);color:var(--accent-1);padding:15px 30px;border-radius:var(--radius);text-decoration:none;font-weight:600;">
		Acessar núcleo →
	</a>
</p>
...
<p style="color:var(--txt-muted);font-size:12px;text-align:center;">
	...
	private static function getDefaultDocumentSignatureTemplate(): string {
		return '<h2 style="color:var(--orange-7);margin-top:0;">📝 Documento aguardando assinatura</h2>

<p>Olá, [display-name]!</p>

<p>Há um documento aguardando sua assinatura na plataforma Apollo.</p>

<div style="background:var(--card);padding:24px;border-radius:var(--radius);margin:20px 0;border:2px solid var(--border);">
	<p style="margin:0 0 10px;font-size:16px;font-weight:600;"><i class="ri-file-text-line"></i> [document-title]</p>
	<p style="margin:0;color:var(--txt-muted);font-size:13px;">Este link é único e válido por 30 dias</p>
</div>

<p style="text-align:center;margin-top:30px;">
	<a href="[signature-link]" style="display:inline-block;background:var(--orange-7);color:var(--white);padding:15px 30px;border-radius:var(--radius);text-decoration:none;font-weight:600;">
		<i class="ri-quill-pen-line"></i> Visualizar e assinar →
	</a>
</p>
