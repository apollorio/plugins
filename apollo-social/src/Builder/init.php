...
// Define Builder constants with guards to prevent redefinition errors
if ( ! defined( 'APOLLO_BUILDER_VERSION' ) ) {
	define( 'APOLLO_BUILDER_VERSION', '1.0.0' );
}
if ( ! defined( 'APOLLO_BUILDER_DIR' ) ) {
	define( 'APOLLO_BUILDER_DIR', plugin_dir_path( __FILE__ ) );
}
if ( ! defined( 'APOLLO_BUILDER_URL' ) ) {
	define( 'APOLLO_BUILDER_URL', plugin_dir_url( __FILE__ ) );
}

// Capability required to use builder (from Live Composer pattern)
if ( ! defined( 'APOLLO_BUILDER_CAPABILITY' ) ) {
	define( 'APOLLO_BUILDER_CAPABILITY', 'edit_posts' );
}

// Post meta key for layout storage (from WOW pattern: _wow_content)
if ( ! defined( 'APOLLO_BUILDER_META_CONTENT' ) ) {
	define( 'APOLLO_BUILDER_META_CONTENT', '_apollo_builder_content' );
}
if ( ! defined( 'APOLLO_BUILDER_META_CSS' ) ) {
	define( 'APOLLO_BUILDER_META_CSS', '_apollo_builder_css' );
}
if ( ! defined( 'APOLLO_BUILDER_META_BACKGROUND' ) ) {
	define( 'APOLLO_BUILDER_META_BACKGROUND', '_apollo_background_texture' );
}
if ( ! defined( 'APOLLO_BUILDER_META_TRAX' ) ) {
	define( 'APOLLO_BUILDER_META_TRAX', '_apollo_trax_url' );
}

// HUB::rio Linktree-style meta keys (Schema v2)
if ( ! defined( 'APOLLO_HUB_META_CONTENT' ) ) {
	define( 'APOLLO_HUB_META_CONTENT', '_apollo_hub_content' );
}
if ( ! defined( 'APOLLO_HUB_SCHEMA_VERSION' ) ) {
	define( 'APOLLO_HUB_SCHEMA_VERSION', 2 );
}
