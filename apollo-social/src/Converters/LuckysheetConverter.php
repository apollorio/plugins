...
class LuckysheetConverter {
     ...
     * @var int
     */
    const MAX_CELL_LENGTH = 32767; // Excel's limit
    ...
    private static $dangerous_patterns = array(
        '/^=\s*cmd/i',        // Command execution
        '/^=\s*exec/i',       // Execute
        '/^=\s*system/i',     // System calls
        '/^=\s*shell/i',      // Shell commands
        '/^=\s*powershell/i', // PowerShell
        '/^=\s*wscript/i',    // Windows Script Host
        '/^=\s*mshta/i',      // MSHTA
        '/^\+\s*cmd/i',       // Plus prefix injection
        '/^-\s*cmd/i',        // Minus prefix injection
        '/^@\s*cmd/i',        // At prefix injection
        '/\|\s*cmd/i',        // Pipe to command
    );
    ...
    public function __construct( array $config = array() ) {
        $this->sheet_config = wp_parse_args( $config, array(
            'sheet_name'      => 'Planilha 1',
            'default_col_width' => self::DEFAULT_COL_WIDTH,
            'default_row_height' => self::DEFAULT_ROW_HEIGHT,
            'freeze_row'      => 0,  // Freeze first N rows (header)
            'freeze_col'      => 0,  // Freeze first N columns
            'show_grid'       => true,
    ...
    public function fromArray( array $rows, array $options = array() ) {
        ...
        $sheet = array(
            ...
            'index'      => 'sheet_' . uniqid(),  // Unique ID for this sheet
            'status'     => 1,                     // 1 = active/visible
            'order'      => 0,                     // First sheet
            'hide'       => 0,                     // Not hidden
            'row'        => count( $rows ),        // Total rows
            'column'     => $max_cols,             // Total columns
            'celldata'   => $celldata,             // All cell data
            'config'     => $config,               // Sheet configuration
            'scrollLeft' => 0,                     // Scroll position
            'scrollTop'  => 0,
            'luckysheet_select_save' => array(),   // Selection state
            'zoomRatio'  => 1,                     // Zoom level
    ...
    private function createCellData( $row, $col, $value, array $options = array() ) {
        ...
        $cell_value = array(
            'v'  => $type_info['value'],   // Raw value
            'm'  => $type_info['display'], // Display/formatted value
            'ct' => array(
                'fa' => $type_info['format'], // Format string (e.g., "General", "#,##0.00")
                't'  => $type_info['type'],   // Type: s=string, n=number, b=boolean, d=date
            ),
        );

        // Apply header styling if this is a header row
        if ( ! empty( $options['is_header'] ) ) {
            $cell_value['bl'] = 1;           // Bold
            $cell_value['bg'] = '#f3f4f6';   // Light gray background
            $cell_value['ht'] = 1;           // Center align
            $cell_value['fc'] = '#1f2937';   // Dark text color
        }
    ...
    private function sanitizeCellValue( $value ) {
        ...
        if ( is_numeric( $value ) ) {
            return $value; // Numbers don't need sanitization
        }
        if ( ! is_string( $value ) ) {
            return (string) $value;
        }
    ...
    private function detectCellType( $value, $type_hint = 'auto' ) {
        ...
        if ( is_string( $value ) && strpos( $value, '=' ) === 0 ) {
            ...
            return array(
                'type'    => 'f', // Formula (custom type, Luckysheet uses 's' but stores formula)
                'value'   => $value,
                'display' => $value,
                'format'  => 'General',
            );
    ...
    private function looksLikeDate( $value ) {
        ...
        $patterns = array(
            '/^\d{2}\/\d{2}\/\d{4}$/',     // 25/12/2025
            '/^\d{4}-\d{2}-\d{2}$/',       // 2025-12-25
            '/^\d{2}-\d{2}-\d{4}$/',       // 25-12-2025
            '/^\d{1,2}\s+\w+\s+\d{4}$/',   // 25 December 2025
        );
    ...
    private function formatMergedCells( array $merged_cells ) {
        ...
        foreach ( $merged_cells as $index => $def ) {
            ...
            $merge[ $key ] = array(
                'r'  => (int) $def['r'],   // Start row
                'c'  => (int) $def['c'],   // Start column
                'rs' => (int) $def['rs'],  // Row span
                'cs' => (int) $def['cs'],  // Column span
            );
        }
