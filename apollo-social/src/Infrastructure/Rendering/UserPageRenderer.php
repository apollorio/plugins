...
class UserPageRenderer
{
    ...
    {
        $param = $template_data['param']; // user ID or login
        ...
        <div class="apollo-user-profile">
            <div class="user-header">
                <div class="user-avatar">
                    <img src="https://via.placeholder.com/150x150?text=<?= esc_attr(substr($user_data['name'], 0, 1)) ?>" 
                         alt="Avatar de <?= esc_attr($user_data['name']) ?>">
                </div>
                <div class="user-info">
                    <h1><?= esc_html($user_data['name']) ?></h1>
                    <p class="user-login">@<?= esc_html($user_data['login']) ?></p>
                    <p class="user-joined">Membro desde <?= esc_html($user_data['joined']) ?></p>
                </div>
            </div>
