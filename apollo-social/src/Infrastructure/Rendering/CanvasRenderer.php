...
class CanvasRenderer {
	...
	private function renderCanvasLayout( $route_config, $handler_output, $template_data ) {
		...
		if ( ! empty( $handler_output['raw'] ) || ! empty( $route_config['raw_html'] ) ) {
			// Raw HTML templates output their own complete HTML structure
			// Do not sanitize - template is trusted internal code
			if ( isset( $handler_output['content'] ) ) {
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Raw template output
				echo $handler_output['content'];
			}
		...
		// Output and clean buffer (do not sanitize full layout HTML).
		echo ob_get_clean();
	}
