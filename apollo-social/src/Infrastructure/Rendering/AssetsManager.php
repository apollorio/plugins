...
class AssetsManager {
	 ...
	 * Now uses Apollo Core's centralized Global Assets Manager
	 */
	private function enqueueCanvasCSS() {
		// Use Apollo Core's centralized Global Assets Manager if available
		if ( function_exists( 'apollo_enqueue_global_assets' ) ) {
			apollo_enqueue_global_assets( 'css' );
		} else {
			// Fallback: Load uni.css from CDN directly
			wp_enqueue_style(
				'apollo-uni-css',
