...
final class SpamRepository {
	...
	public static function unmarkSpammer( int $userId, int $unmarkedBy ): bool {
		global $wpdb;
		$t      = $wpdb->prefix . self::TABLE;
		$result = $wpdb->delete( $t, array( 'user_id' => $userId ), array( '%d' ) ) !== false;
		if ( $result ) {
	...
	public static function removeFromBlacklist( int $id ): bool {
		global $wpdb;
		$t = $wpdb->prefix . 'apollo_spam_blacklist';
		return $wpdb->delete( $t, array( 'id' => $id ), array( '%d' ) ) !== false;
	}
	...
	public static function getBlacklist( string $type = 'all' ): array {
		...
		return $wpdb->get_results( "SELECT * FROM {$t}{$where} ORDER BY created_at DESC", ARRAY_A ) ?? array();
	}
	...
	public static function getSpamScore( int $userId ): int {
		...
		if ( $recentPosts > 10 ) {
			$score += ( $recentPosts - 10 ) * 2;}
		$regDate = get_userdata( $userId )->user_registered ?? null;
		if ( $regDate && strtotime( $regDate ) > strtotime( '-24 hours' ) ) {
			$score += 10;}
	...
	private static function hideSpammerContent( int $userId ): void {
		global $wpdb;
		$wpdb->update( $wpdb->prefix . 'apollo_activities', array( 'is_hidden' => 1 ), array( 'user_id' => $userId ), array( '%d' ), array( '%d' ) );
		$wpdb->update( $wpdb->prefix . 'apollo_forum_topics', array( 'is_closed' => 1 ), array( 'user_id' => $userId ), array( '%d' ), array( '%d' ) );
		$wpdb->update( $wpdb->prefix . 'apollo_activity_comments', array( 'is_hidden' => 1 ), array( 'user_id' => $userId ), array( '%d' ), array( '%d' ) );
	}

	private static function restoreSpammerContent( int $userId ): void {
		global $wpdb;
		$wpdb->update( $wpdb->prefix . 'apollo_activities', array( 'is_hidden' => 0 ), array( 'user_id' => $userId ), array( '%d' ), array( '%d' ) );
	}
}
