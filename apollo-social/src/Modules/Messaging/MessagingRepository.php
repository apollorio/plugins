...
final class MessagingRepository {
	...
	public static function createThread(array $participantIds,?string $subject=null): int {
		...
		foreach($participantIds as $uid){
			$wpdb->insert($wpdb->prefix.self::PARTS,['thread_id'=>$threadId,'user_id'=>(int)$uid]);
		}
		return $threadId;
	}
	...
	public static function sendMessage(int $threadId,int $senderId,string $content): int {
		global $wpdb;
		if(!self::isParticipant($threadId,$senderId))return 0;
		$wpdb->insert($wpdb->prefix.self::MESSAGES,[
			'thread_id'=>$threadId,'sender_id'=>$senderId,'content'=>wp_kses_post($content)
		]);
		$msgId=(int)$wpdb->insert_id;
		$wpdb->update($wpdb->prefix.self::THREADS,['last_message_at'=>current_time('mysql'),'last_message_id'=>$msgId],['id'=>$threadId]);
		$wpdb->query($wpdb->prepare(
			"UPDATE {$wpdb->prefix}".self::PARTS." SET is_read=0 WHERE thread_id=%d AND user_id!=%d",$threadId,$senderId
		));
		return $msgId;
	}
	...
	public static function isParticipant(int $threadId,int $userId): bool {
		...
		return(bool)$wpdb->get_var($wpdb->prepare(
			"SELECT 1 FROM {$wpdb->prefix}".self::PARTS." WHERE thread_id=%d AND user_id=%d",$threadId,$userId
		));
	}

	public static function markRead(int $threadId,int $userId): bool {
		global $wpdb;
		return $wpdb->update($wpdb->prefix.self::PARTS,['is_read'=>1,'last_read_at'=>current_time('mysql')],['thread_id'=>$threadId,'user_id'=>$userId])!==false;
	}

	public static function countUnread(int $userId): int {
		global $wpdb;
		return(int)$wpdb->get_var($wpdb->prepare(
			"SELECT COUNT(*) FROM {$wpdb->prefix}".self::PARTS." WHERE user_id=%d AND is_read=0",$userId
		));
	}

	public static function deleteThread(int $threadId): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::MESSAGES,['thread_id'=>$threadId]);
		$wpdb->delete($wpdb->prefix.self::PARTS,['thread_id'=>$threadId]);
		return $wpdb->delete($wpdb->prefix.self::THREADS,['id'=>$threadId])!==false;
	}
}
