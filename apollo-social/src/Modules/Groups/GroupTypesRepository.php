...
final class GroupTypesRepository {
	...
	public static function getAll(): array {
		global $wpdb;
		$rows = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}" . self::TABLE . ' ORDER BY name', ARRAY_A ) ?: array();
		foreach ( $rows as &$r ) {
			$r['can_create_roles'] = json_decode( $r['can_create_roles'] ?? '', true );
			$r['settings']         = json_decode( $r['settings'] ?? '', true );
		}
	...
	public static function canUserCreate( int $userId, int $typeId ): bool {
		...
		if ( empty( $type['can_create_roles'] ) ) {
			return true;
		}
	...
	public static function update( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'name', 'slug', 'description', 'icon', 'color' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
		if ( isset( $d['can_create_roles'] ) ) {
			$u['can_create_roles'] = wp_json_encode( $d['can_create_roles'] );
		}
		if ( isset( $d['settings'] ) ) {
