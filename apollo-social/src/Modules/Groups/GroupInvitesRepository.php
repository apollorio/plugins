...
final class GroupInvitesRepository {
	...
	public static function create( int $groupId, int $inviterId, int $inviteeId, ?string $message = null ): int {
		...
		$existing = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$wpdb->prefix}" . self::TABLE . " WHERE group_id=%d AND invitee_id=%d AND status='pending'",
				$groupId,
				$inviteeId
			)
		);
	...
	public static function accept( int $inviteId ): bool {
		...
		$wpdb->update( $wpdb->prefix . self::TABLE, array( 'status' => 'accepted' ), array( 'id' => $inviteId ) );
	...
	public static function countPendingForUser( int $userId ): int {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}" . self::TABLE . " WHERE invitee_id=%d AND status='pending'",
				$userId
			)
		);
	}
	...
	public static function delete( int $inviteId ): bool {
		global $wpdb;
		return $wpdb->delete( $wpdb->prefix . self::TABLE, array( 'id' => $inviteId ) ) !== false;
	}
}
