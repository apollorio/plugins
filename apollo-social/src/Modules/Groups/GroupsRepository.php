...
final class GroupsRepository {
	...
	public static function create(array $d): int {
		...
		$wpdb->insert($wpdb->prefix.self::TABLE,[
			...
			'status'=>in_array($d['status']??'',['public','private','hidden'])?$d['status']:'public',
	...
	public static function getBySlug(string $slug): ?array {
		...
		if($r){$r['member_count']=self::countMembers((int)$r['id']);}
		return $r?:null;
	}

	public static function search(array $args=[]): array {
		global $wpdb;
		$defaults=['search'=>'','status'=>'public','type_id'=>null,'orderby'=>'name','order'=>'ASC','limit'=>20,'offset'=>0];
		...
		foreach($rows as &$r){$r['member_count']=self::countMembers((int)$r['id']);}
		return $rows;
	}
	...
	public static function addMember(int $groupId,int $userId,string $role='member'): bool {
		...
		if(!in_array($role,$validRoles))$role='member';
		return $wpdb->replace($wpdb->prefix.self::MEMBERS,[
			'group_id'=>$groupId,'user_id'=>$userId,'role'=>$role,'status'=>'active'
		])!==false;
	}

	public static function removeMember(int $groupId,int $userId): bool {
		global $wpdb;
		return $wpdb->delete($wpdb->prefix.self::MEMBERS,['group_id'=>$groupId,'user_id'=>$userId])!==false;
	}
	...
	public static function updateMemberRole(int $groupId,int $userId,string $role): bool {
		...
		return $wpdb->update($wpdb->prefix.self::MEMBERS,['role'=>$role],['group_id'=>$groupId,'user_id'=>$userId])!==false;
	}

	public static function isMember(int $groupId,int $userId): bool {
		global $wpdb;
		return (bool)$wpdb->get_var($wpdb->prepare(
			"SELECT 1 FROM {$wpdb->prefix}".self::MEMBERS." WHERE group_id=%d AND user_id=%d AND status='active'",$groupId,$userId
		));
	}
	...
	public static function getMemberRole(int $groupId,int $userId): ?string {
		...
		return $wpdb->get_var($wpdb->prepare(
			"SELECT role FROM {$wpdb->prefix}".self::MEMBERS." WHERE group_id=%d AND user_id=%d AND status='active'",$groupId,$userId
		));
	}
	...
	public static function countMembers(int $groupId): int {
		...
		return (int)$wpdb->get_var($wpdb->prepare(
			"SELECT COUNT(*) FROM {$wpdb->prefix}".self::MEMBERS." WHERE group_id=%d AND status='active'",$groupId
		));
	}
	...
	public static function setMeta(int $groupId,string $key,mixed $value): bool {
		global $wpdb;
		return $wpdb->replace($wpdb->prefix.self::META,[
			'group_id'=>$groupId,'meta_key'=>sanitize_key($key),'meta_value'=>maybe_serialize($value)
		])!==false;
	}
	...
	public static function getMeta(int $groupId,string $key,mixed $default=null): mixed {
		...
		$val=$wpdb->get_var($wpdb->prepare(
			"SELECT meta_value FROM {$wpdb->prefix}".self::META." WHERE group_id=%d AND meta_key=%s",$groupId,$key
		));
	...
	public static function deleteMeta(int $groupId,string $key): bool {
		global $wpdb;
		return $wpdb->delete($wpdb->prefix.self::META,['group_id'=>$groupId,'meta_key'=>$key])!==false;
	}

	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['name','slug','description','type_id','status','parent_id','enable_forum'] as $k){
	...
	public static function delete(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::MEMBERS,['group_id'=>$id]);
		$wpdb->delete($wpdb->prefix.self::META,['group_id'=>$id]);
