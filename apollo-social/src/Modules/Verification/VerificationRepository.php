...
final class VerificationRepository {
	...
	public static function request(int $userId, string $type, array $documents=[]): int {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		if(self::hasPendingRequest($userId,$type)){return 0;}
		$wpdb->insert($t,[
			'user_id'=>$userId,
			'type'=>$type,
		...
		if($id){do_action('apollo_verification_requested',$userId,$type,$id);}
		return $id;
	}
	...
	public static function reject(int $requestId, int $rejectedBy, string $reason=''): bool {
		...
		$result=$wpdb->update($t,[
			...
			'reviewed_at'=>current_time('mysql'),
			'notes'=>$reason
		],['id'=>$requestId,'status'=>'pending'],['%s','%d','%s','%s'],['%d','%s']);
		if($result!==false){
			$req=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$t} WHERE id=%d",$requestId),ARRAY_A);
			if($req){do_action('apollo_verification_rejected',(int)$req['user_id'],$req['type'],$reason);}
		}
		return $result!==false;
	}
	...
	public static function getVerificationTypes(int $userId): array {
		$types=['identity','email','phone','business','creator'];
		$verified=[];
		foreach($types as $type){
			if(self::isVerified($userId,$type)){$verified[]=$type;}
		}
		return $verified;
	}
	...
	public static function getVerifiedUsers(string $type='identity', int $limit=50, int $offset=0): array {
		...
		return $wpdb->get_results($wpdb->prepare(
			...
			'apollo_verified_'.$type,$limit,$offset
		),ARRAY_A)??[];
	}
	...
	public static function getVerificationBadge(string $type): ?int {
		$badges=['identity'=>1,'email'=>2,'phone'=>3,'business'=>4,'creator'=>5];
	...
	private static function assignVerificationBadge(int $userId, string $type): void {
		...
		if(!$exists){
			$wpdb->insert($t,['user_id'=>$userId,'badge_id'=>$badgeId,'awarded_at'=>current_time('mysql')],['%d','%d','%s']);
		}
	}
	...
	private static function removeVerificationBadge(int $userId, string $type): void {
		...
		global $wpdb;
		$t=$wpdb->prefix.'apollo_user_badges';
		$wpdb->delete($t,['user_id'=>$userId,'badge_id'=>$badgeId],['%d','%d']);
	}
}
