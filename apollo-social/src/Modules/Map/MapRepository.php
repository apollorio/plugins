...
final class MapRepository {
	...
	public static function deleteLocation(int $userId): bool {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		return $wpdb->delete($t,['user_id'=>$userId],['%d'])!==false;
	}
	...
	public static function getNearbyUsers(float $lat, float $lng, float $radiusKm=50, int $limit=50, int $excludeUserId=0): array {
		...
		return $wpdb->get_results($wpdb->prepare(
			...
			$lng-($radiusKm/(111*cos(deg2rad($lat)))),$lng+($radiusKm/(111*cos(deg2rad($lat)))),
			$radiusKm,$limit
		),ARRAY_A)??[];
	}
	...
	public static function getNearbyFriends(int $userId, float $radiusKm=50): array {
		...
		return $wpdb->get_results($wpdb->prepare(
			...
			HAVING distance<=%f
			ORDER BY distance ASC",
			(float)$loc['latitude'],(float)$loc['longitude'],(float)$loc['latitude'],
			$userId,$radiusKm
		),ARRAY_A)??[];
	}
	...
	public static function geocode(string $address): ?array {
		...
		$url='https://maps.googleapis.com/maps/api/geocode/json?'.http_build_query(['address'=>$address,'key'=>$apiKey]);
		...
		foreach($result['address_components'] as $c){
			if(in_array('locality',$c['types'])){$components['city']=$c['long_name'];}
			if(in_array('administrative_area_level_1',$c['types'])){$components['state']=$c['short_name'];}
			if(in_array('country',$c['types'])){$components['country']=$c['long_name'];}
		}
		return ['lat'=>$loc['lat'],'lng'=>$loc['lng'],'address'=>$result['formatted_address']]+$components;
	}

	public static function reverseGeocode(float $lat, float $lng): ?array {
		$apiKey=get_option('apollo_google_maps_api_key');
		if(!$apiKey){return null;}
		$url='https://maps.googleapis.com/maps/api/geocode/json?'.http_build_query(['latlng'=>"{$lat},{$lng}",'key'=>$apiKey]);
		$response=wp_remote_get($url,['timeout'=>10]);
		if(is_wp_error($response)){return null;}
		$data=json_decode(wp_remote_retrieve_body($response),true);
		if(empty($data['results'][0])){return null;}
