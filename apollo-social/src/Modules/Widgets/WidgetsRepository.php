...
final class WidgetsRepository {
	...
	public static function getNewestMembers(int $limit=10): array {
		...
		$users=$wpdb->get_results($wpdb->prepare(
			"SELECT ID,display_name,user_email,user_registered FROM {$wpdb->users} ORDER BY user_registered DESC LIMIT %d",$limit
		),ARRAY_A)?:[];
		foreach($users as &$u){
			$u['avatar_url']=get_avatar_url($u['ID'],['size'=>50]);
	...
	public static function getDynamicMenuItems(int $userId): array {
		...
		if($pendingFriends>0){
			$items[]=['label'=>'Friend Requests','url'=>'/members/requests/','count'=>$pendingFriends,'icon'=>'users'];
		}
		$unreadMentions=self::getUnreadMentionsCount($userId);
		if($unreadMentions>0){
			$items[]=['label'=>'Mentions','url'=>'/activity/mentions/','count'=>$unreadMentions,'icon'=>'at'];
		}
		return $items;
	}
}
