...
final class BadgesRepository {
	...
	public static function create( array $d ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'rarity'       => in_array( $d['rarity'] ?? '', array( 'common', 'uncommon', 'rare', 'epic', 'legendary' ) ) ? $d['rarity'] : 'common',
	...
	public static function getAll( bool $activeOnly = true ): array {
		...
		return $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}" . self::TABLE . " {$w} ORDER BY sort_order,name", ARRAY_A ) ?: array();
	}
	...
	public static function getByCategory( string $category, bool $activeOnly = true ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}" . self::TABLE . " WHERE category=%s {$w} ORDER BY sort_order,name",
				$category
			),
			ARRAY_A
		) ?: array();
	...
	public static function awardToUser( int $userId, int $badgeId, ?string $reason = null ): bool {
		...
		$exists = (bool) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT 1 FROM {$wpdb->prefix}" . self::USER_TABLE . ' WHERE user_id=%d AND badge_id=%d',
				$userId,
				$badgeId
			)
		);
		...
		$result = $wpdb->insert(
			...
			array(
				'user_id'        => $userId,
				'badge_id'       => $badgeId,
				'awarded_reason' => $reason ? sanitize_text_field( $reason ) : null,
			)
		);
		if ( $result ) {
	...
	public static function countUserBadges( int $userId ): int {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}" . self::USER_TABLE . ' WHERE user_id=%d',
				$userId
			)
		);
	}
	...
	public static function getCategories(): array {
		global $wpdb;
		return $wpdb->get_col( "SELECT DISTINCT category FROM {$wpdb->prefix}" . self::TABLE . " WHERE category!='' AND is_active=1 ORDER BY category" ) ?: array();
	}

	public static function update( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'name', 'slug', 'description', 'icon', 'image_url', 'color', 'category', 'rarity', 'points_value', 'is_active', 'sort_order' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
	...
	public static function delete( int $id ): bool {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . self::USER_TABLE, array( 'badge_id' => $id ) );
