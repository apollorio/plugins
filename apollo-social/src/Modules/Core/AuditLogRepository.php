...
final class AuditLogRepository {
	...
	public static function log( string $action, int $userId = 0, string $objectType = '', int $objectId = 0, array $data = array(), string $ip = '' ): int|false {
		global $wpdb;
	...
	public static function logLogout( int $userId ): int|false {
		return self::log( 'logout', $userId, 'user', $userId, array() );
	}

	public static function logPasswordChange( int $userId, bool $byAdmin = false ): int|false {
		return self::log( 'password_change', $userId, 'user', $userId, array( 'by_admin' => $byAdmin ) );
	}
	...
	public static function logProfileUpdate( int $userId, array $changedFields ): int|false {
		return self::log( 'profile_update', $userId, 'user', $userId, array( 'fields' => $changedFields ) );
	}

	public static function logContentCreated( int $userId, string $type, int $contentId ): int|false {
		return self::log( 'content_created', $userId, $type, $contentId, array() );
	}

	public static function logContentDeleted( int $userId, string $type, int $contentId, array $snapshot = array() ): int|false {
		return self::log( 'content_deleted', $userId, $type, $contentId, array( 'snapshot' => $snapshot ) );
	}
	...
	public static function logModerationAction( int $moderatorId, string $action, string $targetType, int $targetId, string $reason = '' ): int|false {
		return self::log( 'moderation_' . $action, $moderatorId, $targetType, $targetId, array( 'reason' => $reason ) );
	}

	public static function logAdminAction( int $adminId, string $action, array $details = array() ): int|false {
		return self::log( 'admin_' . $action, $adminId, 'admin', 0, $details );
	}

	public static function logSecurityEvent( int $userId, string $event, array $details = array() ): int|false {
