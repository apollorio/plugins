...
final class TeamMembersRepository {
	...
	public static function get(int $id): ?array {
		...
		if($r)$r['socials']=json_decode($r['socials']??'',true);
		return $r?:null;
	}
	...
	public static function getAll(bool $activeOnly=true): array {
		...
		$rows=$wpdb->get_results("SELECT * FROM {$wpdb->prefix}".self::TABLE." {$w} ORDER BY sort_order,name",ARRAY_A)?:[];
		foreach($rows as &$r){$r['socials']=json_decode($r['socials']??'',true);}
		return $rows;
	}
	...
	public static function getByDepartment(string $department,bool $activeOnly=true): array {
		...
		$rows=$wpdb->get_results($wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}".self::TABLE." WHERE department=%s {$w} ORDER BY sort_order,name",$department
		),ARRAY_A)?:[];
		foreach($rows as &$r){$r['socials']=json_decode($r['socials']??'',true);}
		return $rows;
	}
	...
	public static function getDepartments(): array {
		global $wpdb;
		return $wpdb->get_col("SELECT DISTINCT department FROM {$wpdb->prefix}".self::TABLE." WHERE department!='' AND is_active=1 ORDER BY department")?:[];
	}

	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['user_id','name','slug','position','department','bio','photo_url','email','phone','sort_order','is_active'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		if(isset($d['socials']))$u['socials']=wp_json_encode($d['socials']);
