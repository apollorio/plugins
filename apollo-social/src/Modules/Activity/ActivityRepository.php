...
final class ActivityRepository {
	...
	public static function create( array $d ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'privacy'           => in_array( $d['privacy'] ?? '', array( 'public', 'friends', 'private' ) ) ? $d['privacy'] : 'public',
				'is_spam'           => 0,
			)
		);
	...
	public static function getFeed( array $args = array() ): array {
		global $wpdb;
		$defaults = array(
			'user_id'   => null,
			'component' => null,
		...
		$where    = array( 'is_spam=0' );
		$params   = array();
		if ( $a['user_id'] ) {
			$where[]  = 'user_id=%d';
			$params[] = (int) $a['user_id'];}
		if ( $a['component'] ) {
			$where[]  = 'component=%s';
			$params[] = $a['component'];}
		if ( $a['type'] ) {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
			 ...
			 WHERE {$w} ORDER BY a.created_at DESC LIMIT %d OFFSET %d",
				...$params
			),
			ARRAY_A
		) ?: array();
	...
	public static function delete( int $id ): bool {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . self::MENTIONS, array( 'activity_id' => $id ) );
		$wpdb->delete(
			$wpdb->prefix . self::FAVS,
			array(
				'item_type' => 'activity',
				'item_id'   => $id,
			)
		);
		$wpdb->delete( $wpdb->prefix . self::TABLE, array( 'parent_id' => $id ) );
		return $wpdb->delete( $wpdb->prefix . self::TABLE, array( 'id' => $id ) ) !== false;
	}

	public static function markAsSpam( int $id ): bool {
		global $wpdb;
		return $wpdb->update( $wpdb->prefix . self::TABLE, array( 'is_spam' => 1 ), array( 'id' => $id ) ) !== false;
	}
	...
	public static function markMentionRead( int $mentionId ): bool {
		global $wpdb;
		return $wpdb->update( $wpdb->prefix . self::MENTIONS, array( 'is_read' => 1 ), array( 'id' => $mentionId ) ) !== false;
	}

	public static function markAllMentionsRead( int $userId ): bool {
		global $wpdb;
		return $wpdb->update( $wpdb->prefix . self::MENTIONS, array( 'is_read' => 1 ), array( 'user_id' => $userId ) ) !== false;
	}
	...
	public static function countUnreadMentions( int $userId ): int {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}" . self::MENTIONS . ' WHERE user_id=%d AND is_read=0',
				$userId
			)
		);
	}
