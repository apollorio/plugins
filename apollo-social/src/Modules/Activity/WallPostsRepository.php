...
final class WallPostsRepository {
	...
	public static function create(int $authorId, int $profileId, string $content, array $meta=[]): ?int {
		if($authorId!==$profileId&&!self::canPost($authorId,$profileId)){return null;}
		global $wpdb;
		...
		if($r){
			...
			if($authorId!==$profileId){
				do_action('apollo_notify',$profileId,'wall_post',sprintf('%s postou em seu mural',get_userdata($authorId)->display_name),[
					'actor_id'=>$authorId,'object_type'=>'wall_post','object_id'=>$id,'link'=>home_url("/members/{$profileId}/activity/{$id}/")
				]);
			}
	...
	public static function delete(int $postId, int $userId): bool {
		...
		if((int)$post['user_id']!==$userId&&(int)$post['profile_id']!==$userId&&!current_user_can('manage_options')){return false;}
		$wpdb->delete($wpdb->prefix.'apollo_activity_comments',['activity_id'=>$postId],['%d']);
		$wpdb->delete($wpdb->prefix.'apollo_activity_likes',['activity_id'=>$postId],['%d']);
		return (bool)$wpdb->delete($t,['id'=>$postId],['%d']);
	}

	public static function update(int $postId, int $userId, string $content): bool {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		return (bool)$wpdb->update($t,['content'=>wp_kses_post($content),'updated_at'=>current_time('mysql')],['id'=>$postId,'user_id'=>$userId,'type'=>'wall_post']);
	}
	...
	private static function getVisibilityFilter(int $viewerId, int $profileId): string {
		if($viewerId===$profileId||current_user_can('manage_options')){return '';}
		if(self::areCloseFriends($viewerId,$profileId)){return "AND a.visibility IN ('public','friends','close_friends')";}
		if(self::areFriends($viewerId,$profileId)){return "AND a.visibility IN ('public','friends')";}
