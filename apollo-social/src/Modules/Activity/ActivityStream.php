...
final class ActivityStream {
    ...
    public function init(): void {
        ...
        add_action('apollo_achievement_unlocked', fn($user_id, $key) => $this->log(self::TYPE_ACHIEVEMENT, $user_id, 0, ['achievement' => $key]), 10, 2);
        add_action('apollo_rank_changed', fn($user_id, $old, $new) => $this->log(self::TYPE_RANK, $user_id, 0, ['old' => $old, 'new' => $new]), 10, 3);
        add_action('apollo_document_signed', fn($doc_id, $user_id) => $this->log(self::TYPE_DOCUMENT, $user_id, $doc_id), 10, 2);
        add_action('rest_api_init', [$this, 'registerEndpoints']);
    }
    public function log(string $type, int $user_id, int $object_id = 0, array $meta = [], int $group_id = 0): int {
        global $wpdb;
        $wpdb->insert($wpdb->prefix . 'apollo_activity', [
            'user_id' => $user_id,
            'type' => $type,
            'object_id' => $object_id,
            'group_id' => $group_id,
    ...
    public function favoriteActivity(int $user_id, int $activity_id): bool {
        global $wpdb;
        return (bool) $wpdb->insert($wpdb->prefix . 'apollo_activity_favorites', [
            'user_id' => $user_id,
    ...
    public function unfavoriteActivity(int $user_id, int $activity_id): bool {
        global $wpdb;
        return (bool) $wpdb->delete($wpdb->prefix . 'apollo_activity_favorites', ['user_id' => $user_id, 'activity_id' => $activity_id]);
    }
    private function formatActivity(array $row): array {
        ...
        return [
            ...
            'user' => [
                ...
                'avatar' => get_avatar_url((int) $row['user_id'], ['size' => 50]),
            ],
