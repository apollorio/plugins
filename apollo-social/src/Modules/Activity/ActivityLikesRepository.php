...
final class ActivityLikesRepository {
	...
	private const REACTIONS=['like','love','haha','wow','sad','angry'];

	public static function add(int $activityId, int $userId, string $reaction='like'): bool {
		if(!in_array($reaction,self::REACTIONS,true)){$reaction='like';}
		global $wpdb;
		...
		if($exists){
			if($exists['reaction_type']===$reaction){return true;}
			return (bool)$wpdb->update($t,['reaction_type'=>$reaction],['id'=>$exists['id']]);
		}
		$r=$wpdb->insert($t,['activity_id'=>$activityId,'user_id'=>$userId,'reaction_type'=>$reaction],['%d','%d','%s']);
		if($r){
	...
	public static function remove(int $activityId, int $userId): bool {
		global $wpdb;
		$r=$wpdb->delete($wpdb->prefix.self::TABLE,['activity_id'=>$activityId,'user_id'=>$userId],['%d','%d']);
		if($r){self::updateActivityCount($activityId);}
		return (bool)$r;
	}
	...
	public static function toggle(int $activityId, int $userId, string $reaction='like'): array {
		if(self::hasLiked($activityId,$userId)){
			...
			return ['action'=>'removed','count'=>self::getCount($activityId)];
		}
		self::add($activityId,$userId,$reaction);
		return ['action'=>'added','count'=>self::getCount($activityId)];
	}
	...
	public static function getCountByReaction(int $activityId): array {
		...
		foreach($rows as $r){$counts[$r['reaction_type']]=(int)$r['count'];}
		return $counts;
	}
	...
	public static function getUsers(int $activityId, int $limit=50, ?string $reaction=null): array {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		$where='l.activity_id=%d';$params=[$activityId];
		if($reaction){$where.=' AND l.reaction_type=%s';$params[]=$reaction;}
		$params[]=$limit;
	...
	public static function getAvailableReactions(): array {
		return array_map(fn($r)=>['type'=>$r,'emoji'=>self::getEmoji($r)],self::REACTIONS);
	}
	...
	private static function updateActivityCount(int $activityId): void {
		...
		$wpdb->update($wpdb->prefix.'apollo_activity',['like_count'=>$count],['id'=>$activityId]);
	}
	...
	public static function getLikeStats(int $userId): array {
		...
		return [
			...
			'by_reaction'=>$wpdb->get_results($wpdb->prepare(
