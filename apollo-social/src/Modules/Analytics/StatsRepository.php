...
final class StatsRepository {
	...
	public static function getTopUsers( string $metric = 'points', int $limit = 10 ): array {
		...
		return match ( $metric ) {
			...
			'connections'=>$wpdb->get_results(
				...
				GROUP BY u.ID ORDER BY total DESC LIMIT %d",
					$limit
				),
				ARRAY_A
			) ?? array(),
	...
	public static function getPopularGroups( int $limit = 10 ): array {
		...
		return $wpdb->get_results(
			...
			GROUP BY g.id ORDER BY member_count DESC LIMIT %d",
				$limit
			),
			ARRAY_A
		) ?? array();
	...
	public static function getRetentionRate( int $days = 7 ): float {
		...
		$active = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(DISTINCT user_id) FROM {$wpdb->prefix}" . self::T_ACTIVITY . ' WHERE created_at>=%s',
				$startDate
			)
		);
