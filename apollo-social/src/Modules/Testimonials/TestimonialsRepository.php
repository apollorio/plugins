...
final class TestimonialsRepository {
	...
	public static function create( array $d ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'status'         => in_array( $d['status'] ?? '', array( 'pending', 'approved', 'rejected' ) ) ? $d['status'] : 'pending',
	...
	public static function getFeatured( int $limit = 10 ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}" . self::TABLE . " WHERE status='approved' AND is_featured=1 ORDER BY sort_order,created_at DESC LIMIT %d",
				$limit
			),
			ARRAY_A
		) ?: array();
	...
	public static function getPending( int $limit = 50, int $offset = 0 ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}" . self::TABLE . " WHERE status='pending' ORDER BY created_at LIMIT %d OFFSET %d",
				$limit,
				$offset
			),
			ARRAY_A
	...
	public static function approve( int $id ): bool {
		global $wpdb;
		return $wpdb->update( $wpdb->prefix . self::TABLE, array( 'status' => 'approved' ), array( 'id' => $id ) ) !== false;
	}

	public static function reject( int $id ): bool {
		global $wpdb;
		return $wpdb->update( $wpdb->prefix . self::TABLE, array( 'status' => 'rejected' ), array( 'id' => $id ) ) !== false;
	}
	...
	public static function getCategories(): array {
		global $wpdb;
		return $wpdb->get_col( "SELECT DISTINCT category FROM {$wpdb->prefix}" . self::TABLE . " WHERE category!='' AND status='approved' ORDER BY category" ) ?: array();
	}

	public static function update( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'author_name', 'author_title', 'author_company', 'author_image', 'user_id', 'content', 'rating', 'category', 'product_id', 'video_url', 'is_featured', 'status', 'sort_order' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
