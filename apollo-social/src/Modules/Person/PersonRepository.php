...
final class PersonRepository {
	...
	public static function create( array $data ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'social'     => wp_json_encode( $data['social'] ?? array() ),
				'skills'     => wp_json_encode( $data['skills'] ?? array() ),
				'sort_order' => (int) ( $data['sort_order'] ?? 0 ),
				'is_active'  => 1,
				'created_at' => current_time( 'mysql' ),
			),
			array( '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%s' )
		);
	...
	public static function update( int $personId, array $data ): bool {
		...
		if ( isset( $data['department'] ) ) {
			$upd['department'] = sanitize_text_field( $data['department'] );
			$fmt[]             = '%s';}
		if ( isset( $data['bio'] ) ) {
			$upd['bio'] = wp_kses_post( $data['bio'] );
			$fmt[]      = '%s';}
		if ( isset( $data['photo'] ) ) {
			$upd['photo'] = esc_url( $data['photo'] );
			$fmt[]        = '%s';}
		if ( isset( $data['email'] ) ) {
			$upd['email'] = sanitize_email( $data['email'] );
			$fmt[]        = '%s';}
		if ( isset( $data['phone'] ) ) {
			$upd['phone'] = sanitize_text_field( $data['phone'] );
			$fmt[]        = '%s';}
		if ( isset( $data['social'] ) ) {
			$upd['social'] = wp_json_encode( $data['social'] );
			$fmt[]         = '%s';}
		if ( isset( $data['skills'] ) ) {
			$upd['skills'] = wp_json_encode( $data['skills'] );
			$fmt[]         = '%s';}
		...
		$fmt[]             = '%s';
		return $wpdb->update( $t, $upd, array( 'id' => $personId ), $fmt, array( '%d' ) ) !== false;
	}

	public static function delete( int $personId ): bool {
		global $wpdb;
		$t = $wpdb->prefix . self::TABLE;
		return $wpdb->delete( $t, array( 'id' => $personId ), array( '%d' ) ) !== false;
	}
	...
	public static function getAll( bool $activeOnly = true ): array {
		...
		$rows  = $wpdb->get_results( "SELECT * FROM {$t} {$where} ORDER BY sort_order ASC, name ASC", ARRAY_A ) ?? array();
	...
	public static function getDepartments(): array {
		global $wpdb;
		$t = $wpdb->prefix . self::TABLE;
		return $wpdb->get_col( "SELECT DISTINCT department FROM {$t} WHERE department!='' AND is_active=1 ORDER BY department ASC" ) ?? array();
	}
	...
	public static function reorder( array $order ): bool {
		...
		foreach ( $order as $position => $personId ) {
			$wpdb->update( $t, array( 'sort_order' => $position ), array( 'id' => (int) $personId ), array( '%d' ), array( '%d' ) );
		}
		return true;
	}
	...
	public static function linkToUser( int $personId, int $userId ): bool {
		global $wpdb;
		$t = $wpdb->prefix . self::TABLE;
		return $wpdb->update( $t, array( 'user_id' => $userId ), array( 'id' => $personId ), array( '%d' ), array( '%d' ) ) !== false;
	}

	public static function unlinkFromUser( int $personId ): bool {
		global $wpdb;
		$t = $wpdb->prefix . self::TABLE;
		return $wpdb->update( $t, array( 'user_id' => 0 ), array( 'id' => $personId ), array( '%d' ), array( '%d' ) ) !== false;
	}
	...
	public static function import( array $persons ): array {
		...
		foreach ( $persons as $p ) {
			...
			if ( $id ) {
				...
				$errors[] = "Erro ao importar: {$p['name']}";}
		}
	...
	private static function decode( array $row ): array {
		$row['social'] = json_decode( $row['social'] ?? '[]', true ) ?? array();
		$row['skills'] = json_decode( $row['skills'] ?? '[]', true ) ?? array();
		return $row;
	}
}
