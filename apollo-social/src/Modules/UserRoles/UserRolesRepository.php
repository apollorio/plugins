...
final class UserRolesRepository {
	...
	public static function getAll(): array {
		global $wpdb;
		$rows=$wpdb->get_results("SELECT * FROM {$wpdb->prefix}".self::TABLE." ORDER BY priority DESC,name",ARRAY_A)?:[];
		foreach($rows as &$r){
			$r['capabilities']=json_decode($r['capabilities']??'',true);
			$r['restrictions']=json_decode($r['restrictions']??'',true);
		}
	...
	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['name','slug','description','color','icon','priority'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		if(isset($d['capabilities']))$u['capabilities']=wp_json_encode($d['capabilities']);
		if(isset($d['restrictions']))$u['restrictions']=wp_json_encode($d['restrictions']);
