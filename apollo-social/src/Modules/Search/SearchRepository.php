...
final class SearchRepository {
	...
	public static function globalSearch( string $query, int $userId = 0, int $limit = 50 ): array {
		...
		$results['members'] = \Apollo\Modules\Members\MembersDirectoryRepository::search(
			array(
				'search' => $sanitized,
				'limit'  => min( 20, $limit ),
			)
		);
		$results['groups']  = \Apollo\Modules\Groups\GroupsRepository::search(
			array(
				'search' => $sanitized,
				'limit'  => min( 10, $limit ),
			)
		...
		$results['activity'] = $wpdb->get_results(
			$wpdb->prepare(
		...
		$results['forum']    = $wpdb->get_results(
			$wpdb->prepare(
	...
	public static function searchMembers( string $query, array $filters = array(), int $limit = 20, int $offset = 0 ): array {
		$args = array_merge(
			array(
				'search' => $query,
				'limit'  => $limit,
	...
	public static function autocomplete( string $query, string $type = 'all', int $limit = 10 ): array {
		...
		if ( $type === 'all' || $type === 'tags' ) {
			$tags = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT id,name,slug FROM {$wpdb->prefix}apollo_user_tags WHERE name LIKE %s LIMIT %d",
					$like,
					$limit
				),
				ARRAY_A
