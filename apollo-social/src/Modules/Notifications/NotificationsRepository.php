...
final class NotificationsRepository {
	...
	public static function create( int $userId, string $type, string $title, array $data = array() ): ?int {
		global $wpdb;
		$r = $wpdb->insert(
	...
	public static function notify( int $userId, string $type, string $title, array $data = array() ): ?int {
	...
	public static function markRead( int $id, int $userId ): bool {
		...
		if ( $r ) {
			self::decrementUnreadCount( $userId );}
		return (bool) $r;
	}
	...
	private static function shouldNotify( int $userId, string $type ): bool {
		$prefs = get_user_meta( $userId, 'apollo_notification_prefs', true ) ?: array();
	...
	private static function checkDuplicate( int $userId, string $type, array $data ): bool {
		if ( ! isset( $data['actor_id'] ) || ! isset( $data['object_id'] ) ) {
			return false;}
		global $wpdb;
		$t      = $wpdb->prefix . self::TABLE;
	...
	public static function getTypes(): array {
		return apply_filters(
			'apollo_notification_types',
			array(
				'friend_request'   => array(
					'label' => 'Solicitação de amizade',
					'icon'  => 'user-plus',
				),
				'friend_accepted'  => array(
					'label' => 'Amizade aceita',
					'icon'  => 'user-check',
				),
				'group_invite'     => array(
					'label' => 'Convite para grupo',
					'icon'  => 'users',
				),
				'group_join'       => array(
					'label' => 'Novo membro no grupo',
					'icon'  => 'user-plus',
				),
				'new_message'      => array(
					'label' => 'Nova mensagem',
	...
	public static function notifyMultiple( array $userIds, string $type, string $title, array $data = array() ): int {
		$count = 0;
		foreach ( $userIds as $uid ) {
			if ( self::notify( (int) $uid, $type, $title, $data ) ) {
				++$count;}
		}
		return $count;
	}
	...
	public static function sendPush( int $userId, string $title, string $body, array $data = array() ): bool {
	...
	public static function sendEmail( int $userId, string $type, array $data = array() ): bool {
		$user = get_userdata( $userId );
		if ( ! $user ) {
			return false;}
		$emailPrefs = get_user_meta( $userId, 'apollo_email_notification_prefs', true ) ?: array();
		if ( isset( $emailPrefs[ $type ] ) && $emailPrefs[ $type ] === false ) {
			return false;}
	...
	public static function getGrouped( int $userId, int $limit = 20 ): array {
		...
		foreach ( $all as $n ) {
			...
			if ( ! isset( $grouped[ $key ] ) ) {
				$grouped[ $key ]           = $n;
				$grouped[ $key ]['actors'] = array();
				$grouped[ $key ]['count']  = 0;}
			if ( $n['actor_id'] ) {
				$grouped[ $key ]['actors'][] = $n['actor_id'];
				++$grouped[ $key ]['count'];}
		}
