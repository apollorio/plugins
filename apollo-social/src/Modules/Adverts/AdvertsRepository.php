...
final class AdvertsRepository {
	...
	public static function create(array $d): ?int {
		...
		],['%s','%s','%s','%f','%s','%s','%d','%d','%s','%s','%s','%s','%s','%f','%f','%s','%s','%s','%d','%s','%d','%s','%s']);
		return $r?(int)$wpdb->insert_id:null;
	}

	public static function update(int $id, array $d): bool {
		global $wpdb;
		if(isset($d['custom_fields'])){$d['custom_fields']=json_encode($d['custom_fields']);}
		return (bool)$wpdb->update($wpdb->prefix.self::TABLE,$d,['id'=>$id]);
	}

	public static function delete(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::IMAGES,['advert_id'=>$id],['%d']);
		$wpdb->delete($wpdb->prefix.self::FAVORITES,['advert_id'=>$id],['%d']);
		$wpdb->delete($wpdb->prefix.self::VIEWS,['advert_id'=>$id],['%d']);
	...
	public static function get(int $id): ?array {
		...
		if($row&&$row['custom_fields']){$row['custom_fields']=json_decode($row['custom_fields'],true);}
		return $row;
	}
	...
	public static function search(array $filters=[], int $limit=20, int $offset=0): array {
		...
		if(!empty($filters['subcategory_id'])){$where[]='subcategory_id=%d';$params[]=$filters['subcategory_id'];}
		...
		if(!empty($filters['condition'])){$where[]='`condition`=%s';$params[]=$filters['condition'];}
		if(isset($filters['price_min'])){$where[]='price>=%f';$params[]=$filters['price_min'];}
		if(isset($filters['price_max'])){$where[]='price<=%f';$params[]=$filters['price_max'];}
	...
	public static function getByUser(int $userId, ?string $status=null, int $limit=20): array {
		...
		$t=$wpdb->prefix.self::TABLE;
		if($status){
			return $wpdb->get_results($wpdb->prepare("SELECT * FROM {$t} WHERE user_id=%d AND status=%s ORDER BY created_at DESC LIMIT %d",$userId,$status,$limit),ARRAY_A)??[];
		}
		return $wpdb->get_results($wpdb->prepare("SELECT * FROM {$t} WHERE user_id=%d ORDER BY created_at DESC LIMIT %d",$userId,$limit),ARRAY_A)??[];
	}

	public static function addImage(int $advertId, string $url, int $order=0, bool $isPrimary=false): ?int {
		global $wpdb;
		if($isPrimary){$wpdb->update($wpdb->prefix.self::IMAGES,['is_primary'=>0],['advert_id'=>$advertId]);}
		$r=$wpdb->insert($wpdb->prefix.self::IMAGES,[
			'advert_id'=>$advertId,'image_url'=>$url,'sort_order'=>$order,'is_primary'=>$isPrimary?1:0
		],['%d','%s','%d','%d']);
		return $r?(int)$wpdb->insert_id:null;
	}
	...
	public static function getImages(int $advertId): array {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT * FROM {$t} WHERE advert_id=%d ORDER BY is_primary DESC,sort_order ASC",$advertId),ARRAY_A)??[];
	}

	public static function deleteImage(int $imageId): bool {
		global $wpdb;
		return (bool)$wpdb->delete($wpdb->prefix.self::IMAGES,['id'=>$imageId],['%d']);
	}
	...
	public static function addFavorite(int $advertId, int $userId): bool {
		...
		return (bool)$wpdb->insert($t,['advert_id'=>$advertId,'user_id'=>$userId],['%d','%d']);
	}

	public static function removeFavorite(int $advertId, int $userId): bool {
		global $wpdb;
		return (bool)$wpdb->delete($wpdb->prefix.self::FAVORITES,['advert_id'=>$advertId,'user_id'=>$userId],['%d','%d']);
	}
	...
	public static function getUserFavorites(int $userId, int $limit=50): array {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;$f=$wpdb->prefix.self::FAVORITES;
	...
	public static function recordView(int $advertId, ?int $userId=null, string $ip=''): void {
		...
		$wpdb->insert($t,['advert_id'=>$advertId,'user_id'=>$userId,'ip_address'=>$ip],['%d','%d','%s']);
	...
	public static function getCategories(): array {
		...
		return $wpdb->get_results("SELECT * FROM {$t} ORDER BY sort_order ASC,name ASC",ARRAY_A)??[];
	}

	public static function getCategoryTree(): array {
		global $wpdb;
		$t=$wpdb->prefix.'apollo_advert_categories';
		$all=$wpdb->get_results("SELECT * FROM {$t} ORDER BY parent_id,sort_order,name",ARRAY_A)??[];
		$tree=[];$map=[];
		foreach($all as &$c){$c['children']=[];$map[$c['id']]=&$c;}
		foreach($all as &$c){
	...
	public static function approve(int $id): bool {
		global $wpdb;
		return (bool)$wpdb->update($wpdb->prefix.self::TABLE,['status'=>'published','approved_at'=>current_time('mysql')],['id'=>$id]);
	}

	public static function reject(int $id, string $reason=''): bool {
		global $wpdb;
		return (bool)$wpdb->update($wpdb->prefix.self::TABLE,['status'=>'rejected','rejection_reason'=>$reason],['id'=>$id]);
	}

	public static function setFeatured(int $id, bool $featured, ?string $until=null): bool {
		global $wpdb;
		return (bool)$wpdb->update($wpdb->prefix.self::TABLE,['featured'=>$featured?1:0,'featured_until'=>$until],['id'=>$id]);
	}
	...
	public static function renew(int $id, int $days=30): bool {
		...
		return (bool)$wpdb->update($t,['status'=>'published','expires_at'=>$newExpiry],['id'=>$id]);
	}
	...
	public static function getPending(int $limit=50): array {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		return $wpdb->get_results($wpdb->prepare("SELECT * FROM {$t} WHERE status='pending' ORDER BY created_at ASC LIMIT %d",$limit),ARRAY_A)??[];
	}
}
