	...
	public static function getUserAdvertStats( int $userId ): array {
		...
		return array(
			...
			'total_views' => (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$v} WHERE advert_id IN(SELECT id FROM {$t} WHERE user_id=%d)",
					$userId
				)
			),
			'total_saves' => (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$f} WHERE advert_id IN(SELECT id FROM {$t} WHERE user_id=%d)",
					$userId
				)
			),
		);
	...
	public static function republish( int $advertId, int $userId, int $daysActive = 30 ): bool {
		...
		return (bool) $wpdb->update(
			...
			array(
				...
				'republished_at' => gmdate( 'Y-m-d H:i:s' ),
			),
			array(
				'id'      => $advertId,
				'user_id' => $userId,
