...
final class BubbleSystem {
    ...
    public function addToBubble(int $user_id, int $friend_id): array {
        if ($user_id === $friend_id) return ['success' => false, 'error' => 'cannot_add_self'];
        if (!get_userdata($friend_id)) return ['success' => false, 'error' => 'user_not_found'];
        $bubble = $this->getBubble($user_id);
        if (count($bubble) >= self::MAX_BUBBLE) return ['success' => false, 'error' => 'bubble_full', 'max' => self::MAX_BUBBLE];
        if (in_array($friend_id, $bubble, true)) return ['success' => false, 'error' => 'already_in_bubble'];
        $bubble[] = $friend_id;
    ...
    public function getBubble(int $user_id): array {
        return get_user_meta($user_id, '_apollo_bubble', true) ?: [];
    }
    public function addCloseFriend(int $user_id, int $friend_id): array {
        ...
        $close = $this->getCloseFriends($user_id);
        if (count($close) >= self::MAX_CLOSE_FRIENDS) return ['success' => false, 'error' => 'close_friends_full', 'max' => self::MAX_CLOSE_FRIENDS];
        if (in_array($friend_id, $close, true)) return ['success' => false, 'error' => 'already_close_friend'];
        $close[] = $friend_id;
        update_user_meta($user_id, '_apollo_close_friends', $close);
        do_action('apollo_close_friend_added', $user_id, $friend_id);
        return ['success' => true, 'close_count' => count($close)];
    }
    public function removeCloseFriend(int $user_id, int $friend_id): array {
        $close = $this->getCloseFriends($user_id);
        $key = array_search($friend_id, $close, true);
        if ($key === false) return ['success' => false, 'error' => 'not_close_friend'];
        unset($close[$key]);
    ...
    public function getCloseFriends(int $user_id): array {
        return get_user_meta($user_id, '_apollo_close_friends', true) ?: [];
    }
    public function getFollowers(int $user_id): array {
        return get_user_meta($user_id, '_apollo_followers', true) ?: [];
    }
    public function getFollowing(int $user_id): array {
        return get_user_meta($user_id, '_apollo_following', true) ?: [];
    }
    public function follow(int $user_id, int $target_id): array {
        if ($user_id === $target_id) return ['success' => false, 'error' => 'cannot_follow_self'];
        $following = $this->getFollowing($user_id);
        if (in_array($target_id, $following, true)) return ['success' => false, 'error' => 'already_following'];
        $following[] = $target_id;
        ...
        return ['success' => true, 'following_count' => count($following)];
    }
    public function unfollow(int $user_id, int $target_id): array {
        $following = $this->getFollowing($user_id);
        $key = array_search($target_id, $following, true);
        if ($key === false) return ['success' => false, 'error' => 'not_following'];
        unset($following[$key]);
    ...
    public function getBubbleWithDetails(int $user_id): array {
        ...
        foreach ($bubble as $friend_id) {
            ...
            $result[] = [
                ...
                'avatar' => get_avatar_url($friend_id, ['size' => 100]),
    ...
    public function registerEndpoints(): void {
        register_rest_route('apollo/v1', '/bubble', [
            'methods' => 'GET',
            'callback' => fn() => new \WP_REST_Response(['bubble' => $this->getBubbleWithDetails(get_current_user_id()), 'stats' => $this->getConnectionStats(get_current_user_id())], 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/bubble/add', [
            'methods' => 'POST',
            'callback' => fn($r) => new \WP_REST_Response($this->addToBubble(get_current_user_id(), (int) $r->get_param('user_id')), 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/bubble/remove', [
            'methods' => 'POST',
            'callback' => fn($r) => new \WP_REST_Response($this->removeFromBubble(get_current_user_id(), (int) $r->get_param('user_id')), 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/close-friends', [
            'methods' => 'GET',
            'callback' => fn() => new \WP_REST_Response(['close_friends' => $this->getCloseFriends(get_current_user_id()), 'max' => self::MAX_CLOSE_FRIENDS], 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/close-friends/add', [
            'methods' => 'POST',
            'callback' => fn($r) => new \WP_REST_Response($this->addCloseFriend(get_current_user_id(), (int) $r->get_param('user_id')), 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/close-friends/remove', [
            'methods' => 'POST',
            'callback' => fn($r) => new \WP_REST_Response($this->removeCloseFriend(get_current_user_id(), (int) $r->get_param('user_id')), 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/follow', [
            'methods' => 'POST',
            'callback' => fn($r) => new \WP_REST_Response($this->follow(get_current_user_id(), (int) $r->get_param('user_id')), 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/unfollow', [
            'methods' => 'POST',
            'callback' => fn($r) => new \WP_REST_Response($this->unfollow(get_current_user_id(), (int) $r->get_param('user_id')), 200),
            'permission_callback' => 'is_user_logged_in',
        ]);
        register_rest_route('apollo/v1', '/connections/stats', [
            'methods' => 'GET',
