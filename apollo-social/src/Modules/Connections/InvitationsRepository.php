...
final class InvitationsRepository {
	...
	public static function accept( string $code, int $userId ): bool {
		...
		if ( $r ) {
			if ( $inv['group_id'] ) {
				self::addToGroup( $userId, (int) $inv['group_id'] );}
	...
	public static function getByEmail( string $email ): array {
		global $wpdb;
		$t = $wpdb->prefix . self::TABLE;
		return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$t} WHERE email=%s ORDER BY created_at DESC", $email ), ARRAY_A ) ?? array();
	}
	...
	private static function sendInvitationEmail( string $email, ?string $name, int $inviterId, string $code, ?int $groupId ): void {
		...
		$link        = add_query_arg( array( 'invitation' => $code ), home_url( '/register' ) );
		...
		if ( $groupId ) {
			...
			if ( $group ) {
				$message .= "Você foi convidado para o grupo: {$group}\n\n";}
		}
