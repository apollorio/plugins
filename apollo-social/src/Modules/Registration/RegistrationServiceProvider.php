					...
					<p>
						...
						<?php elseif ($question['type'] === 'textarea') : ?>
							<textarea
								...
								rows="3"
								required
								placeholder="<?php echo esc_attr($question['placeholder'] ?? ''); ?>"></textarea>
						<?php else : ?>
							<input
...
<?php
	...
	public function validateRegistrationFields(\WP_Error $errors, string $sanitized_user_login, string $user_email): \WP_Error
	{
		...
		// Validate Cultura::Rio Identity (must include at least subscriber = clubber)
		if (empty($_POST['apollo_cultura_identity']) || ! is_array($_POST['apollo_cultura_identity'])) {
			...
			// Ensure subscriber (clubber) is always included
			if (! in_array('subscriber', $identities, true)) {
				$identities[] = 'subscriber';
			}
