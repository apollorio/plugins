...
final class ModerationRepository {
	...
	public static function addToQueue( string $contentType, int $contentId, int $authorId, string $title, array $meta = array() ): ?int {
		global $wpdb;
		$exists = $wpdb->get_var(
			$wpdb->prepare(
	...
	public static function assign( int $id, int $moderatorId ): bool {
		...
		return (bool) $wpdb->update(
			$wpdb->prefix . self::QUEUE,
			array( 'assigned_moderator_id' => $moderatorId ),
			array(
				'id'     => $id,
				'status' => 'pending',
			)
	...
	public static function unassign( int $id ): bool {
		global $wpdb;
		return (bool) $wpdb->update( $wpdb->prefix . self::QUEUE, array( 'assigned_moderator_id' => null ), array( 'id' => $id ) );
	}
	...
	public static function countByType(): array {
		global $wpdb;
		$t = $wpdb->prefix . self::QUEUE;
		return $wpdb->get_results( "SELECT content_type,COUNT(*) as count FROM {$t} WHERE status='pending' GROUP BY content_type", ARRAY_A ) ?? array();
	}
	...
	public static function hideActivity( int $activityId, int $moderatorId, string $reason = '' ): bool {
		global $wpdb;
		$r = $wpdb->update( $wpdb->prefix . 'apollo_activity', array( 'is_hidden' => 1 ), array( 'id' => $activityId ) );
		if ( $r ) {
	...
	public static function unhideActivity( int $activityId, int $moderatorId ): bool {
		global $wpdb;
		$r = $wpdb->update( $wpdb->prefix . 'apollo_activity', array( 'is_hidden' => 0 ), array( 'id' => $activityId ) );
		if ( $r ) {
			self::logAction( 'unhide_activity', $activityId, $moderatorId, '' );}
		return (bool) $r;
	}
	...
	public static function deleteActivity( int $activityId, int $moderatorId, string $reason = '' ): bool {
		...
		if ( ! $activity ) {
			return false;}
		self::logAction( 'delete_activity', $activityId, $moderatorId, $reason, array( 'backup' => $activity ) );
		$wpdb->delete( $wpdb->prefix . 'apollo_activity_comments', array( 'activity_id' => $activityId ), array( '%d' ) );
		$wpdb->delete( $wpdb->prefix . 'apollo_activity_likes', array( 'activity_id' => $activityId ), array( '%d' ) );
		$wpdb->delete(
			$wpdb->prefix . 'apollo_mentions',
			array(
		...
		return (bool) $wpdb->delete( $wpdb->prefix . 'apollo_activity', array( 'id' => $activityId ), array( '%d' ) );
	}

	private static function logAction( string $action, int $contentId, int $moderatorId, string $reason, array $extra = array() ): void {
		global $wpdb;
		$wpdb->insert(
