...
final class ContentModerationRepository {
	...
	public static function reject(int $queueId, int $moderatorId, string $notes='', bool $deleteContent=false): bool {
		...
		if($result&&$deleteContent){
			...
			if($item)self::deleteContent($item['content_type'],(int)$item['content_id']);
		}
		return $result;
	}
	...
	public static function getQueueStats(): array {
		global $wpdb;
		$t=$wpdb->prefix.self::T_QUEUE;
		$stats=$wpdb->get_results("SELECT status,COUNT(*) as count FROM {$t} GROUP BY status",ARRAY_A)??[];
		$result=['pending'=>0,'approved'=>0,'rejected'=>0,'escalated'=>0];
		foreach($stats as $s)$result[$s['status']]=(int)$s['count'];
		return $result;
	}
	...
	public static function getUserModerationHistory(int $userId): array {
		...
		$history=['approved'=>0,'rejected'=>0,'escalated'=>0,'total'=>0];
	...
	public static function getRules(bool $activeOnly=true): array {
		...
		return $wpdb->get_results("SELECT * FROM {$t} {$where} ORDER BY priority DESC",ARRAY_A)??[];
	}

	public static function toggleRule(int $ruleId, bool $active): bool {
		global $wpdb;
		$t=$wpdb->prefix.self::T_RULES;
		return (bool)$wpdb->update($t,['is_active'=>$active?1:0],['id'=>$ruleId],['%d'],['%d']);
	}

	public static function deleteRule(int $ruleId): bool {
		global $wpdb;
		$t=$wpdb->prefix.self::T_RULES;
		return (bool)$wpdb->delete($t,['id'=>$ruleId],['%d']);
	}
		...
		foreach($rules as $rule){
			if($rule['type']==='keyword'){
				if(stripos($content,$rule['pattern'])!==false){
					$flags[]=['rule_id'=>$rule['id'],'type'=>'keyword','pattern'=>$rule['pattern'],'action'=>$rule['action']];
				}
			}elseif($rule['type']==='regex'){
				if(preg_match('/'.$rule['pattern'].'/iu',$content)){
					$flags[]=['rule_id'=>$rule['id'],'type'=>'regex','pattern'=>$rule['pattern'],'action'=>$rule['action']];
				}
			}elseif($rule['type']==='url'){
				if(preg_match('/https?:\/\/[^\s]*'.preg_quote($rule['pattern'],'/').'/i',$content)){
					$flags[]=['rule_id'=>$rule['id'],'type'=>'url','pattern'=>$rule['pattern'],'action'=>$rule['action']];
				}
			}
		}
		return $flags;
	...
	private static function deleteContent(string $type, int $id): void {
		global $wpdb;
		match($type){
			'activity'=>$wpdb->delete($wpdb->prefix.'apollo_activity',['id'=>$id],['%d']),
			'comment'=>$wpdb->delete($wpdb->prefix.'apollo_activity_comments',['id'=>$id],['%d']),
			'message'=>$wpdb->update($wpdb->prefix.'apollo_messages',['is_deleted'=>1],['id'=>$id],['%d'],['%d']),
			default=>null
		};
	}
