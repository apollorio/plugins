...
final class MyDataRepository {
	...
	public static function getMyActivity(int $userId,int $limit=20,int $offset=0): array {
		return\Apollo\Modules\Activity\ActivityRepository::getFeed(['user_id'=>$userId,'limit'=>$limit,'offset'=>$offset,'privacy'=>null]);
	}
	...
	public static function getMyFriends(int $userId,int $limit=50,int $offset=0): array {
		...
		foreach($friends as &$f){
			...
			$f['avatar_url']=get_avatar_url($f['ID'],['size'=>100]);
		}
		return $friends;
	}
	...
	public static function getMyAdverts(int $userId,int $limit=20,int $offset=0): array {
		$args=['author'=>$userId,'post_type'=>'advert','posts_per_page'=>$limit,'offset'=>$offset,'post_status'=>['publish','pending','draft']];
		$query=new\WP_Query($args);
		return$query->posts?:[];
	}

	public static function getMyEvents(int $userId,int $limit=20,int $offset=0): array {
		$args=['author'=>$userId,'post_type'=>'event_listing','posts_per_page'=>$limit,'offset'=>$offset,'post_status'=>['publish','pending','draft']];
		$query=new\WP_Query($args);
		return$query->posts?:[];
	}
	...
	public static function getMyInterestedEvents(int $userId,int $limit=20,int $offset=0): array {
		...
		$eventIds=$wpdb->get_col($wpdb->prepare(
			"SELECT meta_value FROM {$wpdb->usermeta} WHERE user_id=%d AND meta_key='apollo_interested_events'",$userId
		));
		if(empty($eventIds))return[];
		$ids=maybe_unserialize($eventIds[0]??'');
		if(!is_array($ids)||empty($ids))return[];
		$args=['post_type'=>'event_listing','post__in'=>$ids,'posts_per_page'=>$limit,'offset'=>$offset,'post_status'=>'publish'];
		$query=new\WP_Query($args);
		return$query->posts?:[];
	}
	...
	public static function unmarkEventInterested(int $userId,int $eventId): bool {
		$interested=(array)get_user_meta($userId,'apollo_interested_events',true);
		$interested=array_diff($interested,[$eventId]);
	...
	public static function getMyNotifications(int $userId,int $limit=20): array {
		...
		foreach($pendingFriends as $pr){
			$notifications[]=['type'=>'friend_request','data'=>$pr,'created_at'=>$pr['created_at']??current_time('mysql')];
		}
		$mentions=self::getMyMentions($userId,true,10);
		foreach($mentions as $m){
			$notifications[]=['type'=>'mention','data'=>$m,'created_at'=>$m['created_at']];
		}
