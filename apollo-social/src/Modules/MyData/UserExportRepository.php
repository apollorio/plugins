...
final class UserExportRepository {
	...
	public static function requestExport( int $userId, array $dataTypes = array() ): int|false {
		global $wpdb;
		$t       = $wpdb->prefix . self::T_EXPORTS;
		$pending = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$t} WHERE user_id=%d AND status='pending'",
				$userId
			)
		);
		if ( $pending ) {
		...
		if ( empty( $dataTypes ) ) {
			$dataTypes = array( 'profile', 'activity', 'connections', 'messages', 'media', 'settings' );
		}
		$wpdb->insert(
			$t,
			array(
	...
	public static function getUserExports( int $userId ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$t} WHERE user_id=%d ORDER BY requested_at DESC LIMIT 10",
				$userId
			),
			ARRAY_A
		) ?? array();
	...
	private static function exportProfile( int $userId ): array {
		...
		$meta = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT meta_key,meta_value FROM {$wpdb->usermeta} WHERE user_id=%d AND meta_key LIKE 'apollo_%'",
				$userId
			),
			ARRAY_A
		);
	...
	private static function exportActivity( int $userId ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT type,content,created_at FROM {$wpdb->prefix}apollo_activity WHERE user_id=%d ORDER BY created_at DESC",
				$userId
			),
			ARRAY_A
		) ?? array();
	...
	private static function exportConnections( int $userId ): array {
		...
		return array(
			...
			'followers' => $wpdb->get_col(
				$wpdb->prepare(
					"SELECT follower_id FROM {$wpdb->prefix}apollo_followers WHERE following_id=%d",
					$userId
				)
			),
			'following' => $wpdb->get_col(
				$wpdb->prepare(
					"SELECT following_id FROM {$wpdb->prefix}apollo_followers WHERE follower_id=%d",
					$userId
				)
			),
		);
	...
	private static function exportMedia( int $userId ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT url,file_type,created_at FROM {$wpdb->prefix}apollo_media_library WHERE user_id=%d",
				$userId
			),
			ARRAY_A
		) ?? array();
	...
	private static function exportSettings( int $userId ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT setting_key,setting_value FROM {$wpdb->prefix}apollo_user_settings WHERE user_id=%d",
				$userId
			),
			ARRAY_A
		) ?? array();
	...
	private static function exportGroups( int $userId ): array {
		...
		return array(
			'member_of' => $wpdb->get_results(
				$wpdb->prepare(
					"SELECT g.id,g.name,gm.role,gm.created_at as joined_at FROM {$g} g JOIN {$m} gm ON g.id=gm.group_id WHERE gm.user_id=%d",
					$userId
				),
				ARRAY_A
			) ?? array(),
			'created'   => $wpdb->get_results(
				$wpdb->prepare(
					"SELECT id,name,created_at FROM {$g} WHERE creator_id=%d",
					$userId
				),
				ARRAY_A
			) ?? array(),
	...
	private static function exportEvents( int $userId ): array {
		...
		return array(
			'attending' => $wpdb->get_results(
				$wpdb->prepare(
					"SELECT e.id,e.title,e.start_date,a.status FROM {$e} e JOIN {$a} a ON e.id=a.event_id WHERE a.user_id=%d",
					$userId
				),
				ARRAY_A
			) ?? array(),
			'organized' => $wpdb->get_results(
				$wpdb->prepare(
					"SELECT id,title,start_date,end_date FROM {$e} WHERE organizer_id=%d",
					$userId
				),
				ARRAY_A
			) ?? array(),
	...
	public static function cleanupExpired(): int {
		global $wpdb;
		$t       = $wpdb->prefix . self::T_EXPORTS;
		$expired = $wpdb->get_results( "SELECT id,user_id,file_path FROM {$t} WHERE expires_at<NOW()", ARRAY_A ) ?? array();
		$count   = 0;
		foreach ( $expired as $e ) {
			if ( self::deleteExport( (int) $e['id'], (int) $e['user_id'] ) ) {
				++$count;
			}
		}
		return $count;
	...
	public static function getPendingExports( int $limit = 10 ): array {
		...
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$t} WHERE status='pending' ORDER BY requested_at ASC LIMIT %d",
				$limit
			),
			ARRAY_A
		) ?? array();
	...
	public static function processPendingExports( int $limit = 5 ): int {
		...
		foreach ( $pending as $export ) {
			if ( self::processExport( (int) $export['id'] ) ) {
				++$processed;
			}
		}
		return $processed;
