...
class ClassifiedsModule {
	...
	public const META_KEYS = array(
		...
		'season_id'    => '_classified_season_id',      // Season taxonomy term ID (correlation)
	);
	...
	public static function init(): void {
		...
		// Hook into apollo-events-manager to connect season taxonomy
		add_action( 'apollo_events_post_types_loaded', array( self::class, 'connect_seasons_taxonomy' ), 10 );
	...
	public static function register_post_type(): void {
		...
		// Register event_season taxonomy for classifieds if apollo-events-manager is active
		if ( taxonomy_exists( 'event_season' ) ) {
			register_taxonomy_for_object_type( 'event_season', self::POST_TYPE );

			if ( defined( 'APOLLO_DEBUG' ) && APOLLO_DEBUG ) {
				error_log( '✅ Apollo: event_season taxonomy registered for classifieds' );
			}
		}
	}

	/**
	 * Connect seasons taxonomy from apollo-events-manager
	 * Called via apollo_events_post_types_loaded hook
	 *
	 * @return void
	 */
	public static function connect_seasons_taxonomy(): void {
		if ( taxonomy_exists( 'event_season' ) ) {
			register_taxonomy_for_object_type( 'event_season', self::POST_TYPE );

			if ( defined( 'APOLLO_DEBUG' ) && APOLLO_DEBUG ) {
				error_log( '✅ Apollo: event_season taxonomy connected to classifieds via hook' );
			}
		}
	}
	...
	public static function register_meta_fields(): void {
		foreach ( self::META_KEYS as $key => $meta_key ) {
			// Special handling for season_id (integer type)
			$field_type  = ( 'season_id' === $key ) ? 'integer' : 'string';
			$sanitize_cb = ( 'season_id' === $key ) ? 'absint' : 'sanitize_text_field';
	...
	public static function rest_get_classified( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( ! $post || $post->post_type !== self::POST_TYPE ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Anúncio não encontrado.', 'apollo-social' ),
				),
				404
			);
		}
	...
	public static function rest_create_classified( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( $today_count >= 5 && ! current_user_can( 'manage_options' ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Limite diário de anúncios atingido (máx. 5).', 'apollo-social' ),
				),
				429
			);
		}
		...
		if ( empty( $title ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Título é obrigatório.', 'apollo-social' ),
				),
				400
			);
		}
		...
		if ( ! array_key_exists( $domain, self::DOMAINS ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Categoria inválida.', 'apollo-social' ),
				),
				400
			);
		}
		...
		if ( ! array_key_exists( $intent, self::INTENTS ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Tipo de anúncio inválido.', 'apollo-social' ),
				),
				400
			);
		}
		...
		if ( $domain === 'ingressos' ) {
			...
			if ( empty( $event_date ) ) {
				return new \WP_REST_Response(
					array(
						'success' => false,
						'message' => __( 'Data do evento é obrigatória para ingressos.', 'apollo-social' ),
					),
					400
				);
			}
			...
			if ( empty( $start_date ) || empty( $end_date ) ) {
				return new \WP_REST_Response(
					array(
						'success' => false,
						'message' => __( 'Check-in e check-out são obrigatórios para acomodação.', 'apollo-social' ),
					),
					400
				);
			}
		...
		if ( is_wp_error( $post_id ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Erro ao criar anúncio.', 'apollo-social' ),
				),
				500
			);
		}
	...
	public static function rest_update_classified( \WP_REST_Request $request ): \WP_REST_Response {
		...
		if ( (int) $post->post_author !== $user_id && ! current_user_can( 'manage_options' ) ) {
			return new \WP_REST_Response(
				array(
					'success' => false,
					'message' => __( 'Sem permissão.', 'apollo-social' ),
				),
				403
			);
		}
	...
	public static function rest_delete_classified( \WP_REST_Request $request ): \WP_REST_Response {
		...
		return new \WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Anúncio removido.', 'apollo-social' ),
			),
			200
		);
	}
	...
	public static function rest_report_classified( \WP_REST_Request $request ): \WP_REST_Response {
		...
		return new \WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Denúncia registrada. Obrigado!', 'apollo-social' ),
			),
			200
		);
	}
