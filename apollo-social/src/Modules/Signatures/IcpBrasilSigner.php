...
class IcpBrasilSigner
{
    ...
    {
        if (!file_exists($pdf_path)) {
            return ['valid' => false, 'error' => 'PDF não encontrado'];
        }
        ...
        return ['valid' => false, 'error' => 'Nenhuma assinatura encontrada'];
    }
    ...
    private function extractCertificate(string $pfx_data, string $password): array
    {
        ...
        if (!openssl_pkcs12_read($pfx_data, $certs, $password)) {
            return ['success' => false, 'error' => 'Senha do certificado incorreta ou certificado inválido'];
        }
        
        if (empty($certs['cert']) || empty($certs['pkey'])) {
            return ['success' => false, 'error' => 'Certificado ou chave privada não encontrados'];
        }
    ...
    private function validateIcpBrasilCertificate(string $certificate): array
    {
        ...
        if ($now < $cert_data['validFrom_time_t'] || $now > $cert_data['validTo_time_t']) {
            return ['valid' => false, 'error' => 'Certificado expirado ou ainda não válido'];
        }
        ...
        $icp_identifiers = [
            'AC SERASA', 'AC CERTISIGN', 'AC VALID', 'AC SOLUTI',
            'AC SAFEWEB', 'ICP-Brasil', 'Autoridade Certificadora'
        ];
    ...
    private function verifyPkcs7Signature(string $pdf_path, string $content): array
    {
        ...
        return ['valid' => false, 'error' => 'Formato de assinatura não reconhecido'];
    }
