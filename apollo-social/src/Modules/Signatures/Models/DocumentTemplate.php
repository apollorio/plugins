...
class DocumentTemplate
{
    ...
    private function guessPlaceholderType(string $key): string
    {
        ...
        if (strpos($key, 'phone') !== false || strpos($key, 'telefone') !== false) return 'tel';
        if (strpos($key, 'date') !== false || strpos($key, 'data') !== false) return 'date';
        if (strpos($key, 'cpf') !== false || strpos($key, 'cnpj') !== false) return 'text';
        if (strpos($key, 'value') !== false || strpos($key, 'valor') !== false || strpos($key, 'price') !== false) return 'number';
