...
class AuditLog
{
    ...
    public function logSignature(
        ...
        return $this->log($document_id, 'signed', [
    ...
    public function generateProtocol(int $document_id, string $document_hash): array
    {
        ...
        return ['success' => false, 'error' => 'Falha ao gerar protocolo'];
    }
    ...
    public function verifyByProtocol(string $protocol_code, ?string $provided_hash = null): array
    {
        ...
        // Log verification
        $this->log($protocol['document_id'], 'verified', [
            'details' => ['protocol_code' => $protocol_code]
        ]);
    ...
    public function generateVerificationReport(int $document_id): array
    {
        ...
        return [
            ...
            'audit_trail' => array_slice($logs, 0, 20), // Last 20 events
    ...
    public function revokeProtocol(string $protocol_code, string $reason = ''): array
    {
        ...
        if ($protocol['status'] === 'revoked') {
            return ['success' => false, 'error' => 'Protocolo já foi revogado'];
        }
        ...
        if ($result !== false) {
            // Log revocation
            $this->log($protocol['document_id'], 'revoked', [
                'details' => [
                    'protocol_code' => $protocol_code,
                    'reason' => $reason
                ]
            ...
            return ['success' => true, 'message' => 'Protocolo revogado'];
        }
        
        return ['success' => false, 'error' => 'Erro ao revogar protocolo'];
    }
