...
final class NoticesRepository {
	...
	public static function create(array $d): int {
		...
		$wpdb->insert($wpdb->prefix.self::TABLE,[
			...
			'type'=>in_array($d['type']??'',['info','warning','success','error','promo'])?$d['type']:'info',
	...
	public static function getActiveForUser(int $userId): array {
		...
		foreach($notices as $n){
			...
			if(!empty($n['target_roles'])&&!array_intersect($userRoles,$n['target_roles']))continue;
			$filtered[]=$n;
		}
		return $filtered;
	}
	...
	public static function dismiss(int $userId,int $noticeId): bool {
		global $wpdb;
		return $wpdb->replace($wpdb->prefix.self::USER_TABLE,['user_id'=>$userId,'notice_id'=>$noticeId])!==false;
	}

	public static function isDismissed(int $userId,int $noticeId): bool {
		global $wpdb;
		return (bool)$wpdb->get_var($wpdb->prepare(
			"SELECT 1 FROM {$wpdb->prefix}".self::USER_TABLE." WHERE user_id=%d AND notice_id=%d",$userId,$noticeId
		));
	}
	...
	public static function getAll(bool $activeOnly=true): array {
		...
		$rows=$wpdb->get_results("SELECT * FROM {$wpdb->prefix}".self::TABLE." {$w} ORDER BY priority DESC,created_at DESC",ARRAY_A)?:[];
		foreach($rows as &$r){
			$r['target_roles']=json_decode($r['target_roles']??'',true);
			$r['target_memberships']=json_decode($r['target_memberships']??'',true);
			$r['target_conditions']=json_decode($r['target_conditions']??'',true);
	...
	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['title','content','type','priority','is_dismissible','is_active','start_at','end_at'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		if(isset($d['target_roles']))$u['target_roles']=wp_json_encode($d['target_roles']);
		if(isset($d['target_memberships']))$u['target_memberships']=wp_json_encode($d['target_memberships']);
		if(isset($d['target_conditions']))$u['target_conditions']=wp_json_encode($d['target_conditions']);
		return $wpdb->update($wpdb->prefix.self::TABLE,$u,['id'=>$id])!==false;
	}

	public static function delete(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::USER_TABLE,['notice_id'=>$id]);
		return $wpdb->delete($wpdb->prefix.self::TABLE,['id'=>$id])!==false;
	}

	public static function resetDismissals(int $noticeId): bool {
		global $wpdb;
		return $wpdb->delete($wpdb->prefix.self::USER_TABLE,['notice_id'=>$noticeId])!==false;
	}
}
