...
final class SitewideNotices {
	...
	public function init(): void {
		...
		add_action( 'wp_footer', array( $this, 'renderNotices' ) );
		add_shortcode( 'apollo_notices', array( $this, 'shortcodeNotices' ) );
	}
	public function create( array $data ): int {
		global $wpdb;
		$wpdb->insert(
			$wpdb->prefix . 'apollo_notices',
			array(
	...
	public function delete( int $id ): bool {
		global $wpdb;
		return (bool) $wpdb->delete( $wpdb->prefix . 'apollo_notices', array( 'id' => $id ) );
	}
	public function getActiveNotices( int $user_id = 0 ): array {
		...
		$dismissed = $user_id ? ( get_user_meta( $user_id, '_apollo_dismissed_notices', true ) ?: array() ) : array();
		$result    = array();
		foreach ( $notices as $notice ) {
			if ( in_array( (int) $notice['id'], $dismissed, true ) ) {
				continue;
			}
			if ( ! $this->matchesTarget( $notice, $user_id ) ) {
				continue;
			}
	...
	private function matchesTarget( array $notice, int $user_id ): bool {
		...
		return match ( $target ) {
			...
			'identity' => in_array( $value, get_user_meta( $user_id, 'apollo_cultura_identities', true ) ?: array(), true ),
			default => true,
		};
	}
	...
	public function dismiss( int $user_id, int $notice_id ): bool {
		$dismissed = get_user_meta( $user_id, '_apollo_dismissed_notices', true ) ?: array();
	...
	public function shortcodeNotices( array $atts = array() ): string {
		$notices = $this->getActiveNotices();
		if ( empty( $notices ) ) {
			return '';
		}
		...
		foreach ( $notices as $notice ) {
			$html .= sprintf(
				'<div class="apollo-notice apollo-notice-%s" data-id="%d">%s<strong>%s</strong> %s%s</div>',
	...
	public function registerEndpoints(): void {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => fn( $r ) => new \WP_REST_Response( array( 'success' => $this->dismiss( get_current_user_id(), (int) $r->get_param( 'notice_id' ) ) ), 200 ),
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => fn( $r ) => new \WP_REST_Response( array( 'id' => $this->create( $r->get_json_params() ) ), 200 ),
				'permission_callback' => fn() => current_user_can( 'manage_options' ),
			)
		);
	}
