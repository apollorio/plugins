...
final class ContentRestrictionRepository {
	...
	public static function canAccess(string $objectType,int $objectId,int $userId): bool {
		...
		if(!empty($restriction['users'])&&in_array($userId,$restriction['users']))return true;
		if(!empty($restriction['roles'])){
			if(empty(array_intersect($user->roles,$restriction['roles'])))return false;
		}
		if(!empty($restriction['groups'])){
			...
			if(empty(array_intersect($userGroupIds,$restriction['groups'])))return false;
		}
	...
	public static function getRestrictedObjects(string $objectType): array {
		global $wpdb;
		return $wpdb->get_col($wpdb->prepare("SELECT object_id FROM {$wpdb->prefix}".self::TABLE." WHERE object_type=%s",$objectType))?:[];
	}
}
