...
final class TwoFactorRepository {
	...
	public static function disable( int $userId, string $code ): bool {
		...
		delete_user_meta( $userId, 'apollo_2fa_pending' );
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . self::T_BACKUP_CODES, array( 'user_id' => $userId ), array( '%d' ) );
		$wpdb->delete( $wpdb->prefix . self::T_TRUSTED, array( 'user_id' => $userId ), array( '%d' ) );
		return true;
	}
	...
	public static function generateBackupCodes( int $userId, int $count = 10 ): array {
		...
		$wpdb->delete( $t, array( 'user_id' => $userId ), array( '%d' ) );
		$codes = array();
		for ( $i = 0;$i < $count;$i++ ) {
	...
	public static function useBackupCode( int $userId, string $code ): bool {
		...
		foreach ( $codes as $c ) {
			if ( wp_check_password( $code, $c['code_hash'] ) ) {
				$wpdb->update( $t, array( 'used_at' => gmdate( 'Y-m-d H:i:s' ) ), array( 'id' => $c['id'] ), array( '%s' ), array( '%d' ) );
				return true;
			}
		}
		return false;
	...
	public static function isTrustedDevice( int $userId, string $deviceId ): bool {
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$t} WHERE user_id=%d AND device_hash=%s AND expires_at>NOW()",
				$userId,
				$hash
			)
		);
	...
	public static function validateSession( string $token ): ?int {
		...
		$session = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT user_id FROM {$t} WHERE token_hash=%s AND expires_at>NOW() AND used_at IS NULL",
				$hash
			),
			ARRAY_A
		);
		...
		$wpdb->update( $t, array( 'used_at' => gmdate( 'Y-m-d H:i:s' ) ), array( 'token_hash' => $hash ), array( '%s' ), array( '%s' ) );
