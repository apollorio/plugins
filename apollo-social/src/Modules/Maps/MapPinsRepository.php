...
final class MapPinsRepository {
	...
	public static function getAll( bool $activeOnly = true ): array {
		...
		$rows = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}" . self::TABLE . " {$w} ORDER BY title", ARRAY_A ) ?: array();
		foreach ( $rows as &$r ) {
			$r['meta'] = json_decode( $r['meta'] ?? '', true );}
		return $rows;
	}
	...
	public static function getByCategory( string $category, bool $activeOnly = true ): array {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}" . self::TABLE . " WHERE category=%s {$w} ORDER BY title",
				$category
			),
			ARRAY_A
		) ?: array();
	...
	public static function getInBounds( float $swLat, float $swLng, float $neLat, float $neLng, ?string $category = null ): array {
		...
		$params   = array( $swLat, $neLat, $swLng, $neLng );
		if ( $category ) {
			$params[] = $category;
		}
	...
	public static function update( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'title', 'description', 'latitude', 'longitude', 'address', 'category', 'icon', 'color', 'link_url', 'object_type', 'object_id', 'is_active' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
		if ( isset( $d['meta'] ) ) {
			$u['meta'] = wp_json_encode( $d['meta'] );
		}
	...
	public static function toGeoJson( array $pins ): array {
		...
		foreach ( $pins as $p ) {
			$features[] = array(
				...
				'geometry'   => array(
					'type'        => 'Point',
					'coordinates' => array( (float) $p['longitude'], (float) $p['latitude'] ),
				),
				'properties' => array(
					'id'          => $p['id'],
					'title'       => $p['title'],
