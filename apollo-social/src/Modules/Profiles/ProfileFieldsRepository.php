...
final class ProfileFieldsRepository {
	...
	public static function createGroup(array $d): int {
		global $wpdb;
		$wpdb->insert($wpdb->prefix.self::GROUPS,[
			'name'=>sanitize_text_field($d['name']),
			'slug'=>sanitize_title($d['slug']??$d['name']),
			'description'=>sanitize_textarea_field($d['description']??''),
			'context'=>in_array($d['context']??'',['user','comuna','nucleo','all'])?$d['context']:'user',
	...
	public static function getGroups(string $context='all'): array {
		...
		return $wpdb->get_results("SELECT * FROM {$wpdb->prefix}".self::GROUPS." {$w} AND is_active=1 ORDER BY sort_order",ARRAY_A)?:[];
	}

	public static function updateGroup(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['name','slug','description','context','is_repeatable','sort_order','is_active'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		return $wpdb->update($wpdb->prefix.self::GROUPS,$u,['id'=>$id])!==false;
	}
	...
	public static function deleteGroup(int $id): bool {
		...
		return $wpdb->delete($wpdb->prefix.self::GROUPS,['id'=>$id])!==false;
	}

	public static function reorderGroups(array $order): bool {
		global $wpdb;
		foreach($order as $pos=>$id){
			$wpdb->update($wpdb->prefix.self::GROUPS,['sort_order'=>$pos],['id'=>(int)$id]);
		}
		return true;
	}
	...
	public static function createField(array $d): int {
		global $wpdb;
		$wpdb->insert($wpdb->prefix.self::FIELDS,[
	...
	public static function getFieldsByGroup(int $groupId): array {
		...
		$rows=$wpdb->get_results($wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}".self::FIELDS." WHERE group_id=%d ORDER BY sort_order",$groupId
		),ARRAY_A)?:[];
		foreach($rows as &$r){
			$r['options']=json_decode($r['options']??'',true);
			$r['validation_rules']=json_decode($r['validation_rules']??'',true);
	...
	public static function updateField(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['group_id','name','slug','type','placeholder','default_value','is_required','is_public','is_searchable','is_editable','visibility','sort_order'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		if(isset($d['options']))$u['options']=wp_json_encode($d['options']);
		if(isset($d['validation_rules']))$u['validation_rules']=wp_json_encode($d['validation_rules']);
		if(isset($d['conditional_logic']))$u['conditional_logic']=wp_json_encode($d['conditional_logic']);
		return $wpdb->update($wpdb->prefix.self::FIELDS,$u,['id'=>$id])!==false;
	}

	public static function deleteField(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::VALUES,['field_id'=>$id]);
		return $wpdb->delete($wpdb->prefix.self::FIELDS,['id'=>$id])!==false;
	}

	public static function reorderFields(int $groupId,array $order): bool {
		global $wpdb;
		foreach($order as $pos=>$id){
			$wpdb->update($wpdb->prefix.self::FIELDS,['sort_order'=>$pos],['id'=>(int)$id,'group_id'=>$groupId]);
		}
		return true;
	}
	...
	public static function saveValue(int $userId,int $fieldId,$value): bool {
		...
		return $wpdb->replace($wpdb->prefix.self::VALUES,['user_id'=>$userId,'field_id'=>$fieldId,'value'=>$v])!==false;
	}
	...
	public static function calculateCompleteness(int $userId,string $context='user'): array {
		...
		$total=0;$filled=0;$required=0;$requiredFilled=0;
