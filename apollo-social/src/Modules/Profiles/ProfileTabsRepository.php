...
final class ProfileTabsRepository {
	...
	public static function create( array $d ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'content_type'  => in_array( $d['content_type'] ?? '', array( 'fields', 'shortcode', 'template', 'custom' ) ) ? $d['content_type'] : 'fields',
				'content'       => $d['content'] ?? '',
	...
	public static function get( int $id ): ?array {
		...
		if ( $r ) {
			$r['roles_allowed'] = json_decode( $r['roles_allowed'] ?? '', true );
		}
		return $r ?: null;
	}
	...
	public static function getAll( bool $activeOnly = true ): array {
		...
		$rows = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}" . self::TABLE . " {$w} ORDER BY sort_order", ARRAY_A ) ?: array();
		foreach ( $rows as &$r ) {
			$r['roles_allowed'] = json_decode( $r['roles_allowed'] ?? '', true );
		}
		return $rows;
	...
	public static function update( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'name', 'slug', 'icon', 'content_type', 'content', 'visibility', 'sort_order', 'is_active' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
		if ( isset( $d['roles_allowed'] ) ) {
			$u['roles_allowed'] = wp_json_encode( $d['roles_allowed'] );
		}
	...
	public static function reorder( array $order ): bool {
		...
		foreach ( $order as $pos => $id ) {
			$wpdb->update( $wpdb->prefix . self::TABLE, array( 'sort_order' => $pos ), array( 'id' => (int) $id ) );
		}
		return true;
	}
}
