...
class DocumentsSchema implements SchemaModuleInterface {
	...
	private static function migrate_2_1_0(): array {
		...
		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $signatures_table ) ) !== $signatures_table ) {
	...
	public function getStatus(): array {
		...
		// SECURITY FIX: Use prepared statements for table existence checks.
		return array(
			'apollo_documents'           => $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $docs_table ) ) === $docs_table,
			'apollo_document_signatures' => $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $sigs_table ) ) === $sigs_table,
		);
	}
