...
class DocumentsAjaxHandler
{
    ...
    {
        // Save document.
        add_action('wp_ajax_apollo_save_document', [ $this, 'handle_save_document' ]);

        // Export PDF.
        add_action('wp_ajax_apollo_export_document_pdf', [ $this, 'handle_export_pdf' ]);

        // Prepare for signing.
        add_action('wp_ajax_apollo_prepare_document_signing', [ $this, 'handle_prepare_for_signing' ]);
    }
    ...
    public function handle_save_document(): void
    {
        ...
        if ($is_new) {
            ...
            if ((int) $document['created_by'] !== $user_id && ! current_user_can('manage_options')) {
                wp_send_json_error([ 'message' => 'Você não tem permissão para editar este documento.' ]);
            }
    ...
    public function handle_prepare_for_signing(): void
    {
        ...
        if (0 === $document_id) {
            wp_send_json_error([ 'message' => 'ID do documento inválido.' ]);
        }
        ...
        // Check ownership.
        if ((int) $document['created_by'] !== $user_id && ! current_user_can('manage_options')) {
            wp_send_json_error([ 'message' => 'Você não tem permissão para preparar este documento.' ]);
        }
