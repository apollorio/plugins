...
class DocumentLibraries
{
    ...
    public const LIBRARY_TYPES = [
        ...
        'private' => [
            ...
            'color' => '#3b82f6',
            'roles' => ['owner'] // Apenas o dono
        ]
    ];
    
    /** @var array Status de documento */
    public const DOCUMENT_STATUS = [
        'draft' => ['name' => 'Rascunho', 'color' => '#f59e0b'],
        'ready' => ['name' => 'Pronto', 'color' => '#3b82f6'],
        'signing' => ['name' => 'Em Assinatura', 'color' => '#8b5cf6'],
        'completed' => ['name' => 'Concluído', 'color' => '#10b981'],
        'archived' => ['name' => 'Arquivado', 'color' => '#64748b']
    ];
    ...
    public function updateDocument(string $file_id, array $data, ?int $user_id = null): array
    {
        ...
        if (!$this->canEditDocument((int) $document['id'], $user_id)) {
            return ['success' => false, 'error' => 'Você não tem permissão para editar este documento'];
        }
        
        // Check if document is finalized
        if ($document['status'] !== 'draft') {
            return ['success' => false, 'error' => 'Documento finalizado não pode ser editado'];
        }
    ...
    public function finalizeDocument(string $file_id, ?int $user_id = null): array
    {
        ...
        if ($document['status'] !== 'draft') {
            return ['success' => false, 'error' => 'Documento já foi finalizado'];
        }
    ...
    public function moveDocument(string $file_id, string $target_library, ?int $user_id = null): array
    {
        ...
        if (!$this->canEditDocument((int) $document['id'], $user_id)) {
            return ['success' => false, 'error' => 'Sem permissão para mover'];
        }
        
        // Check target library write permission
        if (!$this->canWriteToLibrary($target_library, $user_id)) {
            return ['success' => false, 'error' => 'Sem permissão para a biblioteca destino'];
        }
        
        // Validate target library
        if (!isset(self::LIBRARY_TYPES[$target_library])) {
            return ['success' => false, 'error' => 'Biblioteca inválida'];
        }
        ...
        if ($result !== false) {
            return ['success' => true, 'message' => 'Documento movido'];
        }
        
        return ['success' => false, 'error' => 'Erro ao mover documento'];
    }
    ...
    {
        return $this->getDocumentsByLibrary('cenario', null, [
            'is_template' => 1,
    ...
    public function createFromTemplate(string $template_file_id, string $target_library = 'private', ?int $user_id = null): array
    {
        ...
        return $this->createDocument($target_library, [
            'type' => $template['type'],
