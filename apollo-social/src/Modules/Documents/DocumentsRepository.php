...
class DocumentsRepository {
	...
	public static function updateDocument( int $post_id, array $data ) {
		...
		ApolloLogger::logDocument( 'document_updated', $post_id, array(
			'version' => $version + 1,
	...
	public static function transitionStatus( int $post_id, string $new_state, array $context = array() ) {
		...
		if ( ! DocumentStatus::isValidTransition( $old_state, $new_state ) ) {
			ApolloLogger::warning( 'invalid_status_transition', array(
				'post_id'   => $post_id,
				'old_state' => $old_state,
				'new_state' => $new_state,
			), ApolloLogger::CAT_DOCUMENT );
		...
		ApolloLogger::logDocument( 'document_status_changed', $post_id, array(
			'old_state'       => $old_state,
			'new_state'       => $new_state,
	...
	public static function storeSignature( int $post_id, array $signature ) {
		...
		if ( in_array( $current_state, array( 'ready', 'signing' ), true ) ) {
			self::transitionStatus( $post_id, 'signed', array(
		...
		ApolloLogger::logSignature( 'signature_stored', $post_id, array(
	...
	public static function getSignatures( int $post_id ): array {
		...
		if ( ! empty( $legacy ) && is_array( $legacy ) ) {
			...
			ApolloLogger::info( 'signature_metakey_migrated', array(
				'post_id' => $post_id,
				'count'   => count( $legacy ),
