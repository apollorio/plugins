...
final class PointsSystem {
    ...
    private static array $triggers = [
        'user_register' => ['points' => 50, 'label' => 'Cadastro completo', 'once' => true],
        'profile_complete' => ['points' => 100, 'label' => 'Perfil 100%', 'once' => true],
        'avatar_upload' => ['points' => 25, 'label' => 'Avatar enviado', 'once' => true],
        'post_created' => ['points' => 10, 'label' => 'Post criado', 'daily_limit' => 5],
        'post_liked' => ['points' => 2, 'label' => 'Recebeu curtida', 'daily_limit' => 50],
        'gave_like' => ['points' => 1, 'label' => 'Curtiu conteúdo', 'daily_limit' => 20],
        'comment_posted' => ['points' => 5, 'label' => 'Comentou', 'daily_limit' => 10],
        'received_comment' => ['points' => 3, 'label' => 'Recebeu comentário', 'daily_limit' => 20],
        'event_created' => ['points' => 50, 'label' => 'Evento criado'],
        'event_attended' => ['points' => 25, 'label' => 'Participou de evento'],
        'event_interested' => ['points' => 5, 'label' => 'Interesse em evento', 'daily_limit' => 10],
        'classified_posted' => ['points' => 30, 'label' => 'Anúncio publicado'],
        'classified_sold' => ['points' => 50, 'label' => 'Anúncio vendido'],
        'group_created' => ['points' => 75, 'label' => 'Grupo criado', 'once' => true],
        'group_joined' => ['points' => 15, 'label' => 'Entrou em grupo', 'daily_limit' => 3],
        'invite_sent' => ['points' => 10, 'label' => 'Convite enviado', 'daily_limit' => 5],
        'invite_accepted' => ['points' => 25, 'label' => 'Convite aceito'],
        'bubble_added' => ['points' => 20, 'label' => 'Close friend adicionado', 'max' => 15],
        'document_signed' => ['points' => 40, 'label' => 'Documento assinado'],
        'document_created' => ['points' => 20, 'label' => 'Documento criado'],
        'first_login' => ['points' => 10, 'label' => 'Primeiro login', 'once' => true],
        'daily_login' => ['points' => 5, 'label' => 'Login diário'],
        'streak_7_days' => ['points' => 50, 'label' => 'Streak 7 dias', 'once' => false],
        'streak_30_days' => ['points' => 200, 'label' => 'Streak 30 dias', 'once' => false],
        'share_event' => ['points' => 5, 'label' => 'Compartilhou evento', 'daily_limit' => 5],
        'profile_viewed' => ['points' => 1, 'label' => 'Perfil visualizado', 'daily_limit' => 100],
        'supplier_contact' => ['points' => 15, 'label' => 'Contato fornecedor'],
        'testimonial_given' => ['points' => 20, 'label' => 'Depoimento dado'],
        'testimonial_received' => ['points' => 30, 'label' => 'Depoimento recebido'],
        'verified_member' => ['points' => 500, 'label' => 'Membro verificado', 'once' => true],
        'membership_approved' => ['points' => 100, 'label' => 'Membership aprovado', 'once' => true],
        'first_event' => ['points' => 100, 'label' => 'Primeiro evento', 'once' => true],
        'first_classified' => ['points' => 50, 'label' => 'Primeiro anúncio', 'once' => true],
        'first_group' => ['points' => 50, 'label' => 'Primeiro grupo', 'once' => true],
        'nucleo_created' => ['points' => 100, 'label' => 'Núcleo criado'],
        'nucleo_invite_sent' => ['points' => 15, 'label' => 'Convite núcleo'],
        'comuna_moderator' => ['points' => 200, 'label' => 'Moderador de comunidade', 'once' => true],
        'content_featured' => ['points' => 100, 'label' => 'Conteúdo destacado'],
        'report_spam' => ['points' => 10, 'label' => 'Reportou spam', 'daily_limit' => 5],
        'help_newbie' => ['points' => 25, 'label' => 'Ajudou novato'],
        'complete_onboarding' => ['points' => 75, 'label' => 'Onboarding completo', 'once' => true],
    ];
    private static array $ranks = [
        'bronze' => ['min' => 0, 'label' => 'Bronze', 'color' => '#CD7F32', 'icon' => '🥉'],
        'silver' => ['min' => 250, 'label' => 'Prata', 'color' => '#C0C0C0', 'icon' => '🥈'],
        'gold' => ['min' => 1000, 'label' => 'Ouro', 'color' => '#FFD700', 'icon' => '🥇'],
        'platinum' => ['min' => 2500, 'label' => 'Platina', 'color' => '#E5E4E2', 'icon' => '💎'],
        'diamond' => ['min' => 5000, 'label' => 'Diamante', 'color' => '#B9F2FF', 'icon' => '💠'],
        'master' => ['min' => 10000, 'label' => 'Master', 'color' => '#9400D3', 'icon' => '👑'],
        'legend' => ['min' => 25000, 'label' => 'Lenda', 'color' => '#FF4500', 'icon' => '🔥'],
    ];
    ...
    public function award(int $user_id, string $trigger, array $meta = []): bool {
        if (!isset(self::$triggers[$trigger])) return false;
        $config = self::$triggers[$trigger];
        if (!empty($config['once']) && $this->hasTriggered($user_id, $trigger)) return false;
        if (!empty($config['daily_limit']) && $this->getDailyCount($user_id, $trigger) >= $config['daily_limit']) return false;
    ...
    public function deduct(int $user_id, int $points, string $reason = ''): bool {
        ...
        update_user_meta($user_id, '_apollo_points', $new_total);
        $this->logTransaction($user_id, 'deduction', -$points, ['reason' => $reason]);
        return true;
    }
    ...
    public function getRank(int $user_id): array {
        ...
        foreach (self::$ranks as $key => $rank) {
            if ($points >= $rank['min']) $current_rank = $key;
        }
    ...
    private function logTransaction(int $user_id, string $trigger, int $points, array $meta = []): void {
        global $wpdb;
        $wpdb->insert($wpdb->prefix . 'apollo_points_log', [
            'user_id' => $user_id,
    ...
    private function handleLogin(int $user_id): void {
        ...
        if ($last_login !== $today) {
            ...
            if ($last_login === gmdate('Y-m-d', strtotime('-1 day'))) {
                $streak++;
                if ($streak === 7) $this->award($user_id, 'streak_7_days');
                if ($streak === 30) $this->award($user_id, 'streak_30_days');
            } else {
                $streak = 1;
            }
    ...
    public static function getTriggers(): array { return self::$triggers; }
    public static function getRanks(): array { return self::$ranks; }
}
