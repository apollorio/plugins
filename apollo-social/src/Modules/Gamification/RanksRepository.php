...
final class RanksRepository {
	...
	public static function get(int $id): ?array {
		...
		if($r)$r['perks']=json_decode($r['perks']??'',true);
		return $r?:null;
	}
	...
	public static function getAll(string $type='default'): array {
		...
		$rows=$wpdb->get_results($wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}".self::TABLE." WHERE points_type=%s ORDER BY min_points ASC",$type
		),ARRAY_A)?:[];
		foreach($rows as &$r)$r['perks']=json_decode($r['perks']??'',true);
		return $rows;
	}
	...
	public static function getProgressToNextRank(int $userId,string $type='default'): array {
		...
		if(!$next)return ['current'=>$current,'next'=>null,'progress'=>100,'points_needed'=>0];
		...
		return ['current'=>$current,'next'=>$next,'progress'=>min(100,max(0,$prog)),'points_needed'=>max(0,$needed)];
	}

	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['name','slug','points_type','min_points','max_points','icon','color','badge_image_id','sort_order'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		if(isset($d['perks']))$u['perks']=wp_json_encode($d['perks']);
