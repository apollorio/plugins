...
final class AchievementsRepository {
	...
	public static function create(array $d): int {
		...
		$wpdb->insert($wpdb->prefix.self::TABLE,[
			...
			'type'=>in_array($d['type']??'',['badge','milestone','special','competition'])?$d['type']:'badge',
	...
	public static function get(int $id): ?array {
		...
		if($r)$r['trigger_conditions']=json_decode($r['trigger_conditions']??'',true);
		return $r?:null;
	}
	...
	public static function getByTrigger(string $trigger): array {
		...
		$rows=$wpdb->get_results($wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}".self::TABLE." WHERE trigger_name=%s AND is_active=1",$trigger
		),ARRAY_A)?:[];
		foreach($rows as &$r)$r['trigger_conditions']=json_decode($r['trigger_conditions']??'',true);
		return $rows;
	}
	...
	public static function incrementProgress(int $userId,int $achievementId,int $amount=1): array {
		...
		if(!$ach)return ['success'=>false,'error'=>'not_found'];
		...
		$wpdb->query($wpdb->prepare(
			...
			$userId,$achievementId,$newProgress,$isComplete?1:0,$isComplete?current_time('mysql'):null,
			$newProgress,$isComplete?1:0,$isComplete?1:0,current_time('mysql')
		));
		...
		return ['success'=>true,'progress'=>$newProgress,'completed'=>$isComplete,'just_completed'=>$isComplete&&!$wasComplete,'achievement'=>$ach];
	}
	...
	public static function revokeAchievement(int $userId,int $achievementId): bool {
		global $wpdb;
		return $wpdb->delete($wpdb->prefix.self::USER_TABLE,['user_id'=>$userId,'achievement_id'=>$achievementId])!==false;
	}

	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['name','slug','description','type','trigger_name','trigger_count','points_reward','icon','badge_image_id','is_secret','is_active'] as $k){
			if(isset($d[$k]))$u[$k]=$d[$k];
		}
		if(isset($d['trigger_conditions']))$u['trigger_conditions']=wp_json_encode($d['trigger_conditions']);
		return $wpdb->update($wpdb->prefix.self::TABLE,$u,['id'=>$id])!==false;
	}

	public static function delete(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::USER_TABLE,['achievement_id'=>$id]);
	...
	public static function processTrigger(int $userId,string $trigger,int $count=1,?array $context=null): array {
		...
		foreach($achievements as $a){
			if(!empty($a['trigger_conditions'])&&!self::checkConditions($a['trigger_conditions'],$context))continue;
