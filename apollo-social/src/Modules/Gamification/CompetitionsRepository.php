...
final class CompetitionsRepository {
	...
	public static function create( array $d ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'type'        => in_array( $d['type'] ?? '', array( 'leaderboard', 'challenge', 'race' ) ) ? $d['type'] : 'leaderboard',
	...
	public static function get( int $id ): ?array {
		...
		if ( $r ) {
			$r['prizes'] = json_decode( $r['prizes'] ?? '', true );
	...
	public static function getActive(): array {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}" . self::TABLE . " WHERE start_at<=%s AND end_at>=%s AND status='active' ORDER BY end_at",
				$now,
				$now
			),
			ARRAY_A
	...
	public static function getUpcoming(): array {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}" . self::TABLE . " WHERE start_at>%s AND status='upcoming' ORDER BY start_at",
				$now
			),
			ARRAY_A
		) ?: array();
	...
	public static function getUserPosition( int $compId, int $userId ): ?int {
		...
		$score = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT score FROM {$wpdb->prefix}" . self::PARTS . ' WHERE competition_id=%d AND user_id=%d',
				$compId,
				$userId
			)
		);
		...
		return 1 + (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}" . self::PARTS . ' WHERE competition_id=%d AND score>%d',
				$compId,
				(int) $score
			)
		);
	...
	public static function isParticipant( int $compId, int $userId ): bool {
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT 1 FROM {$wpdb->prefix}" . self::PARTS . ' WHERE competition_id=%d AND user_id=%d',
				$compId,
				$userId
			)
		);
	...
	public static function activateScheduled(): int {
		...
		return (int) $wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}" . self::TABLE . " SET status='active' WHERE status='upcoming' AND start_at<=%s",
				$now
			)
		);
	}
	...
	public static function endFinished(): int {
		...
		return (int) $wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}" . self::TABLE . " SET status='ended' WHERE status='active' AND end_at<%s",
				$now
			)
		);
	}
	...
	public static function update( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'name', 'slug', 'description', 'type', 'metric', 'start_at', 'end_at', 'status' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
		if ( isset( $d['prizes'] ) ) {
			$u['prizes'] = wp_json_encode( $d['prizes'] );
		}
		if ( isset( $d['rules'] ) ) {
			$u['rules'] = wp_json_encode( $d['rules'] );
		}
	...
	public static function delete( int $id ): bool {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . self::PARTS, array( 'competition_id' => $id ) );
