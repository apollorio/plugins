...
final class LeaderboardRepository {
	...
	public static function getCompetitions( bool $activeOnly = true ): array {
		...
		$args  = $activeOnly ? array( $now, $now ) : array();
		return $wpdb->get_results(
			$wpdb->prepare(
	...
	public static function createCompetition( array $data ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'prizes'      => wp_json_encode( $data['prizes'] ?? array() ),
	...
	public static function getCompetitionLeaderboard( int $competitionId, int $limit = 50 ): array {
		...
		$args      = array( $comp['start_date'], $comp['end_date'] );
		if ( $comp['category'] ) {
			$args[] = $comp['category'];}
		$args[] = $limit;
		return $wpdb->get_results(
	...
	public static function finalizeCompetition( int $competitionId ): array {
		...
		$wpdb->update(
			...
			array(
				'status'  => 'completed',
				'winners' => wp_json_encode( $winners ),
			),
			array( 'id' => $competitionId ),
			array( '%s', '%s' ),
			array( '%d' )
		);
	...
	public static function getMilestones( int $userId ): array {
		...
		$milestones = array( 100, 500, 1000, 2500, 5000, 10000, 25000, 50000, 100000 );
		$reached    = array();
		$next       = null;
		...
		return array(
			...
			'reached'  => $reached,
			'next'     => $next,
			'progress' => $next ? round( $points / $next * 100, 1 ) : 100,
		);
	}
