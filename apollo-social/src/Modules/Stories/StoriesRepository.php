...
final class StoriesRepository {
	...
	public static function create( int $userId, string $mediaUrl, string $mediaType = 'image', array $data = array() ): int|false {
		global $wpdb;
	...
	public static function getUserStories( int $userId, int $viewerId = 0 ): array {
		...
		if ( $viewerId > 0 ) {
			...
			foreach ( $stories as &$story ) {
				$story['viewed'] = (bool) $wpdb->get_var(
					$wpdb->prepare(
						"SELECT id FROM {$v} WHERE story_id=%d AND viewer_id=%d",
						$story['id'],
						$viewerId
					)
				);
	...
	public static function getFeed( int $userId, int $limit = 20 ): array {
		...
		$allIds       = array_unique( array_merge( $friendIds, $followingIds, array( $userId ) ) );
		if ( empty( $allIds ) ) {
			return array();
		}
		...
		$users        = $wpdb->get_results(
			...
			LIMIT %d",
				array_merge( array( $userId ), $allIds, array( $userId, $limit ) )
			),
			ARRAY_A
		) ?? array();
