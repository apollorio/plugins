...
final class ForumRepository {
	...
	public static function getTopics( array $args = array() ): array {
		global $wpdb;
		$defaults = array(
			'group_id' => null,
			'status'   => 'open',
			'search'   => '',
			'orderby'  => 'is_sticky DESC, last_activity_at',
			'order'    => 'DESC',
			'limit'    => 20,
			'offset'   => 0,
		);
		...
		foreach ( $rows as &$r ) {
			$r['reply_count'] = self::countReplies( (int) $r['id'] );}
		return $rows;
	}
		...
		if ( $replyId ) {
			$wpdb->update(
				...
				array(
					...
					'reply_count'      => self::countReplies( (int) $d['topic_id'] ),
				),
				array( 'id' => (int) $d['topic_id'] )
			);
		}
		return $replyId;
	}
	...
	public static function countReplies( int $topicId ): int {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}" . self::REPLIES . " WHERE topic_id=%d AND status='visible'",
				$topicId
			)
		);
	}
	...
	public static function updateTopic( int $id, array $d ): bool {
		global $wpdb;
		$u = array();
		foreach ( array( 'title', 'content', 'status', 'is_sticky', 'is_closed' ) as $k ) {
			if ( isset( $d[ $k ] ) ) {
				$u[ $k ] = $d[ $k ];
			}
		}
		return $wpdb->update( $wpdb->prefix . self::TOPICS, $u, array( 'id' => $id ) ) !== false;
	}
	...
	public static function updateReply( int $id, array $d ): bool {
		...
		$u = array();
		if ( isset( $d['content'] ) ) {
			$u['content'] = wp_kses_post( $d['content'] );
		}
		if ( isset( $d['status'] ) ) {
			$u['status'] = $d['status'];
		}
		return $wpdb->update( $wpdb->prefix . self::REPLIES, $u, array( 'id' => $id ) ) !== false;
	}

	public static function deleteTopic( int $id ): bool {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . self::REPLIES, array( 'topic_id' => $id ) );
		return $wpdb->delete( $wpdb->prefix . self::TOPICS, array( 'id' => $id ) ) !== false;
	}

	public static function deleteReply( int $id ): bool {
		global $wpdb;
		return $wpdb->delete( $wpdb->prefix . self::REPLIES, array( 'id' => $id ) ) !== false;
	}
	...
	public static function toggleSticky( int $topicId ): bool {
		...
		return $wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}" . self::TOPICS . ' SET is_sticky=NOT is_sticky WHERE id=%d',
				$topicId
			)
		) !== false;
	}
	...
	public static function toggleClosed( int $topicId ): bool {
		...
		return $wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}" . self::TOPICS . ' SET is_closed=NOT is_closed WHERE id=%d',
				$topicId
			)
		) !== false;
	}
