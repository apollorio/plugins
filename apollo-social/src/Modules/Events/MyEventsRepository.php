...
final class MyEventsRepository {
	...
	public static function getStats( int $userId ): array {
		...
		return array(
			...
			'attending_upcoming' => (int) $wpdb->get_var(
				$wpdb->prepare(
			...
			'pending_invites'    => (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$wpdb->prefix}apollo_event_invites WHERE user_id=%d AND status='pending'",
					$userId
				)
			),
		);
