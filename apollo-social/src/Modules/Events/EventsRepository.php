...
final class EventsRepository {
	...
	public static function create( array $d ): ?int {
		...
		$r = $wpdb->insert(
			...
			array( '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%f', '%f', '%s', '%s', '%s', '%d', '%d', '%s', '%d', '%s', '%f', '%s', '%s', '%s', '%s', '%d', '%d', '%d' )
		);
	...
	public static function delete( int $id ): bool {
		global $wpdb;
		$wpdb->delete( $wpdb->prefix . self::ATTENDEES, array( 'event_id' => $id ), array( '%d' ) );
		$wpdb->delete( $wpdb->prefix . self::INTERESTED, array( 'event_id' => $id ), array( '%d' ) );
	...
	public static function search( array $filters = array(), int $limit = 20, int $offset = 0 ): array {
		...
		if ( ! empty( $filters['start_after'] ) ) {
			$where[]  = 'start_date>=%s';
			$params[] = $filters['start_after'];}
		if ( ! empty( $filters['start_before'] ) ) {
			$where[]  = 'start_date<=%s';
			$params[] = $filters['start_before'];}
		if ( isset( $filters['is_free'] ) && $filters['is_free'] ) {
			$where[] = '(price=0 OR price IS NULL)';}
		$w        = \implode( ' AND ', $where );
		$params[] = $limit;
		$params[] = $offset;
		return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$t} WHERE {$w} ORDER BY start_date ASC LIMIT %d OFFSET %d", ...$params ), ARRAY_A ) ?? array();
	}
	...
	public static function getCategories(): array {
		...
		return $wpdb->get_results( "SELECT * FROM {$t} ORDER BY name ASC", ARRAY_A ) ?? array();
	}
