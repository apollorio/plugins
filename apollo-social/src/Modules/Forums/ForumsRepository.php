...
final class ForumsRepository {
	...
	public static function updateForum(int $forumId, array $data): bool {
		...
		if(isset($data['visibility'])){$upd['visibility']=$data['visibility'];$fmt[]='%s';}
		if(isset($data['is_active'])){$upd['is_active']=(int)$data['is_active'];$fmt[]='%d';}
		return $wpdb->update($t,$upd,['id'=>$forumId],$fmt,['%d'])!==false;
	}
	...
	public static function updateTopic(int $topicId, int $userId, array $data): bool {
		...
		if(!$topic||(int)$topic['user_id']!==$userId&&!current_user_can('manage_options')){return false;}
		$upd=[];$fmt=[];
		if(isset($data['title'])){$upd['title']=sanitize_text_field($data['title']);$fmt[]='%s';}
		if(isset($data['content'])){$upd['content']=wp_kses_post($data['content']);$fmt[]='%s';}
		if(isset($data['is_sticky'])&&current_user_can('manage_options')){$upd['is_sticky']=(int)$data['is_sticky'];$fmt[]='%d';}
		if(isset($data['is_closed'])&&current_user_can('manage_options')){$upd['is_closed']=(int)$data['is_closed'];$fmt[]='%d';}
		$upd['updated_at']=current_time('mysql');$fmt[]='%s';
		return $wpdb->update($t,$upd,['id'=>$topicId],$fmt,['%d'])!==false;
	}
	...
	public static function deleteTopic(int $topicId, int $userId): bool {
		...
		$wpdb->delete($r,['topic_id'=>$topicId],['%d']);
		$result=$wpdb->delete($t,['id'=>$topicId],['%d'])!==false;
		if($result){self::updateForumStats((int)$topic['forum_id']);}
		return $result;
	}
	...
	public static function createReply(int $topicId, int $userId, string $content): int {
		global $wpdb;
		$topic=self::getTopic($topicId);
		if(!$topic||$topic['is_closed']){return 0;}
	...
	public static function updateReply(int $replyId, int $userId, string $content): bool {
		...
		if(!$reply||(int)$reply['user_id']!==$userId&&!current_user_can('manage_options')){return false;}
		return $wpdb->update($t,['content'=>wp_kses_post($content),'updated_at'=>current_time('mysql')],['id'=>$replyId],['%s','%s'],['%d'])!==false;
	}
	...
	public static function deleteReply(int $replyId, int $userId): bool {
		...
		$reply=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$t} WHERE id=%d",$replyId),ARRAY_A);
		if(!$reply||(int)$reply['user_id']!==$userId&&!current_user_can('manage_options')){return false;}
		$result=$wpdb->delete($t,['id'=>$replyId],['%d'])!==false;
		if($result){self::recountTopicReplies((int)$reply['topic_id']);}
		return $result;
	}
	...
	public static function subscribeTopic(int $topicId, int $userId): bool {
		...
		if(self::isSubscribed($topicId,$userId)){return true;}
		return $wpdb->insert($t,['topic_id'=>$topicId,'user_id'=>$userId,'created_at'=>current_time('mysql')],['%d','%d','%s'])!==false;
	}

	public static function unsubscribeTopic(int $topicId, int $userId): bool {
		global $wpdb;
		$t=$wpdb->prefix.'apollo_forum_subscriptions';
		return $wpdb->delete($t,['topic_id'=>$topicId,'user_id'=>$userId],['%d','%d'])!==false;
	}
	...
	public static function getTopicSubscribers(int $topicId): array {
		global $wpdb;
		$t=$wpdb->prefix.'apollo_forum_subscriptions';
		return $wpdb->get_col($wpdb->prepare("SELECT user_id FROM {$t} WHERE topic_id=%d",$topicId))??[];
	}
	...
	private static function recountTopicReplies(int $topicId): void {
		...
		$wpdb->update($t,['reply_count'=>$count],['id'=>$topicId],['%d'],['%d']);
	}
	...
	private static function updateForumStats(int $forumId): void {
		...
		$wpdb->update($f,['topic_count'=>$count,'updated_at'=>current_time('mysql')],['id'=>$forumId],['%d','%s'],['%d']);
	}
}
