...
final class EmailQueueRepository {
	...
	public static function process( int $batchSize = 20 ): array {
		...
		foreach ( $emails as $email ) {
			$meta    = json_decode( $email['meta'] ?? '', true ) ?: array();
	...
	public static function cleanup( int $daysOld = 30 ): int {
		...
		return (int) $wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}" . self::TABLE . " WHERE status='sent' AND sent_at<DATE_SUB(NOW(),INTERVAL %d DAY)",
				$daysOld
			)
		);
	}
	...
	public static function countByStatus(): array {
		global $wpdb;
		$rows   = $wpdb->get_results( "SELECT status,COUNT(*) as cnt FROM {$wpdb->prefix}" . self::TABLE . ' GROUP BY status', ARRAY_A ) ?: array();
		$result = array();
		foreach ( $rows as $r ) {
