...
final class MediaOffloadRepository {
	...
	public static function register( int $attachmentId, string $provider, string $remoteUrl, array $meta = array() ): bool {
		global $wpdb;
		return $wpdb->replace(
	...
	public static function markUnsync( int $attachmentId ): bool {
		global $wpdb;
		return $wpdb->update( $wpdb->prefix . self::TABLE, array( 'is_synced' => 0 ), array( 'attachment_id' => $attachmentId ) ) !== false;
	}

	public static function delete( int $attachmentId ): bool {
		global $wpdb;
		return $wpdb->delete( $wpdb->prefix . self::TABLE, array( 'attachment_id' => $attachmentId ) ) !== false;
	}
