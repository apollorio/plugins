...
final class BlockListRepository {
	...
	public static function block(int $userId, int $blockedId, string $reason=''): bool {
		...
		$r=$wpdb->insert($t,['user_id'=>$userId,'blocked_id'=>$blockedId,'reason'=>$reason],['%d','%d','%s']);
		if($r){
	...
	public static function unblock(int $userId, int $blockedId): bool {
		global $wpdb;
		$r=$wpdb->delete($wpdb->prefix.self::TABLE,['user_id'=>$userId,'blocked_id'=>$blockedId],['%d','%d']);
		if($r){do_action('apollo_user_unblocked',$userId,$blockedId);}
		return (bool)$r;
	}
	...
	public static function canViewProfile(int $userId, int $targetId): bool {
		if(self::isBlocked($targetId,$userId)){return false;}
		return true;
	}
	...
	public static function reportBlock(int $userId, int $blockedId, string $reason, array $evidence=[]): void {
		global $wpdb;
