...
final class MemberTypesRepository {
	...
	public static function get(int $id): ?array {
		...
		if($r)$r['settings']=json_decode($r['settings']??'',true);
		return $r?:null;
	}
	...
	public static function getAll(bool $selectableOnly=false): array {
		...
		foreach($rows as &$r){$r['settings']=json_decode($r['settings']??'',true);}
		return $rows;
	}
	...
	public static function countByType(int $typeId): int {
		...
		return (int)$wpdb->get_var($wpdb->prepare(
			"SELECT COUNT(*) FROM {$wpdb->usermeta} WHERE meta_key='apollo_member_type' AND meta_value=%s",(string)$typeId
		));
	}
	...
	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		foreach(['name','slug','description','icon','color','badge_image','is_selectable','sort_order'] as $k){
	...
	public static function delete(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->usermeta,['meta_key'=>'apollo_member_type','meta_value'=>(string)$id]);
