...
final class UserSettingsRepository {
	...
	public static function get(int $userId,string $key,mixed $default=null): mixed {
		...
		$val=$wpdb->get_var($wpdb->prepare(
			"SELECT setting_value FROM {$wpdb->prefix}".self::TABLE." WHERE user_id=%d AND setting_key=%s",$userId,$key
		));
	...
	public static function getAll(int $userId): array {
		...
		$rows=$wpdb->get_results($wpdb->prepare(
			"SELECT setting_key,setting_value FROM {$wpdb->prefix}".self::TABLE." WHERE user_id=%d",$userId
		),ARRAY_A)?:[];
		$result=[];
		foreach($rows as $r){$result[$r['setting_key']]=maybe_unserialize($r['setting_value']);}
		return $result;
	}

	public static function delete(int $userId,string $key): bool {
		global $wpdb;
		return $wpdb->delete($wpdb->prefix.self::TABLE,['user_id'=>$userId,'setting_key'=>$key])!==false;
	}
	...
	public static function setMultiple(int $userId,array $settings): bool {
		foreach($settings as $key=>$value){
			if(!self::set($userId,$key,$value))return false;
		}
		return true;
	}
