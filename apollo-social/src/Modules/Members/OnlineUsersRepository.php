...
final class OnlineUsersRepository {
	...
	public static function getOnlineUserIds(): array {
		...
		return $wpdb->get_col(
			$wpdb->prepare(
				"SELECT user_id FROM {$wpdb->prefix}" . self::TABLE . ' WHERE last_activity>DATE_SUB(NOW(),INTERVAL %d MINUTE)',
				$threshold
			)
		) ?: array();
	}
	...
	public static function countOnline(): int {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$wpdb->prefix}" . self::TABLE . ' WHERE last_activity>DATE_SUB(NOW(),INTERVAL %d MINUTE)',
				$threshold
			)
		);
	}
	...
	public static function cleanup( int $olderThanMinutes = 1440 ): int {
		...
		return (int) $wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}" . self::TABLE . ' WHERE last_activity<DATE_SUB(NOW(),INTERVAL %d MINUTE)',
				$olderThanMinutes
			)
		);
	}
