...
final class ProfileFields {
	...
	public static function calculateCompleteness( int $user_id ): int {
		...
		++$total;
		if ( get_avatar_url( $user_id ) && ! str_contains( get_avatar_url( $user_id ), 'gravatar' ) ) {
			++$filled;
		}
		++$total;
	...
	public static function renderField( string $field, int $user_id = 0, array $attrs = array() ): string {
		$fields = self::getFields();
		if ( ! isset( $fields[ $field ] ) ) {
			return '';
		}
