...
final class VerifiedUsersRepository {
	...
	public static function verify( int $userId, ?string $reason = null, ?string $badge = null ): bool {
		...
		if ( $reason ) {
			update_user_meta( $userId, 'apollo_verified_reason', $reason );
		}
		if ( $badge ) {
			update_user_meta( $userId, 'apollo_verified_badge', $badge );
		}
	...
	public static function getVerifiedUserIds(): array {
		global $wpdb;
		return $wpdb->get_col( "SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key='apollo_verified' AND meta_value='1'" ) ?: array();
	}
}
