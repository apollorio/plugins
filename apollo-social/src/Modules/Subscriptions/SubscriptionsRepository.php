...
final class SubscriptionsRepository {
	...
	public static function updatePlan( int $planId, array $data ): bool {
		...
		if ( isset( $data['duration_days'] ) ) {
			$upd['duration_days'] = (int) $data['duration_days'];
			$fmt[]                = '%d';}
		if ( isset( $data['features'] ) ) {
			$upd['features'] = wp_json_encode( $data['features'] );
			$fmt[]           = '%s';}
		if ( isset( $data['limits'] ) ) {
			$upd['limits'] = wp_json_encode( $data['limits'] );
			$fmt[]         = '%s';}
		if ( isset( $data['is_active'] ) ) {
			$upd['is_active'] = (int) $data['is_active'];
			$fmt[]            = '%d';}
		return $wpdb->update( $t, $upd, array( 'id' => $planId ), $fmt, array( '%d' ) ) !== false;
	}
	...
	public static function getPlans( bool $activeOnly = true ): array {
		...
		$rows  = $wpdb->get_results( "SELECT * FROM {$t} {$where} ORDER BY sort_order ASC, price ASC", ARRAY_A ) ?? array();
	...
	public static function processExpiring(): int {
		...
		foreach ( $expiring as $sub ) {
			...
			$wpdb->update( $t, array( 'renewal_notified' => 1 ), array( 'id' => (int) $sub['id'] ), array( '%d' ), array( '%d' ) );
			++$count;
		}
		return $count;
	}
	...
	public static function processExpired(): int {
		...
		foreach ( $expired as $sub ) {
			if ( $sub['auto_renew'] ) {
				...
				$wpdb->update( $t, array( 'status' => 'expired' ), array( 'id' => (int) $sub['id'] ), array( '%s' ), array( '%d' ) );
	...
	public static function getStats(): array {
		...
		$monthlyRevenue = (float) $wpdb->get_var(
			$wpdb->prepare(
	...
	private static function decodePlan( array $row ): array {
		...
		$row['limits']   = json_decode( $row['limits'] ?? '{}', true ) ?? array();
		return $row;
	}
