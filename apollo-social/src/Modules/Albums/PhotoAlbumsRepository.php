...
final class PhotoAlbumsRepository {
	...
	public static function update(int $albumId, int $userId, array $data): bool {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		if(!self::isOwner($albumId,$userId)){return false;}
		$upd=[];$fmt=[];
		...
		if(isset($data['privacy'])){$upd['privacy']=$data['privacy'];$fmt[]='%s';}
		if(isset($data['cover_photo_id'])){$upd['cover_photo_id']=(int)$data['cover_photo_id'];$fmt[]='%d';}
		$upd['updated_at']=current_time('mysql');$fmt[]='%s';
		return $wpdb->update($t,$upd,['id'=>$albumId],$fmt,['%d'])!==false;
	}
	...
	public static function delete(int $albumId, int $userId): bool {
		...
		$wpdb->delete($p,['album_id'=>$albumId],['%d']);
		return $wpdb->delete($t,['id'=>$albumId],['%d'])!==false;
	}
	...
	public static function addPhoto(int $albumId, int $userId, int $attachmentId, string $caption=''): int {
		global $wpdb;
		if(!self::isOwner($albumId,$userId)&&!self::canContribute($albumId,$userId)){return 0;}
		...
		if($photoId&&!self::hasCover($albumId)){self::setCover($albumId,$photoId);}
		return $photoId;
	}
	...
	public static function removePhoto(int $photoId, int $userId): bool {
		...
		if((int)$photo['user_id']!==$userId&&!self::isOwner((int)$photo['album_id'],$userId)){return false;}
		$wpdb->delete($t,['id'=>$photoId],['%d']);
	...
	public static function updatePhotoCaption(int $photoId, int $userId, string $caption): bool {
		...
		$t=$wpdb->prefix.self::PHOTOS;
		$photo=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$t} WHERE id=%d",$photoId),ARRAY_A);
		if(!$photo||(int)$photo['user_id']!==$userId){return false;}
		return $wpdb->update($t,['caption'=>sanitize_text_field($caption)],['id'=>$photoId],['%s'],['%d'])!==false;
	}
	...
	public static function reorderPhotos(int $albumId, int $userId, array $photoIds): bool {
		...
		foreach($photoIds as $order=>$photoId){
			$wpdb->update($t,['sort_order'=>$order],['id'=>(int)$photoId,'album_id'=>$albumId],['%d'],['%d','%d']);
		}
		return true;
	}
	...
	public static function setCover(int $albumId, int $photoId): bool {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		return $wpdb->update($t,['cover_photo_id'=>$photoId],['id'=>$albumId],['%d'],['%d'])!==false;
	}

	public static function likePhoto(int $photoId, int $userId): bool {
		global $wpdb;
		$t=$wpdb->prefix.'apollo_photo_likes';
		if(self::hasLikedPhoto($photoId,$userId)){return true;}
		return $wpdb->insert($t,['photo_id'=>$photoId,'user_id'=>$userId,'created_at'=>current_time('mysql')],['%d','%d','%s'])!==false;
	}

	public static function unlikePhoto(int $photoId, int $userId): bool {
		global $wpdb;
		$t=$wpdb->prefix.'apollo_photo_likes';
		return $wpdb->delete($t,['photo_id'=>$photoId,'user_id'=>$userId],['%d','%d'])!==false;
	}
	...
	public static function canView(int $albumId, int $viewerId): bool {
		...
		if((int)$album['user_id']===$viewerId){return true;}
	...
	private static function updateAlbumTimestamp(int $albumId): void {
		global $wpdb;
		$t=$wpdb->prefix.self::TABLE;
		$wpdb->update($t,['updated_at'=>current_time('mysql')],['id'=>$albumId],['%s'],['%d']);
	}
