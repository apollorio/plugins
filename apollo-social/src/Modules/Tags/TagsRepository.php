...
final class TagsRepository {
	...
	public static function getAll(bool $includeSystem=true): array {
		...
		return $wpdb->get_results("SELECT * FROM {$wpdb->prefix}".self::TABLE." {$w} ORDER BY name",ARRAY_A)?:[];
	}

	public static function update(int $id,array $d): bool {
		global $wpdb;
		$u=[];
		if(isset($d['name']))$u['name']=sanitize_text_field($d['name']);
		if(isset($d['slug']))$u['slug']=sanitize_title($d['slug']);
		if(isset($d['color']))$u['color']=sanitize_hex_color($d['color']);
		if(isset($d['icon']))$u['icon']=sanitize_text_field($d['icon']);
		if(isset($d['description']))$u['description']=sanitize_textarea_field($d['description']);
	...
	public static function delete(int $id): bool {
		global $wpdb;
		$wpdb->delete($wpdb->prefix.self::REL_TABLE,['tag_id'=>$id]);
	...
	public static function assignToUser(int $userId,int $tagId,?int $assignedBy=null): bool {
		global $wpdb;
		$r=$wpdb->replace($wpdb->prefix.self::REL_TABLE,[
			'user_id'=>$userId,'tag_id'=>$tagId,'assigned_by'=>$assignedBy?:get_current_user_id()
		]);
		return $r!==false;
	}
	...
	public static function removeFromUser(int $userId,int $tagId): bool {
		global $wpdb;
		return $wpdb->delete($wpdb->prefix.self::REL_TABLE,['user_id'=>$userId,'tag_id'=>$tagId])!==false;
	}
	...
	public static function getUserTags(int $userId): array {
		...
		return $wpdb->get_results($wpdb->prepare(
			"SELECT t.* FROM {$wpdb->prefix}".self::TABLE." t
			 ...
			 WHERE r.user_id=%d ORDER BY t.name",$userId
		),ARRAY_A)?:[];
	}
	...
	public static function countUsersByTag(int $tagId): int {
		...
		return (int)$wpdb->get_var($wpdb->prepare(
			"SELECT COUNT(*) FROM {$wpdb->prefix}".self::REL_TABLE." WHERE tag_id=%d",$tagId
		));
	}
}
