...
class Apollo_Plano_Save_Handler {
	...
	public static function init() {
		add_action( 'wp_ajax_apollo_save_canvas', [ __CLASS__, 'handle_save_canvas' ] );
		add_action( 'wp_ajax_nopriv_apollo_save_canvas', [ __CLASS__, 'handle_save_canvas' ] );
	}
	...
	public static function handle_save_canvas() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'wp_rest' ) ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Nonce inválido', 'apollo-social' ) ] );
			return;
		}

		// Check user is logged in
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Você precisa estar logado', 'apollo-social' ) ] );
			return;
		}

		// Get data URL
		if ( ! isset( $_POST['data_url'] ) ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Data URL não fornecida', 'apollo-social' ) ] );
			return;
		}
		...
		if ( ! preg_match( '/^data:image\/(png|jpeg|jpg);base64,/', $data_url ) ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Formato de imagem inválido', 'apollo-social' ) ] );
			return;
		}
		...
		if ( empty( $matches[2] ) ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Erro ao processar imagem', 'apollo-social' ) ] );
			return;
		}
		...
		// Validate image data
		if ( $image_data === false ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Erro ao decodificar imagem', 'apollo-social' ) ] );
			return;
		}
		...
		if ( $upload['error'] ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Erro ao fazer upload: ', 'apollo-social' ) . $upload['error'] ] );
			return;
		}
		...
		if ( is_wp_error( $attach_id ) ) {
			wp_send_json_error( [ 'message' => esc_html__( 'Erro ao criar attachment', 'apollo-social' ) ] );
			return;
		}
