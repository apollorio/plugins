...
function apollo_ajax_save_spreadsheet() {
	...
	if ( ! wp_verify_nonce( $_POST['nonce'] ?? '', 'apollo_spreadsheet_save' ) ) {
		wp_send_json_error( array( 'message' => __( 'Nonce inválido.', 'apollo-social' ) ), 403 );
	...
	if ( ! current_user_can( 'edit_posts' ) ) {
		wp_send_json_error( array( 'message' => __( 'Sem permissão.', 'apollo-social' ) ), 403 );
	...
	if ( $post_id < 1 ) {
		wp_send_json_error( array( 'message' => __( 'ID inválido.', 'apollo-social' ) ), 400 );
	...
	if ( json_last_error() !== JSON_ERROR_NONE ) {
		wp_send_json_error( array( 'message' => __( 'JSON inválido.', 'apollo-social' ) ), 400 );
	...
	if ( apollo_save_spreadsheet( $post_id, $data ) ) {
		wp_send_json_success(
			array(
				'message' => __( 'Salvo com sucesso.', 'apollo-social' ),
				'post_id' => $post_id,
			)
		);
	} else {
		wp_send_json_error( array( 'message' => __( 'Erro ao salvar.', 'apollo-social' ) ), 500 );
	}
}
