...
<div class="app">
	...
	<div class="main-content">
		...
		';
		wp_add_inline_style('apollo-inline', $nucleo_css);
	},
	10
);
...
?>
		...
		<div class="flex items-center justify-between">
			...
			<?php if ( $avatar_url ) : ?>
			<img src="<?php echo esc_url( $avatar_url ); ?>" alt="<?php echo esc_attr( $title ); ?>" class="h-9 w-9 rounded-lg object-cover">
			<?php else : ?>
			...
			<?php else : ?>
			<button class="px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors">
				<i class="ri-add-line mr-2"></i>Participar
			</button>
			<?php endif; ?>
			</div>
		</div>
		...
		<div class="w-full max-w-4xl mx-auto">
			...
			<?php else : ?>
				<?php foreach ( $nucleo_posts as $post_item ) : ?>
		...
		<div class="w-full max-w-6xl mx-auto flex flex-col lg:flex-row gap-5 lg:gap-7">
			...
			<div class="bg-white/95 border border-slate-200 rounded-2xl px-4 py-3 md:px-5 md:py-4 shadow-sm">
				...
				<div class="flex-1">
					...
					placeholder="<?php echo esc_attr( sprintf( __( 'Compartilhe uma atualização do %s...', 'apollo-social' ), $title ) ); ?>"
		...
		btn.addEventListener('click', async function() {
			...
			try {
				...
				if (data.success) {
					...
					this.innerHTML = icon.outerHTML + ' ' + (data.like_count || 0);
				}
			} catch (e) {
				console.error(e);
			}
