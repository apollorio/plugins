...
<div class="app">
	...
	<!-- ====================================================================
		[MAIN CONTENT] Single Comunidade
		==================================================================== -->
	<div class="main-content">
		...
		';
		wp_add_inline_style('apollo-inline', $comunidade_css);
	},
	10
);
...
?>
		...
		<div class="flex items-center justify-between">
			...
			<?php if ( $avatar_url ) : ?>
			<img src="<?php echo esc_url( $avatar_url ); ?>" alt="<?php echo esc_attr( $group_title ); ?>" class="h-9 w-9 rounded-lg object-cover">
			<?php else : ?>
			...
			<div class="flex flex-col leading-tight">
				<h1 class="text-[15px] font-extrabold text-slate-900"><?php echo esc_html( $group_title ); ?></h1>
				<p class="text-[12px] text-slate-500"><?php echo esc_html( $subtitle ); ?> · <?php echo intval( $members_count ); ?> membros</p>
			</div>
			</div>

			<div class="hidden md:flex items-center gap-3">
			<?php if ( $is_member ) : ?>
			<button class="px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors">
				<i class="ri-check-line mr-2"></i>Membro
			</button>
			<?php else : ?>
			<button class="px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors">
				<i class="ri-add-line mr-2"></i>Entrar
			</button>
			<?php endif; ?>
			</div>
		</div>
		...
		<a href="<?php echo esc_url( home_url( '/nucleos/' ) ); ?>" data-ap-tooltip="Ver núcleos">
			...
			<div class="md:hidden">
			<?php if ( $is_member ) : ?>
			<button class="px-3 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg">
		...
		<div class="w-full max-w-4xl mx-auto">
			...
			<div class="space-y-4">
			<?php if ( empty( $posts ) ) : ?>
			<div class="bg-white/95 border border-slate-200 rounded-2xl px-6 py-8 text-center">
				<i class="ri-chat-1-line text-4xl text-slate-300 mb-3"></i>
				<p class="text-slate-500">Nenhum post ainda. Seja o primeiro a compartilhar algo!</p>
			</div>
			<?php else : ?>
				<?php foreach ( $posts as $post_item ) : ?>
			<article class="bg-white/95 border border-slate-200 rounded-2xl px-4 py-4 shadow-sm">
				<div class="text-slate-700">
					<?php echo wp_kses_post( get_the_content( null, false, $post_item->ID ) ); ?>
				</div>
			</article>
			<?php endforeach; ?>
			<?php endif; ?>
...
<?php get_footer(); ?>
			...
			<div class="bg-white/95 border border-slate-200 rounded-2xl px-4 py-3 md:px-5 md:py-4 shadow-sm">
				...
				<div class="flex-1">
					...
					placeholder="<?php echo esc_attr( sprintf( __( 'Compartilhe algo com a %s...', 'apollo-social' ), $group_title ) ); ?>"
		...
		<div class="max-w-2xl mx-auto w-full px-4 py-2 flex items-end justify-between h-[60px]">
			<a href="<?php echo esc_url( home_url( '/mural/' ) ); ?>" class="nav-btn w-14 pb-1">
...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.querySelectorAll('[data-action="publish-post"]').forEach(btn => {
		btn.addEventListener('click', async function() {
			...
			if (!content?.trim()) {
				alert('<?php echo esc_js( __( 'Digite algo para publicar', 'apollo-social' ) ); ?>');
				return;
			}
			try {
