		...
		<div class="apollo-modal-content">
			...
			<div class="apollo-modal-body">
				...
				<textarea id="apollo-rejection-reason-<?php echo esc_attr( $group_id ); ?>"
							...
							placeholder="<?php esc_attr_e( 'Explique o motivo da rejeição...', 'apollo-social' ); ?>"
							rows="4"></textarea>
...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.querySelectorAll('.apollo-confirm-reject').forEach(btn => {
		btn.addEventListener('click', function() {
			...
			if (!reason) {
				alert('<?php echo esc_js( __( 'Por favor, informe o motivo da rejeição.', 'apollo-social' ) ); ?>');
				return;
			}
...
function apolloModerateGroup(groupId, action, reason = '') {
	...
	if (action === 'approve') {
		alert('<?php echo esc_js( __( 'Grupo aprovado com sucesso!', 'apollo-social' ) ); ?>');
	} else {
		alert('<?php echo esc_js( __( 'Grupo rejeitado com sucesso!', 'apollo-social' ) ); ?>');
		// Close modal
