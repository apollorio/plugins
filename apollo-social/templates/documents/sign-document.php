...
add_action(
	...
	function () {
		...
		$sign_doc_css = '
			:root {
				--font-primary: var(--ap-font-sans, "Roboto", Roboto, system-ui, sans-serif);
				--bg-main: var(--ap-bg, #ffffff);
				--text-main: var(--ap-text, rgba(19, 21, 23, .6));
				--text-primary: var(--ap-text-dark, rgba(19, 21, 23, .85));
				--border-color-2: var(--ap-border, #e5e7eb);
			}
			body.dark-mode {
				--bg-main: var(--ap-bg-dark, #131517);
				--text-main: var(--ap-text-light, #ffffff91);
				--text-primary: var(--ap-text-light, #fdfdfdfa);
				--border-color-2: var(--ap-border-dark, #374151);
			}
		...
		';
		wp_add_inline_style( 'apollo-inline', $sign_doc_css );
	},
	10
);
