 ...
 * Public users see public documents; logged-in users see all + their private docs
 */
...
// Check if user is logged in - redirect to login if not and filter is "my"
$is_logged_in   = is_user_logged_in();
$current_filter = isset($_GET['filter']) ? sanitize_text_field($_GET['filter']) : 'all';

if (! $is_logged_in && in_array($current_filter, array('my', 'signed'), true)) {
	wp_safe_redirect(home_url('/login/?redirect_to=' . urlencode(home_url('/documentos/'))));
	exit;
}
...
// $current_filter already set above
			...
			<div class="flex items-center justify-between">
				<div>
					<h1 class="text-2xl font-bold text-foreground">
						...
						switch ($current_filter) {
							case 'my':
								echo esc_html__('Meus Documentos', 'apollo-social');

								break;
							case 'signed':
								echo esc_html__('Documentos Assinados', 'apollo-social');

								break;
							default:
								echo esc_html__('Todos os Documentos', 'apollo-social');
						}
						?>
					</h1>
				...
				<div class="flex items-center gap-2">
					<div class="relative">
						...
						<input
							type="text"
							placeholder="<?php esc_attr_e('Buscar documentos...', 'apollo-social'); ?>"
							class="input pl-10 w-64"
							id="searchDocuments">
					</div>
					<button class="btn btn-secondary">
						<i class="ri-filter-line mr-2"></i>
						<?php esc_html_e('Filtros', 'apollo-social'); ?>
					</button>
				</div>
			</div>
		</header>
