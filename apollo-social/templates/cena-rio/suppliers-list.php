...
<head>
	...
	<style>
		:root {
			...
			--ap-text-primary: var(--ap-text, rgba(19, 21, 23, 0.65));
			--ap-text-secondary: var(--ap-text-dark, rgba(15, 23, 42, 0.88));
			--ap-text-muted: var(--ap-text-muted, rgba(19, 21, 23, 0.4));
			--ap-orange-500: var(--ap-orange, #FF6925);
			--ap-orange-600: var(--ap-orange-dark, #E55A1E);
			--nav-height: 70px;
		}
...
<body class="apollo-canvas" <?php echo $open_supplier_id > 0 ? 'data-open-supplier="' . esc_attr( $open_supplier_id ) . '"' : ''; ?>>
	...
	<nav class="navbar">
		...
		<div class="flex items-center gap-2">
			...
			<button type="button" class="nav-btn" data-apollo-notif-trigger aria-label="Notificações">
	...
	(function() {
		...
		document.getElementById('modalShare').addEventListener('click', () => {
			if (navigator.share) {
				...
				alert('<?php echo esc_js( __( 'Link copiado!', 'apollo-social' ) ); ?>');
			}
		});
