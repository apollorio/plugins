...
$conversation_nonce_action = 'apollo_social_chat_conversation';
$conversation_id           = null;

if (isset($_GET['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), $conversation_nonce_action) && isset($_GET['conversation'])) {
	$conversation_id = absint(wp_unslash($_GET['conversation']));
}

if ($conversation_id) {
	$selected_conversation = $conversation_id;
} else {
	$selected_conversation = ! empty($conversations) ? $conversations[0]['id'] : null;
}

// Canvas mode - don't call get_header/footer.
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<head>
	...
	<title>Mensagens - Apollo::rio</title>
	<?php wp_head(); ?>
	<style>
		:root {
			--apollo-viewport-height: var(--ap-viewport-height, 100vh);
		}

		@supports (height: 100dvh) {
			:root {
				--apollo-viewport-height: var(--ap-viewport-height, 100dvh);
			}
		}
		...
		.apollo-chat-viewport {
			height: var(--apollo-viewport-height);
			min-height: var(--apollo-viewport-height);
		}

		.apollo-chat-viewport .sidebar {
			height: var(--apollo-viewport-height);
		}
	</style>
</head>
...
<body class="apollo-canvas-mode">
	...
	<div class="flex w-full overflow-hidden bg-background apollo-chat-viewport">
		...
		<aside class="sidebar border-r border-border bg-card w-80 flex-shrink-0 flex flex-col">
			...
			<div class="p-4 border-b border-border">
				<div class="relative">
					...
					<label for="searchConversations" class="sr-only">Buscar conversas</label>
					<input
						type="text"
				...
				<?php else : ?>
					<div class="space-y-1 p-2">
						<?php foreach ($conversations as $conv) : ?>
							<a
								href="<?php echo esc_url(wp_nonce_url(add_query_arg('conversation', $conv['id']), $conversation_nonce_action)); ?>"
								...
								<img src="<?php echo esc_url($conv['avatar']); ?>" alt="<?php echo esc_attr('Avatar de ' . $conv['name']); ?>" class="rounded-full">
								</div>
			...
			<?php if ($selected_conversation) : ?>
				...
				<?php if ($current_conv) : ?>
					...
					<div class="border-t border-border bg-card p-4">
						<div class="flex items-center gap-2">
							<button class="btn btn-ghost btn-sm" title="Anexar arquivo" aria-label="Anexar arquivo">
								<i class="ri-attachment-line"></i>
							</button>
							<label for="messageInput" class="sr-only">Digite uma mensagem</label>
							<input
								type="text"
							...
							<button class="btn btn-primary btn-sm" title="Enviar mensagem" aria-label="Enviar mensagem">
				...
				<?php else : ?>
					...
					<div class="flex-1 flex items-center justify-center">
						<div class="text-center">
							<i class="ri-error-warning-line text-6xl text-muted-foreground mb-4"></i>
							<h3 class="text-xl font-semibold text-foreground mb-2">Conversa não encontrada</h3>
							<p class="text-muted-foreground">A conversa selecionada não existe.</p>
						</div>
					</div>
				<?php endif; ?>
			<?php else : ?>
