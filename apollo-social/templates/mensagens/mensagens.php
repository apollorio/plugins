 ...
 *
 * STRICT MODE: base.js from CDN handles all core assets (uni.css, normalize.css, icons)
 *
...
// Enqueue only additional assets not provided by base.js
add_action(
	'wp_enqueue_scripts',
	function () {
		// Ensure base assets are loaded (uni.css, normalize.css via base.js)
		if ( function_exists( 'apollo_ensure_base_assets' ) ) {
			apollo_ensure_base_assets();
		}

		// Motion.dev for animations (not included in base.js)
		wp_enqueue_script(
			'motion-one',
...
<head>
	...
	<!-- Apollo CDN - Core JS (Navbar, Clock, Modals) -->
	...
	<style>
		...
		body {
			...
			max-height: var(--apollo-viewport-height);
			margin: 0;
			overflow: hidden;
		}
		...
		@media (min-width: 768px) {
			.apollo-chat-panel {
				height: calc(var(--apollo-viewport-height) - 6.5rem);
			}
		}
	</style>
</head>
...
<section class="aprioEXP-body">
	...
	<header class="h-14 flex items-center justify-between border-b bg-white/90 backdrop-blur px-3 md:px-6">
		...
		<div class="hidden md:flex items-center gap-2">
			...
			<input
				type="text"
				placeholder="<?php esc_attr_e( 'Buscar conversas...', 'apollo-social' ); ?>"
			...
			<div class="border-t border-slate-200 px-3 md:px-4 py-2.5 bg-white">
				...
				<div class="flex-1 min-w-0">
					...
					<textarea
						...
						placeholder="<?php esc_attr_e( 'Escreva uma mensagem para a cena...', 'apollo-social' ); ?>"
					></textarea>
