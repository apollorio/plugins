...
class Apollo_Events_Bookmarks {
	...
	private function init_hooks() {
		// AJAX handlers
		add_action( 'wp_ajax_apollo_toggle_bookmark', array( $this, 'ajax_toggle_bookmark' ) );
		add_action( 'wp_ajax_nopriv_apollo_toggle_bookmark', array( $this, 'ajax_toggle_bookmark' ) );

		// REST API endpoints
		add_action( 'rest_api_init', array( $this, 'register_rest_routes' ) );

		// Shortcode
		add_shortcode( 'apollo_bookmarks', array( $this, 'bookmarks_shortcode' ) );
	...
	public function ajax_toggle_bookmark() {
		...
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => 'Você precisa estar logado para favoritar eventos' ) );
			return;
		}
		...
		if ( $action ) {
			...
			wp_send_json_error( array( 'message' => 'Erro ao atualizar favorito' ) );
		}
	}
	...
	public function rest_toggle_bookmark( $request ) {
		...
		return new WP_Error( 'toggle_failed', 'Erro ao atualizar favorito', array( 'status' => 500 ) );
	}
