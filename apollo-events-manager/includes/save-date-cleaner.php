...
function apollo_cleanup_duplicate_save_dates($post_id, $post, $update) {
    ...
    if (!post_type_exists('save_date')) {
        return; // Save-date CPT not registered
    }
    ...
    if (empty($save_dates)) {
        return; // No save-dates with this date
    }
    ...
    foreach ($save_dates as $save_date) {
        ...
        if ($percent >= 80) {
            $deleted = wp_delete_post($save_date->ID, true); // Force delete (skip trash)
...
function apollo_manual_save_date_cleanup() {
    ...
    return array(
        'success' => true,
        'message' => sprintf(
            __('Checked %d events, deleted %d duplicate save-dates.', 'apollo-events-manager'),
            $checked_count,
            $deleted_count
        ),
