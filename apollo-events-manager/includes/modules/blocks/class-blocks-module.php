...
class Blocks_Module extends Abstract_Module {
    ...
    public function register_blocks(): void {
        // Event List Block.
        register_block_type( 'apollo-events/event-list', array(
        ...
        // Single Event Block.
        register_block_type( 'apollo-events/single-event', array(
            'editor_script'   => 'apollo-blocks-editor',
            'editor_style'    => 'apollo-blocks-editor',
            'style'           => 'apollo-blocks',
        ...
        register_block_type( 'apollo-events/countdown', array(
            'editor_script'   => 'apollo-blocks-editor',
            'editor_style'    => 'apollo-blocks-editor',
            'style'           => 'apollo-blocks',
        ...
        register_block_type( 'apollo-events/calendar', array(
            'editor_script'   => 'apollo-blocks-editor',
            'editor_style'    => 'apollo-blocks-editor',
            'style'           => 'apollo-blocks',
        ...
        // DJ Grid Block.
        register_block_type( 'apollo-events/dj-grid', array(
            'editor_script'   => 'apollo-blocks-editor',
            'editor_style'    => 'apollo-blocks-editor',
            'style'           => 'apollo-blocks',
        ...
        // Local Grid Block.
        register_block_type( 'apollo-events/local-grid', array(
            'editor_script'   => 'apollo-blocks-editor',
            'editor_style'    => 'apollo-blocks-editor',
            'style'           => 'apollo-blocks',
        ...
        // Event Search Block.
        register_block_type( 'apollo-events/search', array(
            'editor_script'   => 'apollo-blocks-editor',
            'editor_style'    => 'apollo-blocks-editor',
            'style'           => 'apollo-blocks',
    ...
    public function enqueue_editor_assets(): void {
        ...
        // Pass data to editor.
        wp_localize_script( 'apollo-blocks-editor', 'apolloBlocksData', array(
    ...
    private function render_event_card( int $event_id, array $attributes ): string {
        ...
        $html = '<div class="apollo-event-card">';
        $html .= sprintf( '<a href="%s" class="apollo-event-card__link">', esc_url( get_permalink( $event_id ) ) );

        if ( $show_image && has_post_thumbnail( $event_id ) ) {
            $html .= '<div class="apollo-event-card__image">';
            $html .= get_the_post_thumbnail( $event_id, 'medium_large' );
            $html .= '</div>';
        }

        $html .= '<div class="apollo-event-card__content">';
        $html .= '<h3 class="apollo-event-card__title">' . esc_html( $event->post_title ) . '</h3>';

        if ( $show_date ) {
            $start_date = get_post_meta( $event_id, '_event_start_date', true );
            if ( $start_date ) {
                $html .= '<p class="apollo-event-card__date">';
                $html .= '<span class="dashicons dashicons-calendar-alt"></span>';
                $html .= wp_date( 'd/m/Y H:i', strtotime( $start_date ) );
                $html .= '</p>';
            }
        }
        ...
        if ( $show_local ) {
            ...
            if ( is_array( $local_ids ) && ! empty( $local_ids ) ) {
                ...
                if ( $local ) {
                    $html .= '<p class="apollo-event-card__local">';
                    $html .= '<span class="dashicons dashicons-location"></span>';
                    $html .= esc_html( $local->post_title );
                    $html .= '</p>';
                }
            }
        }
        ...
        if ( $show_djs ) {
            ...
            if ( is_array( $dj_ids ) && ! empty( $dj_ids ) ) {
                ...
                if ( ! empty( $dj_names ) ) {
                    $html .= '<p class="apollo-event-card__djs">';
                    $html .= '<span class="dashicons dashicons-admin-users"></span>';
                    $html .= esc_html( implode( ', ', $dj_names ) );
                    if ( count( $dj_ids ) > 3 ) {
                        $html .= sprintf( ' +%d', count( $dj_ids ) - 3 );
                    }
                    $html .= '</p>';
                }
            }
        ...
        if ( $show_button ) {
            $html .= sprintf(
                '<a href="%s" class="apollo-event-card__button">%s</a>',
                esc_url( get_permalink( $event_id ) ),
                esc_html__( 'Ver Evento', 'apollo-events' )
            );
        }
