...
class Lists_Module extends Abstract_Module {
	...
	public function register_shortcodes(): void {
		add_shortcode( 'apollo_events_grid', array( $this, 'render_grid' ) );
		add_shortcode( 'apollo_events_list', array( $this, 'render_list' ) );
		add_shortcode( 'apollo_events_table', array( $this, 'render_table' ) );
		add_shortcode( 'apollo_events_slider', array( $this, 'render_slider' ) );
		add_shortcode( 'apollo_events_compact', array( $this, 'render_compact' ) );
		add_shortcode( 'apollo_featured_events', array( $this, 'render_featured' ) );
	}
	...
	private function get_event_data( \WP_Post $post ): array {
		...
		if ( $start_date ) {
			...
			if ( $ts ) {
				$months_pt = array( 'jan', 'fev', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez' );
				$parsed    = array(
					'day'     => gmdate( 'd', $ts ),
		...
		return array(
			...
			'local_id'   => $local_id,
			'categories' => wp_get_post_terms( $post->ID, 'event_listing_category', array( 'fields' => 'names' ) ),
		);
	}
