...
function apollo_get_global_event_stats() {
    ...
    $top_events_query = new WP_Query(array(
        'post_type' => 'event_listing',
        'posts_per_page' => 5,
        'post_status' => 'publish',
    ...
    if ($top_events_query->have_posts()) {
        foreach ($top_events_query->posts as $event) {
            $stats['top_events_by_views'][] = array(
                'id' => $event->ID,
    ...
    global $wpdb;
    $location_counts = $wpdb->get_results($wpdb->prepare(
...
function apollo_get_user_event_stats($user_id) {
        ...
        if (!is_wp_error($sounds) && !empty($sounds)) {
            foreach ($sounds as $sound) {
                if (!isset($sounds_counts[$sound->term_id])) {
                    $sounds_counts[$sound->term_id] = array(
                        'name' => $sound->name,
                        'count' => 0
                    );
                }
    ...
    if ($total_sound_events > 0) {
        foreach ($sounds_counts as $term_id => $data) {
            $stats['sounds_distribution'][] = array(
                'name' => $data['name'],
                'count' => $data['count'],
        ...
        usort($stats['sounds_distribution'], function($a, $b) {
    ...
    if ($total_location_events > 0) {
        foreach ($location_counts as $location_name => $count) {
            $stats['locations_distribution'][] = array(
                'name' => $location_name,
                'count' => $count,
        ...
        // Sort by count descending
        usort($stats['locations_distribution'], function($a, $b) {
            return $b['count'] - $a['count'];
        });
    }
