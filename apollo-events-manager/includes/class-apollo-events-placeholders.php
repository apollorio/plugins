...
function apollo_get_placeholder( $key, $default = '' ) {
	...
	$overrides = get_option( 'apollo_events_custom_placeholders', array() );
 ...
 */
function apollo_event_get_placeholder_value( $placeholder_id, $event_id = null, $args = array() ) {
	// Resolve event ID
	if ( $event_id === null ) {
		$event_id = get_the_ID();
	}
...
function apollo_get_event_local_ids( $event_id ) {
	...
	return $local_id > 0 ? array( $local_id ) : array();
}
