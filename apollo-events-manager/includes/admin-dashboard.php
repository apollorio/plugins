...
class Apollo_Admin_Dashboard {
    ...
    public function enqueue_assets($hook) {
        ...
        // Localize script
        wp_localize_script('apollo-admin-dashboard', 'apolloDashboard', array(
    ...
    public function register_rest_routes() {
        register_rest_route('apollo/v1', '/analytics', array(
            'methods' => 'GET',
        ...
        register_rest_route('apollo/v1', '/likes', array(
            'methods' => 'GET',
        ...
        register_rest_route('apollo/v1', '/technotes/(?P<venue_id>\d+)', array(
            'methods' => 'GET',
    ...
    public function rest_post_analytics($request) {
        ...
        return new WP_Error('insert_failed', 'Failed to insert analytics', array('status' => 500));
    }
    ...
    public function rest_get_likes($request) {
        ...
        if ($event_id > 0) {
            ...
            return new WP_REST_Response(array('event_id' => $event_id, 'count' => (int)$count), 200);
        }
    ...
    public function rest_post_like($request) {
        ...
        if ($exists) {
            // Unlike
            $wpdb->delete($table, array('event_id' => $event_id, 'user_id' => $user_id), array('%d', '%d'));
            return new WP_REST_Response(array('success' => true, 'liked' => false), 200);
        }
        ...
        if ($result) {
            return new WP_REST_Response(array('success' => true, 'liked' => true), 201);
        }
        
        return new WP_Error('insert_failed', 'Failed to insert like', array('status' => 500));
    }
    ...
    public function rest_get_technotes($request) {
        ...
        $notes = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$table} WHERE venue_id = %d AND user_id = %d",
            $venue_id, $user_id
        ), ARRAY_A);
        
        return new WP_REST_Response($notes ?: array(), 200);
    }
    ...
    public function rest_post_technotes($request) {
        ...
        if ($exists) {
            $result = $wpdb->update(
                $table,
                array('notes' => $notes),
                array('venue_id' => $venue_id, 'user_id' => $user_id),
                array('%s'),
                array('%d', '%d')
            );
        } else {
        ...
        return new WP_Error('update_failed', 'Failed to save notes', array('status' => 500));
    }
