...
class Apollo_Events_Performance_Optimizer {
     ...
     * Cache group name
     */
    const CACHE_GROUP = 'apollo_events';
    ...
    public static function optimize_query($args) {
        ...
        if (!isset($args['posts_per_page']) || $args['posts_per_page'] > 100) {
            $args['posts_per_page'] = 50; // Reasonable default
        }
     ...
     */
    public static function lazy_image($image_url, $alt = '', $attributes = array()) {
        $default_attrs = array(
            'loading' => 'lazy',
            'decoding' => 'async',
    ...
    public static function defer_script($handle, $defer = true, $async = false) {
        add_filter('script_loader_tag', function($tag, $script_handle) use ($handle, $defer, $async) {
