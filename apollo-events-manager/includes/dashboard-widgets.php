...
function apollo_get_user_reminders() {
    ...
    $active_reminders = array_filter($reminders, function($reminder) {
        return !isset($reminder['completed']) || !$reminder['completed'];
    });
    
    // Sort by date
    usort($active_reminders, function($a, $b) {
        $date_a = isset($a['date']) ? strtotime($a['date']) : 0;
        $date_b = isset($b['date']) ? strtotime($b['date']) : 0;
        return $date_a - $date_b;
    });
    
    return array_slice($active_reminders, 0, 5); // Top 5
}
...
function apollo_get_personal_todos() {
    ...
    // Filter active todos
    $active_todos = array_filter($todos, function($todo) {
...
function apollo_get_nucleo_todos() {
    ...
    $todos = get_option('_apollo_nucleo_todos', array());
    if (!is_array($todos)) {
        $todos = array();
    }
