...
class Apollo_Event_Data_Helper {
	...
	public static function get_local_data( $event_id ) {
		...
		return array(
			...
			'city'       => $city,
			'state'      => $state,
			'region'     => trim( implode( ', ', array_filter( array( $city, $state ) ) ) ),
			'lat'        => $lat,
			'lng'        => $lng,
	...
	public static function get_coordinates( $event_id, $local_id = 0 ) {
		...
		if ( $local_id ) {
			foreach ( array( '_local_latitude', '_local_lat' ) as $key ) {
			...
			foreach ( array( '_local_longitude', '_local_lng' ) as $key ) {
				if ( $val = apollo_get_post_meta( $local_id, $key, true ) ) {
		...
		if ( ! $lat ) {
			foreach ( array( '_event_latitude', 'geolocation_lat' ) as $key ) {
		...
		if ( ! $lng ) {
			foreach ( array( '_event_longitude', 'geolocation_long' ) as $key ) {
				if ( $val = apollo_get_post_meta( $event_id, $key, true ) ) {
					$lng = is_numeric( $val ) ? floatval( $val ) : 0;
					if ( $lng ) {
						break;
	...
	public static function get_cached_event_ids( $future_only = true, $cache_ttl = null ) {
		...
		return is_array( $event_ids ) ? $event_ids : array();
	}
	...
	public static function get_event_data( $event_id ) {
		...
		// Categories and tags.
		$categories = array();
		$terms_cats = wp_get_post_terms( $event_id, 'event_listing_category', array( 'fields' => 'names' ) );
		if ( ! is_wp_error( $terms_cats ) ) {
			$categories = $terms_cats;
		}

		$tags       = array();
		$terms_tags = wp_get_post_terms( $event_id, 'event_listing_tag', array( 'fields' => 'names' ) );
		...
		// Sounds/genres.
		$sounds       = array();
		$terms_sounds = wp_get_post_terms( $event_id, 'event_sounds', array( 'fields' => 'names' ) );
