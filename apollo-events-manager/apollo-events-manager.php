...
class Apollo_Events_Manager_Plugin {
	...
	private function init_hooks() {
		...
		add_action( 'apollo_core_ready', array( $this, 'on_core_ready' ) );
		...
		// Profile update AJAX
		add_action( 'wp_ajax_apollo_save_profile', array( $this, 'ajax_save_profile' ) );
		...
		add_action( 'trashed_post', array( $this, 'clear_portal_cache' ), 10 );
		add_action( 'deleted_post', array( $this, 'clear_portal_cache' ), 10 );
	...
	public function auto_geocode_local( $post_id, $post ) {
		...
		// Build search query
		$query_parts = array_filter( array( $addr, $city, 'Brasil' ) );
	...
	public function ajax_toggle_favorite() {
		...
		if ( class_exists( 'Apollo_Event_Stats' ) ) {
			...
			if ( $action === 'add' ) {
				call_user_func_array( array( $stats_class, 'record_action' ), array( $event_id, 'favorite', get_current_user_id() ) );
			} else {
				call_user_func_array( array( $stats_class, 'record_action' ), array( $event_id, 'unfavorite', get_current_user_id() ) );
			}
		}
	...
	public function ajax_get_event_modal() {
		try {
			...
			if ( ! $event_id ) {
				$error_data = array( 'message' => 'ID do evento inválido' );
	...
	private function process_event_submission( $event_id = 0 ) {
		...
		$dj_ids     = isset( $_POST['event_djs'] ) && is_array( $_POST['event_djs'] ) ? array_map( 'absint', $_POST['event_djs'] ) : array();
		$sounds     = isset( $_POST['event_sounds'] ) && is_array( $_POST['event_sounds'] ) ? array_map( 'sanitize_text_field', $_POST['event_sounds'] ) : array();
	...
	private function render_submit_form_html( $event_id = 0 ) {
		...
		$sounds_terms = $is_edit ? wp_get_post_terms( $event_id, 'event_sounds', array( 'fields' => 'slugs' ) ) : ( isset( $_POST['event_sounds'] ) ? array_map( 'sanitize_text_field', $_POST['event_sounds'] ) : array() );
		...
		$all_sounds = is_wp_error( $all_sounds ) ? array() : $all_sounds;
	...
	public function track_event_view() {
		...
		if ( class_exists( 'Apollo_Event_Stats' ) ) {
			$stats_class = 'Apollo_Event_Stats';
			call_user_func_array( array( $stats_class, 'record_action' ), array( $event_id, 'view' ) );
		}
	}
	...
	public function industry_overview_shortcode( $atts ) {
		...
		$user_id             = get_current_user_id();
		$stats_class         = 'Apollo_Event_Stats';
		$stats               = call_user_func( array( $stats_class, 'get_user_stats' ), $user_id );
		$sounds_interactions = call_user_func( array( $stats_class, 'get_sounds_interactions' ), $user_id );
...
function apollo_events_manager_activate() {
	...
	try {
		...
		if ( file_exists( $stats_file ) ) {
			...
			if ( class_exists( 'Apollo_Event_Stats' ) ) {
				$stats_class = 'Apollo_Event_Stats';
				call_user_func( array( $stats_class, 'create_table' ) );
			}
		}
