...
<div class="simple-favorites-post-types">
	...
	<div class="post-type-row">
		<div class="post-type-checkbox">
			<input type="checkbox" name="simplefavorites_display[posttypes][<?php echo $posttype; ?>][display]" value="true" 
			<?php
			if ( $display ) {
				echo ' checked';}
			?>
		...
		<?php echo $post_type_object->labels->name; ?>
			<button class="button" data-favorites-toggle-post-type-settings 
			<?php
			if ( ! $display ) {
				echo 'style="display:none;"';}
			?>
			><?php _e( 'Settings', 'favorites' ); ?></button>
		</div>
		<div class="post-type-settings">
			<div class="row">
				...
				<div class="field">
					<input type="checkbox" name="simplefavorites_display[posttypes][<?php echo $posttype; ?>][before_content]" value="true" 
					<?php
					if ( isset( $display['before_content'] ) ) {
						echo ' checked';}
					?>
					/> <?php _e( 'Include before content', 'favorites' ); ?>
				</div>
			</div><!-- .row -->
			<div class="row">
				<div class="description">
				...
				<div class="field">
					<input type="checkbox" name="simplefavorites_display[posttypes][<?php echo $posttype; ?>][after_content]" value="true" 
					<?php
					if ( isset( $display['after_content'] ) ) {
						echo ' checked';}
					?>
					/> <?php _e( 'Include after content', 'favorites' ); ?>
				</div>
			</div><!-- .row -->
			<div class="row">
				<div class="description">
				...
				<div class="field">
					<input type="checkbox" name="simplefavorites_display[posttypes][<?php echo $posttype; ?>][postmeta]" value="true" 
					<?php
					if ( isset( $display['postmeta'] ) ) {
						echo ' checked';}
					?>
					/> <?php _e( 'Add meta box', 'favorites' ); ?>
				</div>
			</div><!-- .row -->
			<div class="row">
				<div class="description">
				...
				<div class="field">
					<input type="checkbox" name="simplefavorites_display[posttypes][<?php echo $posttype; ?>][admincolumns]" value="true" 
					<?php
					if ( isset( $display['admincolumns'] ) ) {
						echo ' checked';}
					?>
					/> <?php _e( 'Add admin column', 'favorites' ); ?>
				</div>
			</div><!-- .row -->
...
<div class="simple-favorites-display-settings">
		...
		<div class="field">
			...
			<select name="simplefavorites_display[button_element_type]">
				...
				if ( $button_type == 'button' ) {
					echo 'selected';}
				?>
				><?php _e( 'Button', 'favorites' ); ?></option>
				<option value="a" 
				<?php
				if ( $button_type == 'a' ) {
					echo 'selected';}
				?>
				><?php _e( 'a (link)', 'favorites' ); ?></option>
				<option value="div" 
				<?php
				if ( $button_type == 'div' ) {
					echo 'selected';}
				?>
				><?php _e( 'Div', 'favorites' ); ?></option>
				<option value="span" 
				<?php
				if ( $button_type == 'span' ) {
					echo 'selected';}
				?>
				><?php _e( 'Span', 'favorites' ); ?></option>
			</select>
		</div>
	</div><!-- .row -->
	<div class="row">
		...
		<div class="field">
			...
			<select name="simplefavorites_display[buttontype]" data-favorites-preset-button-select>
				...
				foreach ( $preset_buttons as $button_name => $attrs ) {
					...
					if ( $button_name == $button_type_selected ) {
						$out .= ' selected';
					}
	...
	<div class="row">
		...
		<div class="field">
			<label class="block"><input type="checkbox" name="simplefavorites_display[button_colors][custom]" value="true" data-favorites-custom-colors-checkbox 
			<?php
			if ( $this->settings_repo->buttonColors( 'custom' ) ) {
				echo 'checked';}
			?>
			/><?php _e( 'Specify custom colors', 'favorites' ); ?></label>
			<div class="color-options" data-favorites-custom-colors-options>
				<div class="option-group">
					...
					foreach ( $default_options as $option => $label ) {
						...
						$out .= ' value="';
						if ( $this->settings_repo->buttonColors( $option ) ) {
							$out .= $this->settings_repo->buttonColors( $option );
						}
						$out .= '" />';
						$out .= '</div><!-- .option -->';
						echo $out;
				...
				<div class="option-group">
					...
					foreach ( $default_options as $option => $label ) {
						...
						$out .= '<input type="text" data-favorites-color-picker="' . $option . '" name="simplefavorites_display[button_colors][' . $option . ']"';
						if ( $this->settings_repo->buttonColors( $option ) ) {
							$out .= ' value="' . $this->settings_repo->buttonColors( $option ) . '"';
						}
						$out .= '" />';
						$out .= '</div><!-- .option -->';
						echo $out;
				...
				<div class="option-group">
					<div class="option box-shadow">
						<label><input type="checkbox" name="simplefavorites_display[button_colors][box_shadow]" value="true" 
						<?php
						if ( $this->settings_repo->buttonColors( 'box_shadow' ) ) {
							echo 'checked';}
						?>
						data-favorites-button-shadow /><?php _e( 'Include button shadow', 'favorites' ); ?>
					</div>
				</div>
			</div><!-- .color-options -->
		</div>
	...
	<div class="row">
		...
		<div class="field">
			<label><input type="checkbox" name="simplefavorites_display[buttoncount]" value="true" 
			<?php
			if ( $this->settings_repo->includeCountInButton() ) {
				echo 'checked';}
			?>
			data-favorites-include-count-checkbox /> <?php _e( 'Include count in button', 'favorites' ); ?></label>
		</div>
	</div><!-- .row -->
...
<div class="simple-favorites-post-types">
	<div class="post-type-row">
		<div class="post-type-checkbox">
			<input type="checkbox" name="simplefavorites_display[loadingindicator][include]" value="true" 
			<?php
		...
		<div class="post-type-settings">
			...
			<div class="row">
				...
				<div class="field">
					<label class="block"><input type="checkbox" name="simplefavorites_display[loadingindicator][include_html]" value="true" 
					<?php
					if ( $this->settings_repo->loadingIndicatorType( 'include_html' ) ) {
						echo 'checked';}
					?>
					data-favorites-spinner-type="html">                  <?php _e( 'Use CSS/HTML Spinner', 'favorites' ); ?>
					</label>
					<label><input type="checkbox" name="simplefavorites_display[loadingindicator][include_image]" value="true" 
					<?php
					if ( $this->settings_repo->loadingIndicatorType( 'include_image' ) ) {
						echo 'checked';}
					?>
					data-favorites-spinner-type="image">                 <?php _e( 'Use Image/GIF Spinner', 'favorites' ); ?>
					</label>
				</div>
			</div><!-- .row -->
			<div class="row">
				...
				<div class="field">
					<label><input type="checkbox" name="simplefavorites_display[loadingindicator][include_preload]" value="true" 
					<?php
					if ( $this->settings_repo->includeLoadingIndicatorPreload() ) {
						echo 'checked';}
					?>
					><?php _e( 'Add loading state on page load', 'favorites' ); ?>
				</label>
				</div>
			</div><!-- .row -->
		</div><!-- .post-type-settings -->
...
<div class="simple-favorites-post-types">
	<div class="post-type-row">
		<div class="post-type-checkbox">
			<input type="checkbox" name="simplefavorites_display[listing][customize]" value="true" 
			<?php
			if ( $this->settings_repo->listCustomization() ) {
				echo 'checked';}
			?>
			data-favorites-posttype-checkbox />
		</div>
		...
		<div class="post-type-settings">
			<div class="row">
				...
				<div class="field">
					...
					<select name="simplefavorites_display[listing][wrapper_type]">
						<option value="ul" 
						<?php
						if ( $this->settings_repo->listCustomization( 'wrapper_type' ) == 'ul' ) {
							echo 'selected';}
						?>
						><?php _e( 'Unordered List (ul)', 'favorites' ); ?></option>
						<option value="ol" 
						<?php
						if ( $this->settings_repo->listCustomization( 'wrapper_type' ) == 'ol' ) {
							echo 'selected';}
						?>
						><?php _e( 'Ordered List (ol)', 'favorites' ); ?></option>
						<option value="div" 
						<?php
						if ( $this->settings_repo->listCustomization( 'wrapper_type' ) == 'div' ) {
							echo 'selected';}
						?>
						><?php _e( 'Div', 'favorites' ); ?></option>
					</select>
			...
			<div class="row">
				...
				<div class="field">
					...
					<select name="simplefavorites_display[listing][listing_type]">
						<option value="li" 
						<?php
						if ( $this->settings_repo->listCustomization( 'listing_type' ) == 'ul' ) {
							echo 'selected';}
						?>
						><?php _e( 'List Item (li)', 'favorites' ); ?></option>
						<option value="p" 
						<?php
						if ( $this->settings_repo->listCustomization( 'listing_type' ) == 'ol' ) {
							echo 'selected';}
						?>
						><?php _e( 'Paragraph (p)', 'favorites' ); ?></option>
						<option value="div" 
						<?php
						if ( $this->settings_repo->listCustomization( 'listing_type' ) == 'div' ) {
							echo 'selected';}
						?>
						><?php _e( 'Div', 'favorites' ); ?></option>
					</select>
			...
			<div class="row">
				...
				<div class="field">
					<p>
						<label class="block"><input type="checkbox" name="simplefavorites_display[listing][customize_markup]" value="true" data-favorites-listing-customizer-checkbox 
						<?php
						if ( $this->settings_repo->listCustomization( 'customize_markup' ) ) {
							echo 'checked';}
						?>
						/><?php _e( 'Customize Content', 'favorites' ); ?></label>
					</p>
