...
class SettingsRepository {
	...
	public function saveType() {
		$option = get_option( 'simplefavorites_users' );
		if ( ! isset( $option['anonymous']['saveas'] ) ) {
			return 'cookie';
		}
	...
	public function metaEnabled() {
		$posttypes = array();
		$types     = get_option( 'simplefavorites_display' );
		if ( ! isset( $types['posttypes'] ) || $types['posttypes'] == '' ) {
			return $posttypes;
		}
		foreach ( $types['posttypes'] as $key => $type ) {
			if ( isset( $type['postmeta'] ) && $type['postmeta'] == 'true' ) {
				array_push( $posttypes, $key );
			}
		}
		return $posttypes;
	}
	...
	public function loadingImage( $state = 'inactive' ) {
		$option = get_option( 'simplefavorites_display' );
		if ( isset( $option['loadingindicator']['include_html'] ) && $option['loadingindicator']['include_html'] ) {
			return $this->loadingHtml( $state );
		}
	...
	public function loadingIndicatorType( $type = 'include_image' ) {
		$option = get_option( 'simplefavorites_display' );
		if ( ! isset( $option['loadingindicator'][ $type ] ) || $option['loadingindicator'][ $type ] !== 'true' ) {
			return false;
		}
		return true;
	}
	...
	public function loadingHtml( $state = 'inactive' ) {
		$option = get_option( 'simplefavorites_display' );
		if ( ! isset( $option['loadingindicator']['include_html'] ) || $option['loadingindicator']['include_html'] !== 'true' ) {
			return false;
		}
	...
	public function getButtonType() {
		$option = get_option( 'simplefavorites_display' );
		if ( ! isset( $option['buttontype'] ) || $option['buttontype'] == '' ) {
			return 'custom';
		}
	...
	public function getButtonHtmlType() {
		$option = get_option( 'simplefavorites_display' );
		if ( ! isset( $option['button_element_type'] ) || $option['button_element_type'] == '' ) {
			return 'button';
		}
	...
	public function buttonColors( $color = 'background_default' ) {
		$option = get_option( 'simplefavorites_display' );
		if ( ! isset( $option['button_colors'] ) ) {
			$option['button_colors'] = array();
		}
	...
	public function listCustomization( $setting = 'customize' ) {
		$option = get_option( 'simplefavorites_display' );
		if ( ! isset( $option['listing']['customize'] ) || $option['listing']['customize'] !== 'true' ) {
			return false;
		}
		if ( $setting == 'all' && isset( $option['listing'] ) ) {
			return $option['listing'];
		}
