...
class UserRepository
{
    ...
    private function favoritesWithSiteID($favorites)
    {
        ...
        if (Helpers::keyExists('site_id', $favorites)) {
            return $favorites;
        }
        $new_favorites = [
            [
    ...
    {
        if (Helpers::groupsExist($favorites[0])) {
            return $favorites;
        }
        $data = [
            'group_id'   => 1,
    ...
    private function getLoggedInFavorites($user_id = null, $site_id = null, $group_id = null)
    {
        ...
        if (! is_null($site_id) && is_null($group_id)) {
            $favorites = Helpers::pluckSiteFavorites($site_id, $favorites);
        }
        if (! is_null($group_id)) {
            $favorites = Helpers::pluckGroupFavorites($group_id, $site_id, $favorites);
        }
    ...
    {
        if (! isset($_SESSION['simplefavorites'])) {
            $_SESSION['simplefavorites'] = [];
        }
    ...
    private function getCookieFavorites($site_id = null, $group_id = null)
    {
        if (! isset($_COOKIE['simplefavorites'])) {
            $_COOKIE['simplefavorites'] = json_encode([]);
        }
        $favorites = json_decode(stripslashes($_COOKIE['simplefavorites']), true);
        $favorites = $this->favoritesWithSiteID($favorites);
        $favorites = $this->favoritesWithGroups($favorites);
        if (isset($_POST['user_consent_accepted']) && $_POST['user_consent_accepted'] == 'true') {
            $favorites[0]['consent_provided'] = time();
        }
        if (! is_null($site_id) && is_null($group_id)) {
            $favorites = Helpers::pluckSiteFavorites($site_id, $favorites);
        }
    ...
    public function consentedToCookies()
    {
        ...
        if (isset($_COOKIE['simplefavorites'])) {
            ...
            if (isset($cookie[0]['consent_provided'])) {
                return true;
            }
            if (isset($cookie[0]['consent_denied'])) {
                return false;
            }
        }
