...
class FavoriteButton {
	...
	public function display( $loading = true ) {
		...
		if ( $this->settings_repo->includeLoadingIndicator() && $this->settings_repo->includeLoadingIndicatorPreload() && $loading ) {
			...
			$out .= $html;
			if ( $this->button_options['include_count'] ) {
				$out .= $this->addCount();
			}
		}
	...
	private function styleAttributes( $icon = false ) {
		if ( ! $this->button_options['custom_colors'] ) {
			return null;
		}
		$html           = '';
		...
		if ( $icon ) {
			if ( $this->favorited ) {
				if ( $active_colors['icon_active'] ) {
					$html .= 'color:' . $active_colors['icon_active'] . ';';
				}
			} elseif ( $default_colors['icon_default'] ) {
					$html .= 'color:' . $default_colors['icon_default'] . ';';
			}
			return $html;
		}

		if ( ! $this->button_options['box_shadow'] ) {
			$html .= 'box-shadow:none;-webkit-box-shadow:none;-moz-box-shadow:none;';
		}

		if ( $this->favorited ) {
			if ( $active_colors['background_active'] ) {
				$html .= 'background-color:' . $active_colors['background_active'] . ';';
			}
			if ( $active_colors['border_active'] ) {
				$html .= 'border-color:' . $active_colors['border_active'] . ';';
			}
			if ( $active_colors['text_active'] ) {
				$html .= 'color:' . $active_colors['text_active'] . ';';
			}
			return $html;
		}
		if ( $default_colors['background_default'] ) {
			$html .= 'background-color:' . $default_colors['background_default'] . ';';
		}
		if ( $default_colors['border_default'] ) {
			$html .= 'border-color:' . $default_colors['border_default'] . ';';
		}
		if ( $default_colors['text_default'] ) {
			$html .= 'color:' . $default_colors['text_default'] . ';';
		}
		return $html;
	}
	...
	private function addCount() {
		...
		if ( $this->favorited ) {
			if ( $active_colors['count_active'] ) {
				$html .= 'color:' . $active_colors['count_active'] . ';';
			}
		} elseif ( $default_colors['count_default'] ) {
				$html .= 'color:' . $default_colors['count_default'] . ';';
		}
