...
class FavoriteFilter
{
    ...
    {
        if (isset($this->filters['post_type']) && is_array($this->filters['post_type'])) {
            $this->filterByPostType();
        }
        if (isset($this->filters['terms']) && is_array($this->filters['terms'])) {
            $this->filterByTerm();
        }
        if (isset($this->filters['status']) && is_array($this->filters['status'])) {
            $this->filterByStatus();
        }
    ...
    private function filterByStatus()
    {
        foreach ($this->favorites as $key => $favorite) {
            ...
            if (! in_array($status, $this->filters['status'])) {
                unset($this->favorites[ $key ]);
            }
        }
    }
    ...
    private function filterByTerm()
    {
        ...
        foreach ($favorites as $key => $favorite) :
            ...
            foreach ($taxonomies as $taxonomy => $terms) {
                ...
                $post_terms = wp_get_post_terms($favorite, $taxonomy, [ 'fields' => 'slugs' ]);
                if (! empty($post_terms)) {
