...
abstract class FavoriteListBase
{
    ...
    protected function setOptions()
    {
        ...
        if (($this->list_options->user_id && isset($options['include_button'])) && (get_current_user_id() !== $this->list_options->user_id)) {
            unset($options['include_button']);
        }
        ...
        if (! property_exists($this->list_options, 'customize_markup')) {
            $this->list_options->customize_markup = false;
        }
        if (! property_exists($this->list_options, 'custom_markup_html')) {
            $this->list_options->custom_markup_html = false;
        }
    }
    ...
    protected function setCustomOptions()
    {
        ...
        foreach ($options as $option => $val) {
            ...
            if ($option == 'custom_markup_html') {
                $this->list_options->custom_markup_html = $val;
            }
        }
    }
}
