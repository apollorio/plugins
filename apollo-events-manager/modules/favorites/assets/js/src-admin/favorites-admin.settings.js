...
FavoritesAdmin.Settings = function () {
	...
	plugin.bindEvents = function () {
		$( document ).ready(
			function () {
				...
				$.each(
					$( '[data-favorites-dependency-checkbox]' ),
					function () {
		...
		$( document ).on(
			'change',
			'*[data-favorites-anonymous-checkbox]',
			function () {
				plugin.toggleAnonymousSave();
				plugin.toggleAnonymousSettings();
			}
		...
		$( document ).on(
			'change',
			'[data-favorites-anonymous-settings]',
			function () {
		...
		$( document ).on(
			'click',
			'[data-favorites-toggle-post-type-settings]',
			function (e) {
				e.preventDefault();
		...
		$( document ).on(
			'change',
			'[data-favorites-preset-button-select]',
			function () {
				plugin.toggleButtonTypes();
			}
		);
		...
		$( document ).on(
			'change',
			'[data-favorites-include-count-checkbox]',
			function () {
				plugin.toggleCountOptions();
			}
		);
		...
		$( document ).on(
			'change',
			'[data-favorites-custom-colors-checkbox]',
			function () {
				plugin.toggleCustomColorOptions();
			}
		);
		...
		$( document ).on(
			'change',
			'[data-favorites-require-consent-checkbox]',
			function () {
				plugin.toggleModalConsentContent();
			}
		);
	...
	*/
	plugin.toggleAnonymousSettings = function (checkbox) {
		...
		if ( ! $( '[data-favorites-anonymous-settings="redirect"]' ).is( ':checked' ) ) {
			$( '[data-favorites-anonymous-redirect-content]' ).hide();
		}
		if ( ! $( '[data-favorites-anonymous-settings="modal"]' ).is( ':checked' ) ) {
			$( '[data-favorites-authentication-modal-content]' ).hide();
		}
	}
