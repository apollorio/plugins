	...
	*/
	plugin.isFavorite = function (post_id, site_favorites) {
		...
		$.each(
			site_favorites,
			function (i, v) {
				if ( v.post_id === parseInt( post_id ) ) {
					status = true;
				}
				if ( parseInt( v.post_id ) === post_id ) {
					status = true;
				}
			}
		);
	...
	*/
	plugin.siteIndex = function (siteid) {
		for ( var i = 0; i < Favorites.userFavorites.length; i++ ) {
			if ( Favorites.userFavorites[i].site_id !== parseInt( siteid ) ) {
				continue;
			}
			return i;
		}
...
Favorites.Formatter = function () {
	...
	*/
	plugin.addFavoriteCount = function (html, count) {
		if ( ! Favorites.jsData.button_options.include_count ) {
			return html;
		}
		if ( count <= 0 ) {
			count = 0;
...
Favorites.ButtonOptionsFormatter = function () {
	...
	*/
	plugin.format = function (button, isFavorite) {
		if ( plugin.options.custom_colors ) {
			plugin.colors( button, isFavorite );
		}
	...
	*/
	plugin.html = function (button, isFavorite) {
		...
		if ( plugin.options.button_type === 'custom' ) {
			if ( isFavorite ) {
				$( button ).html( plugin.formatter.addFavoriteCount( Favorites.jsData.favorited, count ) );
			}
			if ( ! isFavorite ) {
				$( button ).html( plugin.formatter.addFavoriteCount( Favorites.jsData.favorite, count ) );
			}
	...
	plugin.colors = function (button, isFavorite) {
		...
		if ( isFavorite ) {
			...
			if ( options.background_active ) {
				$( button ).css( 'background-color', options.background_active );
			}
			if ( options.border_active ) {
				$( button ).css( 'border-color', options.border_active );
			}
			if ( options.text_active ) {
				$( button ).css( 'color', options.text_active );
			}
			return;
		}
		var options = plugin.options.default;
		if ( options.background_default ) {
			$( button ).css( 'background-color', options.background_default );
		}
		if ( options.border_default ) {
			$( button ).css( 'border-color', options.border_default );
		}
		if ( options.text_default ) {
			$( button ).css( 'color', options.text_default );
		}
...
Favorites.Clear = function () {
	...
	plugin.bindEvents = function () {
		$( document ).on(
			'click',
			Favorites.selectors.clear_button,
			function (e) {
				e.preventDefault();
	...
	plugin.updateClearButtons = function () {
		...
		for ( var i = 0; i < $( Favorites.selectors.clear_button ).length; i++ ) {
			...
			for ( var c = 0; c < Favorites.userFavorites.length; c++ ) {
				if ( Favorites.userFavorites[c].site_id !== parseInt( siteid ) ) {
					continue;
				}
	...
	*/
	plugin.clearSiteFavorites = function (site_id) {
		$.each(
			...
			function (i, v) {
				if ( this.site_id !== parseInt( site_id ) ) {
					return;
				}
...
Favorites.Lists = function () {
	...
	*/
	plugin.updateAllLists = function () {
		if ( typeof Favorites.userFavorites === 'undefined' ) {
			return;
		}
		for ( var i = 0; i < Favorites.userFavorites.length; i++ ) {
	...
	plugin.removeInvalidListItems = function (list, favorites) {
		...
		$.each(
			...
			function (i, v) {
				...
				if ( ! plugin.utilities.isFavorite( postid, favorites ) ) {
					$( this ).remove();
				}
			}
		);
...
Favorites.Button = function () {
	...
	*/
	plugin.addLoadingIndication = function (html) {
		if ( Favorites.jsData.indicate_loading !== '1' ) {
			return html;
		}
		if ( plugin.data.status === 'active' ) {
			return Favorites.jsData.loading_text + Favorites.jsData.loading_image_active;
		}
...
Favorites.ButtonUpdater = function () {
	...
	*/
	plugin.setButtonData = function () {
		...
		if ( plugin.data.favorite_count <= 0 ) {
			plugin.data.favorite_count = 0;
		}
	}
	...
	plugin.updateTotal = function () {
		...
		for ( var i = 0; i < $( Favorites.selectors.total_favorites ).length; i++ ) {
			...
			for ( var c = 0; c < Favorites.userFavorites.length; c++ ) {
				...
				if ( site_favorites.site_id !== siteid ) {
					continue;
				}
				$.each(
					site_favorites.posts,
					function () {
						if ( $( item ).attr( 'data-posttypes' ) === 'all' ) {
							count++;
							return;
						}
						if ( $.inArray( this.post_type, posttypes_array ) !== -1 ) {
							count++;
						}
					}
				);
...
Favorites.PostFavoriteCount = function () {
	...
	plugin.bindEvents = function () {
		...
		$( document ).on(
			'favorites-cleared',
			function (event, button, old_favorites) {
	...
	plugin.updateCounts = function (favorites, decrement) {
		...
		for ( var i = 0; i < $( '[' + Favorites.selectors.post_favorite_count + ']' ).length; i++ ) {
			...
			for ( var c = 0; c < favorites.length; c++ ) {
				...
				if ( site_favorites.site_id !== parseInt( siteid ) ) {
					continue;
				}
				$.each(
					site_favorites.posts,
...
Favorites.RequireAuthentication = function () {
	...
	plugin.bindEvents = function () {
		...
		$( document ).on(
			'click',
			'[' + Favorites.selectors.close_modals + ']',
			function (e) {
				e.preventDefault();
				plugin.closeModal();
			}
...
Favorites.RequireConsent = function () {
	...
	plugin.bindEvents = function () {
		$( document ).on(
			'favorites-require-consent',
			function (event, consent_data, post_data, active_button) {
		...
		$( document ).on(
			'favorites-user-consent-approved',
			function (e, button) {
