...
class APRIO_REST_Authentication extends APRIO_REST_CRUD_Controller {
	...
	public function perform_user_authentication( $request ) {
		...
		if ( ! empty( $username ) && ! empty( $password ) ) {
			...
			if ( is_wp_error( $user ) ) {
				...
				$user_mobile_menu = is_array( $user_mobile_menu ) ? $user_mobile_menu : array();
				...
				if ( $is_compatibilidade && $enable_compatibilidade ) {
					...
					// --- Skills ---
					$skills_slugs = array();
					$skills_arr   = maybe_unserialize( isset( $user_meta['_skills'][0] ) ? $user_meta['_skills'][0] : array() );
					...
					$interests_arr   = maybe_unserialize( isset( $user_meta['_interests'][0] ) ? $user_meta['_interests'][0] : array() );
	...
	private function validate_jwt_token( $token ) {
		...
		if ( isset( $payload['exp'] ) && time() > $payload['exp'] ) {
			return new WP_Error( 'rest_forbidden', __( 'Token has expired.', 'textdomain' ), array( 'status' => 401 ) );
		}
