...
class APRIO_REST_Create_Meeting_Controller extends APRIO_REST_CRUD_Controller
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route(
            ...
            [
                ...
                'callback'            => [ $this, 'aprio_create_compatibilidade_meeting' ],
        ...
        register_rest_route(
            ...
            [
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => [ $this, 'aprio_get_compatibilidade_user_meetings' ],
                'permission_callback' => [ $this, 'check_user_permission' ],
            ]
        );
        ...
        register_rest_route(
            ...
            [
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => [ $this, 'aprio_cancel_compatibilidade_meetings' ],
                'permission_callback' => [ $this, 'check_user_permission' ],
                'args'                => [
                    'meeting_id' => [
                        'required'          => true,
        ...
        register_rest_route(
            ...
            [
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => [ $this, 'aprio_update_compatibilidade_meeting_status' ],
                'permission_callback' => [ $this, 'check_user_permission' ],
                'args'                => [
                    'meeting_id' => [
                        'required'          => true,
        ...
        register_rest_route(
            ...
            [
                ...
                'callback'            => [ $this, 'aprio_get_available_compatibilidade_meeting_slots' ],
                'permission_callback' => [ $this, 'check_user_permission' ],
                'args'                => [
                    'user_id' => [
                        'required'          => false,
        ...
        register_rest_route(
            ...
            [
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => [ $this, 'aprio_update_compatibilidade_availability_slots' ],
                'permission_callback' => [ $this, 'check_user_permission' ],
                'args'                => [
                    'availability_slots' => [
                        'required'          => true,
        ...
        register_rest_route(
            ...
            [
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => [ $this, 'aprio_get_common_availability_slots' ],
                'permission_callback' => [ $this, 'check_user_permission' ],
                'args'                => [
                    'event_id' => [
                        'required'          => true,
    ...
    public function aprio_create_compatibilidade_meeting(WP_REST_Request $request)
    {
        ...
        if ($auth_check) {
            ...
            if (
                ! $user_id || ! get_userdata($user_id) || empty($meeting_date) || empty($slot) || empty($participants) || ! is_array($participants)
            ) {
                return new WP_REST_Response(
                    [
                        'code'    => 400,
            ...
            if (!preg_match('/^' . preg_quote($wpdb->prefix, '/') . 'aprio_\w+$/', $table)) {
                error_log('APRIO REST API: Invalid table name: ' . $table);
    ...
    public function aprio_get_compatibilidade_user_meetings(WP_REST_Request $request)
    {
        ...
        if ($auth_check) {
            ...
            $all_meetings = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$safe_table} WHERE event_id = %d", $event_id), ARRAY_A);
    ...
    public function aprio_cancel_compatibilidade_meetings(WP_REST_Request $request)
    {
        ...
        if ($auth_check) {
            ...
            if (! $meeting_id || ! $user_id) {
                return new WP_REST_Response(
                    [
                        'code'    => 400,
                        'status'  => 'error',
                        'message' => 'Missing meeting_id or user_id.',
                    ],
                    400
                );
            }
            ...
            $safe_table = esc_sql($table);
            $meeting    = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$safe_table} WHERE id = %d", $meeting_id), ARRAY_A);
            ...
            // Validate table name (already validated above, but ensure consistency)
            if (!preg_match('/^' . preg_quote($wpdb->prefix, '/') . 'aprio_\w+$/', $table)) {
                error_log('APRIO REST API: Invalid table name: ' . $table);
                return new WP_Error('invalid_table', 'Invalid table name', ['status' => 400]);
            }
            $safe_table = esc_sql($table);
            $updated    = $wpdb->update($safe_table, [ 'meeting_status' => -1 ], [ 'id' => $meeting_id ], [ '%d' ], [ '%d' ]);
    ...
    public function aprio_update_compatibilidade_meeting_status(WP_REST_Request $request)
    {
        ...
        if ($auth_check) {
            ...
            if ($new_status === 1) {
                ...
                $conflicting_meeting = $wpdb->get_row(
                    $wpdb->prepare(
                        "
                ...
                if ($conflicting_meeting) {
                    $existing_participants = $wpdb->get_var(
                        $wpdb->prepare(
                            "
    ...
    public function aprio_update_compatibilidade_availability_slots(WP_REST_Request $request)
    {
        ...
        if ($auth_check) {
            ...
            // This is the "slots" array from GET
