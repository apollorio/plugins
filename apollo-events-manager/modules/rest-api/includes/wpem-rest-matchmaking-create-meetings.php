...
class WPEM_REST_Create_Meeting_Controller extends WPEM_REST_CRUD_Controller {
		...
		if ( $auth_check ) {
			...
			if ( ! empty( $participants ) ) {
				foreach ( array_keys( $participants ) as $participant_id ) {
					...
					$profession          = (object) array( 'name' => $participant_meta['_profession'][0] ?? '' );
					...
					wp_mail( $participant_email, $subject, $body, array( 'Content-Type: text/html; charset=UTF-8' ) );
				}//end foreach
			}//end if
	...
	public function wpem_cancel_matchmaking_meetings( WP_REST_Request $request ) {
		...
		if ( $auth_check ) {
			...
			$updated = $wpdb->update( $table, array( 'meeting_status' => -1 ), array( 'id' => $meeting_id ), array( '%d' ), array( '%d' ) );
			...
			foreach ( $participant_ids as $pid ) {
				...
				if ( ! isset( $slot_data[ $event_id ] ) ) {
					$slot_data[ $event_id ] = array();
				}
				if ( ! isset( $slot_data[ $event_id ][ $meeting_date ] ) ) {
					$slot_data[ $event_id ][ $meeting_date ] = array();
				}
	...
	public function wpem_update_matchmaking_meeting_status( WP_REST_Request $request ) {
		...
		if ( $auth_check ) {
			...
			if ( ! $meeting_id || ! $user_id || ! in_array( $new_status, array( 0, 1 ), true ) ) {
				return new WP_REST_Response(
					array(
						'code'    => 400,
		...
		if ( $auth_check ) {
			...
			foreach ( $rows as $row ) {
				...
				foreach ( $user_ids as $u_id ) {
					...
					if ( isset( $participant_ids[ $u_id ] ) && in_array( $participant_ids[ $u_id ], array( 0, 1 ), true ) ) {
						$booked_slots[] = $meeting_time;
						break;
					}
				}
