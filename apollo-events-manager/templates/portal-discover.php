        ...
        <div class="filters-and-search" data-tooltip="<?php esc_attr_e('Filtros e busca de eventos', 'apollo-events-manager'); ?>">
            ...
            <div class="apollo-period-filters" role="group" aria-label="<?php esc_attr_e('Filtros por período', 'apollo-events-manager'); ?>" data-tooltip="<?php esc_attr_e('Filtrar eventos por período', 'apollo-events-manager'); ?>">
                ...
                foreach ($periods as $slug => $label):
                    ...
                    $url = add_query_arg('period', $slug, remove_query_arg(['local', 'style']));
                ?>
            ...
            <div class="menutags event_types" role="group" aria-label="Filtros de eventos">
                ...
                $sounds = get_terms(['taxonomy' => 'event_sounds', 'hide_empty' => false]);
                if (!is_wp_error($sounds)) {
                    foreach (array_slice($sounds, 0, 5) as $sound) {
                        ...
                        echo sprintf('<a href="%s" class="menutag event-category %s" data-slug="%s">%s</a>',
                            esc_url($url), $is_active ? 'active' : '', esc_attr($sound->slug), esc_html($sound->name));
                    }
                }
                ...
                foreach ($locals as $local) {
                    ...
                    $url = add_query_arg('local', $local->post_name, remove_query_arg(['period', 'style']));
                    echo sprintf('<a href="%s" class="menutag event-category event-local-filter %s" data-slug="%s" data-filter-type="local">%s</a>',
                        esc_url($url), $is_active ? 'active' : '', esc_attr($local->post_name), esc_html($name));
                }
                ?>
        ...
        <section class="apollo-events-section apollo-events-section--featured">
            ...
            <div class="apollo-events-grid">
                ...
                foreach ($featured_events as $post) {
                    setup_postdata($post); // Bug fix: Configurar contexto global para get_the_ID() funcionar
        ...
        <section class="apollo-events-section apollo-events-section--all">
            ...
            <div class="apollo-events-grid event_listings card-view">
                ...
                if (empty($event_ids)) {
                    ...
                    if ($local_filter) {
                        $filtered_ids = array_filter($filtered_ids, function($id) use ($local_filter) {
                    ...
                    if ($style_filter) {
                        $filtered_ids = array_filter($filtered_ids, function($id) use ($style_filter) {
                            $tags = wp_get_post_terms($id, 'event_sounds');
                            if (is_wp_error($tags)) return false;
                            foreach ($tags as $tag) {
                                if ($tag->slug === $style_filter) return true;
                            }
                            return false;
                        });
                    }
