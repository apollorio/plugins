...
// Build link arrays
$music_links = [];
if ($dj_soundcloud) $music_links[] = ['url' => $dj_soundcloud, 'icon' => 'ri-soundcloud-line', 'label' => 'SoundCloud', 'active' => true];
if ($dj_spotify) $music_links[] = ['url' => $dj_spotify, 'icon' => 'ri-spotify-line', 'label' => 'Spotify', 'active' => false];
if ($dj_youtube) $music_links[] = ['url' => $dj_youtube, 'icon' => 'ri-youtube-line', 'label' => 'YouTube', 'active' => false];

$social_links = [];
if ($dj_instagram) $social_links[] = ['url' => apollo_dj_normalize_url($dj_instagram, 'instagram'), 'icon' => 'ri-instagram-line', 'label' => 'Instagram'];
if ($dj_facebook) $social_links[] = ['url' => apollo_dj_normalize_url($dj_facebook, 'facebook'), 'icon' => 'ri-facebook-circle-line', 'label' => 'Facebook'];
if ($dj_twitter) $social_links[] = ['url' => apollo_dj_normalize_url($dj_twitter, 'twitter'), 'icon' => 'ri-twitter-x-line', 'label' => 'Twitter'];
if ($dj_tiktok) $social_links[] = ['url' => apollo_dj_normalize_url($dj_tiktok, 'tiktok'), 'icon' => 'ri-tiktok-line', 'label' => 'TikTok'];

$asset_links = [];
if ($dj_media_kit_url) $asset_links[] = ['url' => $dj_media_kit_url, 'icon' => 'ri-clipboard-line', 'label' => 'Media kit'];
if ($dj_rider_url) $asset_links[] = ['url' => $dj_rider_url, 'icon' => 'ri-clipboard-fill', 'label' => 'Rider'];
if ($dj_press_photos_url) $asset_links[] = ['url' => $dj_press_photos_url, 'icon' => 'ri-play-list-2-line', 'label' => 'Mix / Playlist'];
