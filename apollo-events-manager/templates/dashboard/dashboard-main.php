...
function apollo_events_get_dashboard_data() {
        ...
        while ($user_events->have_posts()) {
            ...
            if (count($data['recent_activity']) < 5) {
                $data['recent_activity'][] = array(
                    'message' => sprintf(
                        __('Evento "%s" recebeu %d visualizações', 'apollo-events-manager'),
    ...
    $data['trends'] = array(
        ...
        'views' => 12.5, // Placeholder - would need historical data
    );
...
function apollo_events_render_dashboard_content() {
    ...
    <!-- Stats Cards -->
    <?php
    apollo_events_render_stat_cards(array(
        array(
                ...
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    ...
                    <div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                            ...
                            <span style="font-size: 0.875rem; font-weight: 600; color: hsl(var(--foreground));">
                                <?php 
                                $published_count = count(array_filter($data['events'], function($e) { return $e['status'] === 'publish'; }));
...
apollo_events_render_dashboard_layout(array(
