...
if ($user_events_query->have_posts()) {
    while ($user_events_query->have_posts()) {
        ...
        $stats = array('total_views' => 0, 'popup_count' => 0, 'page_count' => 0);
...
usort($top_events, function($a, $b) { return $b['views'] - $a['views']; });
...
<style>
    :root {
        ...
        --ring: 240 5.9% 10%;
        --radius: 0.5rem;
        --status-published: 142 76% 36%;
        --status-pending: 48 96% 53%;
        --status-draft: 240 3.8% 46.1%;
    }
