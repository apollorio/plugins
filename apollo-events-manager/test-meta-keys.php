...
foreach ($events as $event) {
    ...
    echo "📋 Resumo:\n";
    $issues = array();
    if (empty($dj_ids)) $issues[] = "DJs não configurados";
    if (empty($local_ids)) $issues[] = "Local não configurado";
    if (empty($timetable)) $issues[] = "Timetable não configurado";
    if (!empty($dj_ids_old)) $issues[] = "Meta key antiga _event_djs encontrada";
    if (!empty($local_old)) $issues[] = "Meta key antiga _event_local encontrada";
    if (!empty($timetable_old)) $issues[] = "Meta key antiga _timetable encontrada";
