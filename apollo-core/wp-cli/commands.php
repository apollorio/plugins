...
class Apollo_Core_CLI_Commands {
	...
	public function db_test( $args, $assoc_args ) {
		...
		if ( $table_exists ) {
			...
			// Check row count - table name is safe (hardcoded with prefix).
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnquotedComplexPlaceholder -- Table name constructed from safe prefix + hardcoded value.
			$row_count = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM `%1$s`', $table_name ) );
