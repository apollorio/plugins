...
class Apollo_Moderation_WP_CLI {
	...
	public static function mod_stats() {
		...
		$safe_table = $wpdb->_escape( $table ); // phpcs:ignore WordPress.DB.RestrictedFunctions.restricted_db_escape -- Table name is validated.

        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$total = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM `{$safe_table}`" ) );

        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$by_action = $wpdb->get_results(
			$wpdb->prepare( "SELECT action, COUNT(*) as count FROM `{$safe_table}` GROUP BY action ORDER BY count DESC" )
		);

        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$by_actor = $wpdb->get_results(
			$wpdb->prepare( "SELECT actor_id, COUNT(*) as count FROM `{$safe_table}` GROUP BY actor_id ORDER BY count DESC LIMIT 10" )
		);
