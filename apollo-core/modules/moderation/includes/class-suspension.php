...
class Apollo_Moderation_Suspension {
	 ...
	 * @param string                $username Username (required by filter, unused).
	 * @param string                $password Password (required by filter, unused).
	 * @return WP_User|WP_Error
	 *
	 * @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	public static function check_suspension( $user, $username, $password ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- Required by 'authenticate' filter signature.
	...
	public static function suspend_user( $user_id, $days, $reason = '', $actor_id = null ) {
		...
		// Check permission.
		if ( ! user_can( $actor_id, 'suspend_users' ) ) { // phpcs:ignore WordPress.WP.Capabilities.Unknown -- Custom capability registered by Apollo_Moderation_Roles.
			return false;
		}
	...
	public static function block_user( $user_id, $reason = '', $actor_id = null ) {
		...
		// Check permission.
		if ( ! user_can( $actor_id, 'block_users' ) ) { // phpcs:ignore WordPress.WP.Capabilities.Unknown -- Custom capability registered by Apollo_Moderation_Roles.
			return false;
		}
