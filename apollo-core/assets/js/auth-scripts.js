...
(function() {
    ...
    async function handleLogin(e) {
        ...
        try {
            ...
            if (result.success) {
                handleLoginSuccess();
                const redirectUrl = result.data?.redirect_url || CONFIG.redirectAfterLogin;
                setTimeout(() => {
                    window.location.href = redirectUrl;
                }, 1200);
            } else {
                handleLoginFailure();
                const message = result.data?.message || result.data || CONFIG.strings.loginFailed;
    ...
    async function submitRegistration() {
        ...
        try {
            const quizToken = await fetchQuizToken();
            if (!quizToken) {
                showNotification('Validação do quiz expirada. Refazer o teste.', 'error');
                return;
            }

            const docType = form.querySelector('[name="doc_type"]')?.value || 'cpf';
            const cpf = form.querySelector('[name="cpf"]')?.value || '';
            const passport = form.querySelector('[name="passport"]')?.value || '';
            const soundsSelected = Array.from(form.querySelectorAll('[name="sounds[]"]:checked')).map((el) => el.value);

            const payload = new URLSearchParams({
                action: 'apollo_register',
                nonce: CONFIG.nonce,
                quiz_token: quizToken,
                user_name: form.querySelector('[name="nome"]')?.value?.trim() || '',
                user_instagram: form.querySelector('[name="instagram"]')?.value?.trim() || '',
                user_email: form.querySelector('[name="email"]')?.value?.trim() || '',
                user_password: form.querySelector('[name="senha"]')?.value || '',
                doc_type: docType,
                user_cpf: docType === 'cpf' ? cpf : '',
                user_passport: docType === 'passport' ? passport : '',
                user_sounds: JSON.stringify(soundsSelected)
            });
            ...
            if (result.success) {
                ...
                const message = result.data?.message || result.data || 'Erro ao processar cadastro. Tente novamente.';
                showNotification(message, 'error');
            }
        } catch (error) {
    ...
    async function fetchQuizToken() {
        try {
            const response = await fetch(CONFIG.ajaxUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
                body: new URLSearchParams({
                    action: 'apollo_set_quiz_token',
                    nonce: CONFIG.nonce
                })
            });

            const result = await response.json();
            if (result.success && result.data?.quiz_token) {
                return result.data.quiz_token;
            }
        } catch (error) {
            console.error('Quiz token error:', error);
        }
