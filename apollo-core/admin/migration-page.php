...
function apollo_core_handle_migration_action(): void {
	if ( ! isset( $_GET['page'] ) || 'apollo-core-migration' !== $_GET['page'] ) {
		return;
	}
	...
	if ( isset( $_POST['apollo_run_migration'] ) && check_admin_referer( 'apollo_migration_action' ) ) {
		...
		if ( $result['success'] ) {
			add_settings_error(
				'apollo_migration',
				'migration_success',
				/* translators: %1$d is the number of options migrated, %2$d is the number of meta keys migrated. */
				sprintf(
					__( 'Migration completed successfully! %1$d options and %2$d meta keys migrated.', 'apollo-core' ),
	...
	if ( isset( $_POST['apollo_create_tables'] ) && check_admin_referer( 'apollo_migration_action' ) ) {
		if ( function_exists( 'Apollo_Core\create_db_tables' ) ) {
			Apollo_Core\create_db_tables();
			add_settings_error(
				'apollo_migration',
				'tables_created',
				__( 'Database tables created successfully.', 'apollo-core' ),
				'success'
			);
		} else {
			add_settings_error(
				'apollo_migration',
				'tables_error',
				__( 'Failed to create database tables. Function not found.', 'apollo-core' ),
				'error'
			);
		}
	}
	...
	<div class="wrap">
		...
		<div class="card" style="max-width: 800px; margin-top: 20px;">
			...
			<form method="post" action="" style="margin-bottom: 20px;">
				...
				<p>
					<strong><?php esc_html_e( 'Database Setup:', 'apollo-core' ); ?></strong>
					<?php esc_html_e( 'Create required database tables (moderation log, audit log).', 'apollo-core' ); ?>
				</p>
				<p>
					<button type="submit" name="apollo_create_tables" class="button button-secondary">
						<?php esc_html_e( 'Create Database Tables', 'apollo-core' ); ?>
					</button>
				</p>
			</form>
