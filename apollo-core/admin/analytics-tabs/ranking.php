...
$valid_tax=['event_sounds','event_listing_category','event_listing_type','event_listing_tag'];
if(!in_array($tax,$valid_tax,true))$tax='event_sounds';
$ranking=class_exists('Apollo_Core\\Interesse_Ranking')?\Apollo_Core\Interesse_Ranking::get_global_ranking($tax,50):[];
$tax_labels=['event_sounds'=>__('Sons','apollo-core'),'event_listing_category'=>__('Categorias','apollo-core'),'event_listing_type'=>__('Tipos','apollo-core'),'event_listing_tag'=>__('Tags','apollo-core')];
...
<div class="apollo-ranking-filters">
	<div class="filter-group">
		...
		<select id="tax-filter" onchange="location.href='<?php echo esc_url(remove_query_arg(['tax','uid']));?>&tab=ranking&tax='+this.value">
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
	...
	<div class="apollo-chart-container">
		...
		<?php $max=$ranking[0]['count']??1;foreach($ranking as $i=>$r):$pct=$max>0?round(($r['count']/$max)*100,1):0;?>
...
<?php wp_enqueue_script('apollo-vendor-chartjs');$labels=wp_json_encode(array_column(array_slice($ranking,0,15),'name'));$data=wp_json_encode(array_map('intval',array_column(array_slice($ranking,0,15),'count')));?>
<script>
