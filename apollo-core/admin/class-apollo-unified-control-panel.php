...
class Apollo_Unified_Control_Panel {
	...
	public static function render_page() {
		...
		<div class="wrap apollo-control-panel">
			...
			<?php if ( $active_tab !== 'permissions' && $active_tab !== 'page-access' ) : ?>
			...
			<?php if ( $active_tab === 'page-access' ) : ?>
			<form method="post" action="options.php">
				<?php settings_fields( 'apollo_page_access' ); ?>
			<?php endif; ?>
	...
	private static function get_all_permissions() {
		return array(
			// Events permissions
			'read_eva_event'              => __( 'View Events', 'apollo-core' ),
			'create_eva_events'           => __( 'Create Events', 'apollo-core' ),
			'edit_eva_events'             => __( 'Edit Own Events', 'apollo-core' ),
			'edit_others_eva_events'      => __( 'Edit Others\' Events', 'apollo-core' ),
			'publish_eva_events'          => __( 'Publish Events', 'apollo-core' ),
			'delete_eva_events'           => __( 'Delete Own Events', 'apollo-core' ),
			'delete_others_eva_events'    => __( 'Delete Others\' Events', 'apollo-core' ),
			'read_private_eva_events'     => __( 'Read Private Events', 'apollo-core' ),

			// DJs permissions
			'read_dj_profile'             => __( 'View DJs', 'apollo-core' ),
			'create_dj_profiles'          => __( 'Create DJs', 'apollo-core' ),
			'edit_dj_profiles'            => __( 'Edit Own DJ Profiles', 'apollo-core' ),
			'edit_others_dj_profiles'     => __( 'Edit Others\' DJ Profiles', 'apollo-core' ),
			'delete_dj_profiles'          => __( 'Delete Own DJ Profiles', 'apollo-core' ),
			'delete_others_dj_profiles'   => __( 'Delete Others\' DJ Profiles', 'apollo-core' ),

			// Venues permissions
			'read_local'                  => __( 'View Venues', 'apollo-core' ),
			'create_locals'               => __( 'Create Venues', 'apollo-core' ),
			'edit_locals'                 => __( 'Edit Own Venues', 'apollo-core' ),
			'edit_others_locals'          => __( 'Edit Others\' Venues', 'apollo-core' ),
			'delete_locals'               => __( 'Delete Own Venues', 'apollo-core' ),
			'delete_others_locals'        => __( 'Delete Others\' Venues', 'apollo-core' ),

			// Classifieds permissions
			'read_classified'             => __( 'View Classifieds', 'apollo-core' ),
			'create_classifieds'          => __( 'Create Classifieds', 'apollo-core' ),
			'edit_classifieds'            => __( 'Edit Own Classifieds', 'apollo-core' ),
			'edit_others_classifieds'     => __( 'Edit Others\' Classifieds', 'apollo-core' ),
			'delete_classifieds'          => __( 'Delete Own Classifieds', 'apollo-core' ),
			'delete_others_classifieds'   => __( 'Delete Others\' Classifieds', 'apollo-core' ),

			// Groups permissions
			'read_apollo_group'           => __( 'View Groups', 'apollo-core' ),
			'create_apollo_groups'        => __( 'Create Groups', 'apollo-core' ),
			'edit_apollo_groups'          => __( 'Edit Own Groups', 'apollo-core' ),
			'edit_others_apollo_groups'   => __( 'Edit Others\' Groups', 'apollo-core' ),
			'delete_apollo_groups'        => __( 'Delete Own Groups', 'apollo-core' ),
			'delete_others_apollo_groups' => __( 'Delete Others\' Groups', 'apollo-core' ),
			'manage_apollo_group_members' => __( 'Manage Group Members', 'apollo-core' ),

			// Documents permissions
			'read_document'               => __( 'View Documents', 'apollo-core' ),
			'create_documents'            => __( 'Create Documents', 'apollo-core' ),
			'edit_documents'              => __( 'Edit Own Documents', 'apollo-core' ),
			'edit_others_documents'       => __( 'Edit Others\' Documents', 'apollo-core' ),
			'delete_documents'            => __( 'Delete Own Documents', 'apollo-core' ),
			'delete_others_documents'     => __( 'Delete Others\' Documents', 'apollo-core' ),
	...
	private static function render_emails_tab( $settings ) {
		...
		<div class="apollo-tab-content">
			...
			<div class="apollo-smtp-section" style="background: #fff; border: 1px solid var(--ap-border-color, #e0e0e0); border-radius: var(--ap-radius-md, 8px); padding: 24px; margin-bottom: 30px;">
				<h3 style="margin-top: 0; display: flex; align-items: center; gap: 8px;">
					<span class="dashicons dashicons-admin-settings" style="color: var(--ap-orange-500, #ff6925);"></span>
					<?php esc_html_e( 'SMTP Configuration', 'apollo-core' ); ?>
				</h3>
				<p class="description" style="margin-bottom: 20px;"><?php esc_html_e( 'Configure SMTP settings for reliable email delivery. Leave empty to use WordPress default mail settings.', 'apollo-core' ); ?></p>

				<?php
				$smtp_config = get_option( 'apollo_smtp_config', array() );
				$smtp_enabled = isset( $smtp_config['enabled'] ) ? $smtp_config['enabled'] : false;
				?>

				<table class="form-table" style="margin-bottom: 20px;">
					<tr>
						<th scope="row"><?php esc_html_e( 'Enable SMTP', 'apollo-core' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="apollo_smtp_config[enabled]" value="1" <?php checked( $smtp_enabled ); ?>>
								<?php esc_html_e( 'Use SMTP for sending emails', 'apollo-core' ); ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_host"><?php esc_html_e( 'SMTP Host', 'apollo-core' ); ?></label></th>
						<td>
							<input type="text" id="smtp_host" name="apollo_smtp_config[host]" value="<?php echo esc_attr( $smtp_config['host'] ?? '' ); ?>" class="regular-text">
							<p class="description"><?php esc_html_e( 'e.g., smtp.gmail.com, smtp.mailgun.org', 'apollo-core' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_port"><?php esc_html_e( 'SMTP Port', 'apollo-core' ); ?></label></th>
						<td>
							<input type="number" id="smtp_port" name="apollo_smtp_config[port]" value="<?php echo esc_attr( $smtp_config['port'] ?? '587' ); ?>" class="small-text">
							<p class="description"><?php esc_html_e( 'Common ports: 587 (TLS), 465 (SSL), 25 (non-encrypted)', 'apollo-core' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_encryption"><?php esc_html_e( 'Encryption', 'apollo-core' ); ?></label></th>
						<td>
							<select id="smtp_encryption" name="apollo_smtp_config[encryption]" class="regular-text">
								<option value="tls" <?php selected( $smtp_config['encryption'] ?? 'tls', 'tls' ); ?>><?php esc_html_e( 'TLS', 'apollo-core' ); ?></option>
								<option value="ssl" <?php selected( $smtp_config['encryption'] ?? 'tls', 'ssl' ); ?>><?php esc_html_e( 'SSL', 'apollo-core' ); ?></option>
								<option value="" <?php selected( $smtp_config['encryption'] ?? 'tls', '' ); ?>><?php esc_html_e( 'None', 'apollo-core' ); ?></option>
							</select>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_username"><?php esc_html_e( 'SMTP Username', 'apollo-core' ); ?></label></th>
						<td>
							<input type="text" id="smtp_username" name="apollo_smtp_config[username]" value="<?php echo esc_attr( $smtp_config['username'] ?? '' ); ?>" class="regular-text">
							<p class="description"><?php esc_html_e( 'Usually your email address', 'apollo-core' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_password"><?php esc_html_e( 'SMTP Password', 'apollo-core' ); ?></label></th>
						<td>
							<input type="password" id="smtp_password" name="apollo_smtp_config[password]" value="<?php echo esc_attr( $smtp_config['password'] ?? '' ); ?>" class="regular-text">
							<p class="description"><?php esc_html_e( 'Your email account password or app-specific password', 'apollo-core' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_from_name"><?php esc_html_e( 'From Name', 'apollo-core' ); ?></label></th>
						<td>
							<input type="text" id="smtp_from_name" name="apollo_smtp_config[from_name]" value="<?php echo esc_attr( $smtp_config['from_name'] ?? get_bloginfo( 'name' ) ); ?>" class="regular-text">
							<p class="description"><?php esc_html_e( 'Name that appears as the sender', 'apollo-core' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="smtp_from_email"><?php esc_html_e( 'From Email', 'apollo-core' ); ?></label></th>
						<td>
							<input type="email" id="smtp_from_email" name="apollo_smtp_config[from_email]" value="<?php echo esc_attr( $smtp_config['from_email'] ?? get_option( 'admin_email' ) ); ?>" class="regular-text">
							<p class="description"><?php esc_html_e( 'Email address that appears as the sender', 'apollo-core' ); ?></p>
						</td>
					</tr>
				</table>

				<div style="background: rgba(255, 105, 37, 0.1); border-left: 4px solid #ff6925; padding: 12px 16px; border-radius: 0 6px 6px 0;">
					<strong><?php esc_html_e( 'Security Note:', 'apollo-core' ); ?></strong>
					<?php esc_html_e( 'SMTP passwords are stored securely. For Gmail, use an App Password instead of your regular password.', 'apollo-core' ); ?>
				</div>
			</div>
	...
	private static function render_notifications_tab( $settings ) {
		...
		<div class="apollo-tab-content">
			...
			<div class="apollo-notification-composer" style="background: #fff; border: 1px solid var(--ap-border-color, #e0e0e0); border-radius: var(--ap-radius-md, 8px); padding: 24px; max-width: 700px;">
				...
				<div class="apollo-form-group" style="margin-bottom: 16px;">
					<label for="notification_icon" style="display: block; margin-bottom: 4px; font-weight: 500;"><?php esc_html_e( 'Icon', 'apollo-core' ); ?></label>
					<select id="notification_icon" class="regular-text" style="width: 100%;">
						<option value="ri-information-line"><?php esc_html_e( 'Info (Blue)', 'apollo-core' ); ?></option>
						<option value="ri-error-warning-line"><?php esc_html_e( 'Warning (Orange)', 'apollo-core' ); ?></option>
						<option value="ri-check-line"><?php esc_html_e( 'Success (Green)', 'apollo-core' ); ?></option>
						<option value="ri-close-circle-line"><?php esc_html_e( 'Error (Red)', 'apollo-core' ); ?></option>
						<option value="ri-notification-2-line"><?php esc_html_e( 'Bell (Purple)', 'apollo-core' ); ?></option>
						<option value="ri-star-line"><?php esc_html_e( 'Star (Yellow)', 'apollo-core' ); ?></option>
						<option value="ri-heart-line"><?php esc_html_e( 'Heart (Pink)', 'apollo-core' ); ?></option>
						<option value="ri-slack-line"><?php esc_html_e( 'Slack (Orange)', 'apollo-core' ); ?></option>
					</select>
				</div>

				<div class="apollo-form-group" style="margin-bottom: 16px;">
					<label for="notification_color" style="display: block; margin-bottom: 4px; font-weight: 500;"><?php esc_html_e( 'Color', 'apollo-core' ); ?></label>
					<select id="notification_color" class="regular-text" style="width: 100%;">
						<option value="bg-blue"><?php esc_html_e( 'Blue', 'apollo-core' ); ?></option>
						<option value="bg-orange"><?php esc_html_e( 'Orange', 'apollo-core' ); ?></option>
						<option value="bg-green"><?php esc_html_e( 'Green', 'apollo-core' ); ?></option>
						<option value="bg-red"><?php esc_html_e( 'Red', 'apollo-core' ); ?></option>
						<option value="bg-purple"><?php esc_html_e( 'Purple', 'apollo-core' ); ?></option>
						<option value="bg-yellow"><?php esc_html_e( 'Yellow', 'apollo-core' ); ?></option>
						<option value="bg-pink"><?php esc_html_e( 'Pink', 'apollo-core' ); ?></option>
						<option value="bg-gray"><?php esc_html_e( 'Gray', 'apollo-core' ); ?></option>
					</select>
				</div>
				...
				<div class="apollo-form-group" style="margin-bottom: 16px;">
					...
					<input type="text" id="notification_title" class="regular-text" style="width: 100%;" placeholder="apollo::rio" required>
				</div>
				...
				<div class="apollo-form-group" style="margin-bottom: 16px;">
					...
					<textarea id="notification_message" class="large-text" rows="5" style="width: 100%;" placeholder="Your notification message here..." required></textarea>
				</div>

				<div class="apollo-form-group" style="margin-bottom: 16px;">
					<label for="notification_url" style="display: block; margin-bottom: 4px; font-weight: 500;"><?php esc_html_e( 'URL (Optional)', 'apollo-core' ); ?></label>
					<input type="url" id="notification_url" class="regular-text" style="width: 100%;" placeholder="https://apollo.rio/...">
				</div>
			...
			jQuery(document).ready(function($) {
				...
				$('.apollo-send-notification').on('click', function() {
					var $button = $(this);
					var $result = $('#apollo-notification-result');
					var originalText = $button.html();

					// Get form data
					var data = {
						action: 'apollo_send_notification',
						nonce: '<?php echo esc_js( wp_create_nonce( 'apollo_admin_nonce' ) ); ?>',
						recipient: $('#notification_recipient').val(),
						user_id: $('#notification_user_id').val(),
						role: $('#notification_role').val(),
					...
					if (!data.title || !data.message) {
						$result.html('<span style="color: #dc3545;">Title and message are required.</span>');
						return;
					}

					// Disable button and show loading
					$button.prop('disabled', true).html('<span class="spinner is-active" style="float: none; margin-right: 4px;"></span> Sending...');
					$result.html('');
					...
					$.post(ajaxurl, data)
						.done(function(response) {
							if (response.success) {
								...
								// Clear form
								$('#notification_title, #notification_message, #notification_url').val('');
							} else {
								$result.html('<span style="color: #dc3545;">✗ ' + response.data.message + '</span>');
							}
						})
						.fail(function() {
							$result.html('<span style="color: #dc3545;">✗ Network error. Please try again.</span>');
						})
						.always(function() {
							// Re-enable button
	...
	public static function sanitize_page_access( $input ) {
		...
		foreach ( $input as $page_id => $rules ) {
			...
			$sanitized[ $page_id ] = array(
				'allowed'   => array(),
				'forbidden' => array(),
			);

			if ( isset( $rules['allowed'] ) && is_array( $rules['allowed'] ) ) {
				foreach ( $rules['allowed'] as $role_key => $value ) {
					if ( $value ) {
						$sanitized[ $page_id ]['allowed'][ sanitize_key( $role_key ) ] = true;
					}
				}
			}

			if ( isset( $rules['forbidden'] ) && is_array( $rules['forbidden'] ) ) {
				foreach ( $rules['forbidden'] as $role_key => $value ) {
					if ( $value ) {
						$sanitized[ $page_id ]['forbidden'][ sanitize_key( $role_key ) ] = true;
					}
				}
			}
		}
	...
	public static function ajax_send_notification() {
		...
		$icon      = isset( $_POST['icon'] ) ? sanitize_text_field( wp_unslash( $_POST['icon'] ) ) : 'ri-slack-line';
		$color     = isset( $_POST['color'] ) ? sanitize_text_field( wp_unslash( $_POST['color'] ) ) : 'bg-orange';
		...
		switch ( $recipient ) {
			case 'all':
				// Send to all users (frontend notification).
				if ( class_exists( 'Apollo_Notification_Manager' ) ) {
					$result = Apollo_Notification_Manager::send_to_all_users( $notification_data );
				}
				break;

			case 'specific':
				// Send to specific user.
				if ( $user_id && class_exists( 'Apollo_Notification_Manager' ) ) {
					$result = Apollo_Notification_Manager::send_to_user( $user_id, $notification_data );
				}
				break;

			case 'role':
				// Send to users with specific role.
				if ( $role && class_exists( 'Apollo_Notification_Manager' ) ) {
					$result = Apollo_Notification_Manager::send_to_role( $role, $notification_data );
				}
				break;
		}
	...
	public static function ajax_test_email() {
		...
		// Use CommunicationManager if available.
		if ( isset( $GLOBALS['apollo_communication_manager'] ) &&
			 method_exists( $GLOBALS['apollo_communication_manager'], 'get_email_manager' ) ) {
			$email_manager = $GLOBALS['apollo_communication_manager']->get_email_manager();
			if ( $email_manager ) {
				$result = $email_manager->send_test( $to, $type );
		...
		// Fallback to original method if CommunicationManager not available.
		$email_config = get_option( 'apollo_email_config', array() );
