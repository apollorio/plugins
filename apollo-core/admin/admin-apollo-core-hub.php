	...
	<div class="wrap apollo-hub-wrap">
		...
		<nav class="nav-tab-wrapper apollo-hub-tabs">
			...
			<a href="?page=apollo-core-hub&tab=rbac"
				class="nav-tab <?php echo esc_attr( $current_tab === 'rbac' ? 'nav-tab-active' : '' ); ?>">
				<span class="dashicons dashicons-groups"></span>
				<?php esc_html_e( 'Permissões Apollo', 'apollo-core' ); ?>
			</a>
			<a href="?page=apollo-core-hub&tab=audit"
				class="nav-tab <?php echo esc_attr( $current_tab === 'audit' ? 'nav-tab-active' : '' ); ?>">
				...
				<table class="widefat">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo esc_html( $field['required'] ? '✅' : '—' ); ?></td>
						</tr>
						<?php endforeach; ?>
					</tbody>
				</table>
			...
			<table class="widefat apollo-roles-table">
				...
				<tbody>
					...
					<tr>
						...
						<td class="<?php echo esc_attr( $info['admin'] ? 'cap-yes' : 'cap-no' ); ?>">
							<?php echo esc_html( $info['admin'] ? '✅' : '❌' ); ?>
						</td>
						<td class="<?php echo esc_attr( $info['apollo'] ? 'cap-yes' : 'cap-no' ); ?>">
							<?php echo esc_html( $info['apollo'] ? '✅' : '❌' ); ?>
						</td>
						<td class="<?php echo esc_attr( $info['cena'] ? 'cap-yes' : 'cap-no' ); ?>">
							<?php echo esc_html( $info['cena'] ? '✅' : '❌' ); ?>
						</td>
						<td class="<?php echo esc_attr( $info['subscriber'] ? 'cap-yes' : 'cap-no' ); ?>">
							<?php echo esc_html( $info['subscriber'] ? '✅' : '❌' ); ?>
						</td>
					</tr>
					<?php endforeach; ?>
				</tbody>
...
function apollo_core_render_settings_content(): void {
	...
	if ( isset( $_POST['apollo_core_settings_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['apollo_core_settings_nonce'] ) ), 'apollo_core_save_settings' ) ) {
		...
		update_option( 'apollo_core_cleanup_data_on_delete', isset( $_POST['cleanup_data_on_delete'] ) ? 1 : 0 );

		// Analytics settings.
		update_option( 'apollo_analytics_enabled', isset( $_POST['analytics_enabled'] ) ? 1 : 0 );
		update_option( 'apollo_analytics_heatmap_enabled', isset( $_POST['analytics_heatmap_enabled'] ) ? 1 : 0 );
		update_option( 'apollo_analytics_track_admins', isset( $_POST['analytics_track_admins'] ) ? 1 : 0 );
		update_option( 'apollo_analytics_gdpr_consent_required', isset( $_POST['analytics_gdpr_consent_required'] ) ? 1 : 0 );
		update_option( 'apollo_analytics_retention_days', absint( $_POST['analytics_retention_days'] ?? 365 ) );
	...
	$cleanup_data_on_delete = get_option( 'apollo_core_cleanup_data_on_delete', 0 );

	// Analytics settings.
	$analytics_enabled             = get_option( 'apollo_analytics_enabled', 1 );
	$analytics_heatmap_enabled     = get_option( 'apollo_analytics_heatmap_enabled', 0 );
	$analytics_track_admins        = get_option( 'apollo_analytics_track_admins', 0 );
	$analytics_gdpr_consent_required = get_option( 'apollo_analytics_gdpr_consent_required', 1 );
	$analytics_retention_days      = get_option( 'apollo_analytics_retention_days', 365 );
	?>
	<div class="apollo-hub-section">
		...
		<form method="post" action="">
			...
			<table class="form-table">
				...
				<tr>
					<th scope="row"><?php esc_html_e( 'Desinstalação', 'apollo-core' ); ?></th>
					<td>
						<fieldset>
							<label>
								<input type="checkbox" name="cleanup_data_on_delete" value="1"
										<?php checked( $cleanup_data_on_delete, 1 ); ?>>
								<?php esc_html_e( 'Excluir todos os dados ao desinstalar o plugin', 'apollo-core' ); ?>
							</label>
							<p class="description">
								<?php esc_html_e( 'Se marcado, todas as tabelas personalizadas, opções, metadados de posts/usuários e dados relacionados serão permanentemente excluídos quando você deletar este plugin. Caso contrário, os dados serão preservados.', 'apollo-core' ); ?>
							</p>
						</fieldset>
					</td>
				<tr>
					<th scope="row"><?php esc_html_e( 'Analytics', 'apollo-core' ); ?></th>
					<td>
						<fieldset>
							<label>
								<input type="checkbox" name="analytics_enabled" value="1"
										<?php checked( $analytics_enabled, 1 ); ?>>
								<?php esc_html_e( 'Habilitar Analytics', 'apollo-core' ); ?>
							</label>
							<br>
							<label>
								<input type="checkbox" name="analytics_heatmap_enabled" value="1"
										<?php checked( $analytics_heatmap_enabled, 1 ); ?>>
								<?php esc_html_e( 'Habilitar Heatmap Tracking', 'apollo-core' ); ?>
							</label>
							<br>
							<label>
								<input type="checkbox" name="analytics_track_admins" value="1"
										<?php checked( $analytics_track_admins, 1 ); ?>>
								<?php esc_html_e( 'Rastrear Administradores', 'apollo-core' ); ?>
							</label>
							<br>
							<label>
								<input type="checkbox" name="analytics_gdpr_consent_required" value="1"
										<?php checked( $analytics_gdpr_consent_required, 1 ); ?>>
								<?php esc_html_e( 'Exigir Consentimento GDPR para Analytics', 'apollo-core' ); ?>
							</label>
							<br>
							<label>
								<?php esc_html_e( 'Dias de Retenção de Dados:', 'apollo-core' ); ?>
								<input type="number" name="analytics_retention_days" value="<?php echo esc_attr( $analytics_retention_days ); ?>" min="30" max="2555" step="30">
								<?php esc_html_e( 'dias (0 = sem limite)', 'apollo-core' ); ?>
							</label>
						</fieldset>
						<p class="description">
							<?php esc_html_e( 'Configure o rastreamento de analytics. Dados são automaticamente anonimizados e sujeitos às leis de privacidade.', 'apollo-core' ); ?>
						</p>
					</td>
				</tr>
			</table>
...
function apollo_core_render_audit_content() {
	...
	require_once APOLLO_CORE_PLUGIN_DIR . 'includes/class-apollo-audit-log.php';
	...
	$entries = Apollo_Core\Audit_Log::get_entries( $args );
