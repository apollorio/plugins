    ...
    <div class="wrap apollo-hub-wrap">
        ...
        <div class="apollo-hub-content">
            <div class="apollo-hub-section">
                ...
                <?php
                global $wpdb;
                $expected = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->posts} p 
                    INNER JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id 
                    WHERE pm.meta_key = '_apollo_cena_status' AND pm.meta_value = 'expected'");
                $confirmed = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->posts} p 
                    INNER JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id 
...
function apollo_core_get_all_shortcodes(): array {
    return [
        'CENA-RIO' => [
            ['code' => '[apollo_cena_submit_event]', 'description' => 'Formulário de submissão CENA-RIO', 'attributes' => ''],
            ['code' => '[apollo_cena_moderation_queue]', 'description' => 'Queue de moderação CENA-RIO', 'attributes' => ''],
        ],
        'Documentos (Apollo Social)' => [
            ['code' => '[apollo_document_editor]', 'description' => 'Editor de documentos WYSIWYG', 'attributes' => 'doc_id'],
            ['code' => '[apollo_documents]', 'description' => 'Lista de documentos', 'attributes' => 'library'],
            ['code' => '[apollo_sign_document]', 'description' => 'Página de assinatura', 'attributes' => 'doc_id'],
            ['code' => '[apollo_verify_document]', 'description' => 'Verificar documento', 'attributes' => ''],
        ],
        'Social' => [
            ['code' => '[apollo_profile]', 'description' => 'Perfil do usuário', 'attributes' => 'user_id'],
            ['code' => '[apollo_profile_card]', 'description' => 'Card de perfil', 'attributes' => 'user_id'],
            ['code' => '[apollo_event_list]', 'description' => 'Lista de eventos social', 'attributes' => ''],
            ['code' => '[apollo_dj_contacts]', 'description' => 'Contatos de DJs', 'attributes' => ''],
        ],
    ];
}
...
function apollo_core_get_all_placeholders(): array {
    return [
        'Moderação' => [
            ['code' => '{mod_action}', 'description' => 'Ação de moderação', 'example' => 'approved/rejected'],
            ['code' => '{mod_reason}', 'description' => 'Motivo da ação', 'example' => 'Conteúdo aprovado'],
            ['code' => '{mod_actor}', 'description' => 'Moderador que agiu', 'example' => 'admin'],
            ['code' => '{mod_date}', 'description' => 'Data da ação', 'example' => '25/12/2025'],
        ],
        'Documentos' => [
            ...
            ['code' => '{doc_status}', 'description' => 'Status do documento', 'example' => 'signed'],
        ...
        'CENA-RIO' => [
            ['code' => '{cena_event_title}', 'description' => 'Título do evento', 'example' => 'Festa Techno'],
            ['code' => '{cena_status}', 'description' => 'Status interno', 'example' => 'expected/confirmed'],
            ['code' => '{cena_submitter}', 'description' => 'Quem submeteu', 'example' => 'user@email.com'],
        ],
    ];
}
...
function apollo_core_get_all_forms(): array {
    return [
        [
            ...
            'fields' => [
                ['label' => 'Título do Evento', 'type' => 'text', 'required' => true],
                ['label' => 'Data', 'type' => 'date', 'required' => true],
                ['label' => 'Local', 'type' => 'select', 'required' => true],
                ['label' => 'Descrição', 'type' => 'textarea', 'required' => false],
            ],
        ],
        [
        ...
        [
            ...
            'fields' => [
                ...
                ['label' => 'CPF', 'type' => 'cpf', 'required' => true],
                ['label' => 'Aceitar Termos', 'type' => 'checkbox', 'required' => true],
                ['label' => 'Assinatura', 'type' => 'canvas', 'required' => true],
            ],
        ],
        [
    ...
    return [
        'custom_roles' => [
            [
                ...
                'caps' => ['view_moderation_queue', 'approve_content', 'reject_content', 'send_mod_notifications'],
            ],
            [
                'name' => 'CENA-RIO User',
                'slug' => 'cena_role',
        ...
        'capabilities' => [
            'view_moderation_queue' => [
                ...
                'admin' => true, 'apollo' => true, 'cena' => false, 'subscriber' => false,
            ],
            'approve_content' => [
            ...
            'suspend_users' => [
                ...
                'admin' => true, 'apollo' => false, 'cena' => false, 'subscriber' => false,
            ],
            'submit_cena_events' => [
                'label' => 'Submeter Eventos CENA-RIO',
                'admin' => true, 'apollo' => false, 'cena' => true, 'subscriber' => false,
            ],
            'view_cena_calendar' => [
                'label' => 'Ver Calendário CENA-RIO',
                'admin' => true, 'apollo' => true, 'cena' => true, 'subscriber' => false,
            ],
            'confirm_cena_events' => [
...
function apollo_core_get_all_metakeys(): array {
    return [
        'Moderação' => [
            ['key' => '_apollo_mod_status', 'type' => 'string', 'description' => 'Status de moderação'],
            ['key' => '_apollo_mod_actor', 'type' => 'int', 'description' => 'ID do moderador'],
            ['key' => '_apollo_mod_date', 'type' => 'datetime', 'description' => 'Data da ação'],
            ['key' => '_apollo_mod_reason', 'type' => 'text', 'description' => 'Motivo da ação'],
        ],
        'CENA-RIO' => [
            ['key' => '_apollo_cena_status', 'type' => 'string', 'description' => 'Status interno (expected/confirmed)'],
            ['key' => '_apollo_cena_submitter', 'type' => 'int', 'description' => 'ID do submissor'],
            ['key' => '_apollo_cena_confirmed_by', 'type' => 'int', 'description' => 'ID de quem confirmou'],
            ['key' => '_apollo_cena_confirmed_at', 'type' => 'datetime', 'description' => 'Data de confirmação'],
        ],
        'Documentos' => [
            ['key' => '_apollo_doc_protocol', 'type' => 'string', 'description' => 'Código do protocolo'],
            ['key' => '_apollo_doc_hash', 'type' => 'string', 'description' => 'Hash SHA-256'],
        ...
        'Assinaturas' => [
            ...
            ['key' => '_apollo_sig_timestamp', 'type' => 'datetime', 'description' => 'Data/hora da assinatura'],
            ['key' => '_apollo_sig_ip', 'type' => 'string', 'description' => 'IP do assinante'],
        ],
        'Usuário' => [
            ['key' => '_apollo_membership_type', 'type' => 'string', 'description' => 'Tipo de membership'],
            ['key' => '_apollo_quiz_completed', 'type' => 'bool', 'description' => 'Quiz completado'],
            ['key' => '_apollo_quiz_score', 'type' => 'int', 'description' => 'Pontuação do quiz'],
        ],
    ];
}
