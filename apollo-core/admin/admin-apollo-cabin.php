...
function apollo_admin_cabin_handle_submissions(): void {
	...
	if ( isset( $_POST['apollo_cabin_modules_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['apollo_cabin_modules_nonce'] ) ), 'apollo_cabin_modules' ) ) {
		$modules   = array();
		$available = \Apollo_Core\Modules_Config::get_available_modules();

		foreach ( $available as $module ) {
			$modules[ $module ] = isset( $_POST['modules'][ $module ] );
		}

		\Apollo_Core\Modules_Config::update_modules( $modules, get_current_user_id() );
	...
	if ( isset( $_POST['apollo_cabin_limits_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['apollo_cabin_limits_nonce'] ) ), 'apollo_cabin_limits' ) ) {
		...
		if ( isset( $_POST['limits'] ) && is_array( $_POST['limits'] ) ) {
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Values are sanitized with absint() in the loop below.
			$limits_input = array_map( 'sanitize_text_field', wp_unslash( $_POST['limits'] ) );
			foreach ( $limits_input as $key => $value ) {
				$limits[ sanitize_key( $key ) ] = absint( $value );
			}
		}

		Apollo_Core\Modules_Config::update_limits( $limits, get_current_user_id() );
	...
	if ( isset( $_POST['apollo_cabin_mod_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['apollo_cabin_mod_nonce'] ) ), 'apollo_cabin_mod' ) ) {
		...
		if ( $user_id > 0 && class_exists( 'Apollo_Core\User_Moderation' ) ) {
			Apollo_Core\User_Moderation::set_mod_level( $user_id, $level, get_current_user_id() );
	...
	if ( isset( $_POST['apollo_cabin_ip_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['apollo_cabin_ip_nonce'] ) ), 'apollo_cabin_ip' ) ) {
		...
		if ( $ip && class_exists( 'Apollo_Core\User_Moderation' ) ) {
			$result = Apollo_Core\User_Moderation::block_ip( $ip, get_current_user_id(), $reason );
			if ( $result ) {
...
function apollo_admin_cabin_render_page(): void {
	...
	$modules = \Apollo_Core\Modules_Config::get_modules();
...
function apollo_admin_cabin_render_limits(): void {
	...
	$limits   = \Apollo_Core\Modules_Config::get_limits();
	$defaults = \Apollo_Core\Modules_Config::get_default_limits();
				...
				<table class="wp-list-table widefat fixed striped">
					...
					<tbody>
						<?php foreach ( $mod_users as $mod ) : ?>
							<?php
							$level  = Apollo_Core\User_Moderation::get_mod_level( $mod->ID );
							$status = Apollo_Core\User_Moderation::get_user_status( $mod->ID );
							?>
							<tr>
								<td>
...
function apollo_admin_cabin_render_security(): void {
	...
	$blocklist = Apollo_Core\User_Moderation::get_ip_blocklist();
	?>
...
function apollo_admin_cabin_render_logs(): void {
	...
	// Verify nonce for filters.
	if ( ! isset( $_GET['apollo_cabin_logs_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['apollo_cabin_logs_nonce'] ) ), 'apollo_cabin_logs_filter' ) ) {
		// If nonce is invalid or missing, reset filters to defaults.
		$filter_action = '';
		$filter_actor  = 0;
	} else {
		// Get filters.
	...
	// Get logs.
	if ( function_exists( 'Apollo_Core\apollo_get_mod_log' ) ) {
		$logs = Apollo_Core\apollo_get_mod_log( $args );
	} else {
		$logs = array();
	}
	...
	<div class="apollo-cabin-logs">
		...
		<div class="log-filters">
			<form method="get">
				...
				<?php wp_nonce_field( 'apollo_cabin_logs_filter', 'apollo_cabin_logs_nonce' ); ?>
