...
final class PerformanceModule {
	...
	public function init(): void {
		...
		add_action( 'wp_enqueue_scripts', [ $this, 'minifyAndCacheAssets' ], 999 );
		add_action( 'admin_enqueue_scripts', [ $this, 'minifyAndCacheAssets' ], 999 );

		// Non-blocking script loading
		add_filter( 'script_loader_tag', [ $this, 'addAsyncDeferAttributes' ], 10, 3 );

		// Aggressive caching headers
		add_action( 'send_headers', [ $this, 'setAggressiveCaching' ] );

		// Remove query strings from static resources
		add_filter( 'script_loader_src', [ $this, 'removeQueryStrings' ], 999 );
		add_filter( 'style_loader_src', [ $this, 'removeQueryStrings' ], 999 );

		// Optimize database queries
		add_action( 'wp_loaded', [ $this, 'optimizeQueries' ] );
	...
	public function addAsyncDeferAttributes( string $tag, string $handle, string $src ): string {
		...
		if ( strpos( $handle, 'apollo' ) === false && ! in_array( $handle, [ 'jquery', 'jquery-core' ] ) ) {
			return $tag;
		}
