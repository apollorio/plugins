...
final class SEOModule {
	...
	public function init(): void {
		...
		add_action( 'wp_head', array( $this, 'injectSchemaJsonLd' ), 1 );

		// Dynamic schema updates via MutationObserver
		add_action( 'wp_footer', array( $this, 'injectMutationObserverScript' ), 999 );

		// Meta tags optimization
		add_action( 'wp_head', array( $this, 'optimizeMetaTags' ), 0 );

		// Sitemap generation
		add_action( 'init', array( $this, 'registerSitemapRoutes' ) );
	...
	private function getSocialProfiles(): array {
		$profiles       = array();
		$social_options = get_option( 'apollo_social_profiles', array() );
