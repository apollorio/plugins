...
class Apollo_GONO_Checklist {
	...
	private function check_rest_routes(): void {
		...
		foreach ( $routes as $route => $handlers ) {
			...
			foreach ( $handlers as $handler ) {
				...
				$has_perm = isset( $handler['permission_callback'] ) && null !== $handler['permission_callback'];
	...
	private function check_modules_config(): void {
		...
		if ( class_exists( '\Apollo_Core\Modules_Config' ) ) {
			$this->pass( 'Classe \Apollo_Core\Modules_Config existe' );
			...
			$this->fail( 'Classe \Apollo_Core\Modules_Config não existe' );
		}
	...
	private function check_admin_pages(): void {
		...
		if ( is_array( $menu ) ) {
			foreach ( $menu as $item ) {
				if ( isset( $item[2] ) && 'apollo-cabin' === $item[2] ) {
					$cabin_found = true;
