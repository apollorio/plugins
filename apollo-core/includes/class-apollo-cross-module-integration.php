 ...
 */
class Cross_Module_Integration {
	...
	public static function init(): void {
		...
		add_action( 'publish_event_listing', array( self::class, 'maybe_create_event_social_post' ), 10, 2 );

		// Filter explore endpoint for bolha priority.
		add_filter( 'apollo_explore_query_args', array( self::class, 'prioritize_bolha_content' ), 10, 2 );

		// Add "Eventos que vou" filter to feed.
		add_filter( 'apollo_feed_filters', array( self::class, 'add_events_attending_filter' ) );

		// Hook into bolha changes for notifications.
		add_action( 'apollo_user_added_to_bolha', array( self::class, 'notify_bolha_accepted' ), 10, 2 );
		add_action( 'apollo_bolha_invite_sent', array( self::class, 'notify_bolha_invite' ), 10, 2 );

		// Hook into event RSVP for notifications.
		add_action( 'apollo_event_rsvp_added', array( self::class, 'notify_event_rsvp' ), 10, 3 );

		// Check module enabled before each feature.
		add_filter( 'apollo_can_create_event', array( self::class, 'check_events_module' ) );
		add_filter( 'apollo_can_create_post', array( self::class, 'check_social_module' ) );
		add_filter( 'apollo_can_use_bolha', array( self::class, 'check_bolha_module' ) );
		add_filter( 'apollo_can_create_comuna', array( self::class, 'check_comunas_module' ) );

		// Integrate events into comuna pages.
		add_filter( 'apollo_comuna_sidebar_widgets', array( self::class, 'add_comuna_events_widget' ) );

		// REST API filters.
		add_action( 'rest_api_init', array( self::class, 'register_integration_routes' ) );
	}
...
add_action( 'plugins_loaded', array( Cross_Module_Integration::class, 'init' ), 15 );
