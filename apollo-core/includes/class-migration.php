 ...
 * Migration Script
 *
 * Handles migration of options and meta from legacy Apollo plugins.
 *
 * @package Apollo_Core
 * @since 3.0.0
 */
...
class Migration {
	 ...
	 */
	private static array $option_mappings = array(
	 ...
	 * @var array<string, string>
	 */
	private static array $meta_mappings = array(
	 ...
	 * @return void
	 */
	private static function create_backup(): void {
		$backup_data = array(
			'options' => array(),
	 ...
	 */
	private static function migrate_meta(): int {
		global $wpdb;
		$migrated = 0;
		...
		foreach ( self::$meta_mappings as $old_key => $new_key ) {
			...
			foreach ( $results as $row ) {
				...
				if ( empty( $existing ) ) {
					// Add new meta.
					update_post_meta( (int) $row->post_id, $new_key, $row->meta_value );
					++$migrated;
				}
			}
		}//end foreach
...
class_alias( \Apollo_Core\Migration::class, 'Apollo_Core_Migration' );
