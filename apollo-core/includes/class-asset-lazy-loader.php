...
class Asset_Lazy_Loader {
	...
	private static function should_lazy_load_script( $script ): bool {
		...
		// Check file size if available (must be a valid URL string, not true/false).
	 ...
	 * DISABLED: get_headers() causes massive lag with many assets.
	 * Always returns 0 to skip file size checks - use handle patterns instead.
	 */
	private static function get_remote_file_size( string $url ): int {
		// Return 0 immediately to avoid network requests that cause lag.
		// Lazy loading decisions are now based on handle patterns only.
		return 0;
	}
