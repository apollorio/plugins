 ...
 * Route: /apollo/v1/mod/aprovar, /users/suspender, /users/bloquear
 ...
 *
 * DEPRECATION NOTICE (2024-12-11):
 * - users/suspender is DEPRECATED → Use mod/suspender (class-apollo-user-moderation.php)
 * - users/bloquear is DEPRECATED → Use mod/ban (class-apollo-user-moderation.php)
 * These routes are kept for backward compatibility but forward to the main implementation.
 */
...
/**
 * Register mod REST routes (shortened to /mod)
 *
 * NOTE: users/suspender and users/bloquear are DEPRECATED legacy aliases.
 * Primary routes are in class-apollo-user-moderation.php:
 * - POST mod/suspender
 * - POST mod/unsuspend
 * - POST mod/ban
 */
function apollo_register_mod_rest_routes() {
	// Approve content endpoint - UNIQUE to this file (different from /mod/aprovar).
	register_rest_route(
		'apollo/v1',
		'mod/aprovar',
		array(
	...
	// @deprecated 3.0.0 Use mod/suspender instead.
	register_rest_route(
		'apollo/v1',
		'users/suspender',
		array(
	...
	// @deprecated 3.0.0 Use mod/ban instead.
	register_rest_route(
		'apollo/v1',
		'users/bloquear',
		array(
...
function apollo_rest_suspend_user( $request ) {
	// Forward to Apollo_User_Moderation if available (preferred implementation).
	if ( class_exists( 'Apollo_User_Moderation' ) && method_exists( 'Apollo_User_Moderation', 'rest_suspend_user' ) ) {
		return Apollo_User_Moderation::rest_suspend_user( $request );
	}
...
function apollo_rest_block_user( $request ) {
	...
	if ( class_exists( 'Apollo_User_Moderation' ) && method_exists( 'Apollo_User_Moderation', 'rest_ban_user' ) ) {
		return Apollo_User_Moderation::rest_ban_user( $request );
	}
