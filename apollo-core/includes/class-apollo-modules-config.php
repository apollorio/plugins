...
namespace {
	...
	function apollo_is_module_enabled( string $module ): bool {
		return \Apollo_Core\Modules_Config::is_module_enabled( $module );
	}
	...
	function apollo_get_limit( string $key, int $default = 0 ): int {
		return \Apollo_Core\Modules_Config::get_limit( $key, $default );
	}
	...
	function apollo_check_limit( int $user_id, string $resource ): array {
		return \Apollo_Core\Modules_Config::can_create_resource( $user_id, $resource );
	}
	...
	function apollo_has_reached_limit( int $user_id, string $limit_key, int $current ): bool {
		return \Apollo_Core\Modules_Config::has_reached_limit( $user_id, $limit_key, $current );
	}
