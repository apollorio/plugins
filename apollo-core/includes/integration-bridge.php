...
// ============================================================================.
// CORE PROTECTION - Ensure WordPress core functions are available.
// ============================================================================.

// Load core function checker to verify WordPress core is loaded correctly.
require_once __DIR__ . '/core-function-checker.php';

// Verify WordPress core is loaded before proceeding.
if ( ! apollo_verify_wp_core_loaded() ) {
	// WordPress core not fully loaded - log error and prevent plugin initialization.
	if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
		// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug logging.
		error_log( 'Apollo Core: WordPress core not fully loaded. Plugin initialization prevented.' );
	}
	return;
}
...
if ( ! function_exists( 'apollo_get_local_asset_url' ) ) {
	/**
	 * Get Apollo local asset URL (fallback source)
	 *
	 * @param string $asset Asset filename.
	 * @return string Full URL to local asset.
	 */
	function apollo_get_local_asset_url( string $asset ): string {
		$base = defined( 'APOLLO_CORE_PLUGIN_URL' )
...
if ( ! function_exists( 'apollo_render_css_with_fallback' ) ) {
	/**
	 * Render CSS link tag with CDN and local fallback
	 *
	 * @param string $file     CSS file path (relative to assets/).
	 * @param string $id       Optional ID attribute.
	 * @param bool   $echo     Whether to echo or return.
	 * @return string HTML link tag.
	 */
	function apollo_render_css_with_fallback( string $file, string $id = '', bool $echo = true ): string {
		$cdn   = esc_url( apollo_get_asset_url( $file ) );
		$local = esc_url( apollo_get_local_asset_url( 'core/' . $file ) );
		$id    = $id ? ' id="' . esc_attr( $id ) . '"' : '';
		$html  = '<link rel="stylesheet"' . $id . ' href="' . $cdn . '" onerror="this.onerror=null;this.href=\'' . $local . '\'">';
		if ( $echo ) {
			echo $html;
		}
		return $html;
...
if ( ! function_exists( 'apollo_render_js_with_fallback' ) ) {
	/**
	 * Render JS script tag with CDN and local fallback
	 *
	 * @param string $file     JS file path (relative to assets/).
	 * @param string $id       Optional ID attribute.
	 * @param bool   $defer    Whether to defer loading.
	 * @param bool   $echo     Whether to echo or return.
	 * @return string HTML script tag.
	 */
	function apollo_render_js_with_fallback( string $file, string $id = '', bool $defer = false, bool $echo = true ): string {
		$cdn   = esc_url( apollo_get_asset_url( $file ) );
		$local = esc_url( apollo_get_local_asset_url( 'core/' . $file ) );
		$id    = $id ? ' id="' . esc_attr( $id ) . '"' : '';
		$d     = $defer ? ' defer' : '';
		$html  = '<script' . $id . $d . ' src="' . $cdn . '" onerror="this.onerror=null;this.src=\'' . $local . '\'"></script>';
		if ( $echo ) {
			echo $html;
		}
		return $html;
...
if ( ! function_exists( 'apollo_enqueue_canvas_assets' ) ) {
	/**
	 * Enqueue Canvas mode assets via Apollo CDN
	 *
	 * Uses the centralized \Apollo_Core\Global_Assets class for consistent asset management.
	 * All CSS/JS now comes from Apollo CDN: https://assets.apollo.rio.br/
	 *
	 * @return void
	 */
	function apollo_enqueue_canvas_assets(): void {
		// Use the Global Assets Manager if available.
		if ( class_exists( '\Apollo_Core\Global_Assets' ) ) {
			\Apollo_Core\Global_Assets::enqueue_all();
		} else {
...
if ( ! function_exists( 'apollo_enqueue_global_assets' ) ) {
	...
	function apollo_enqueue_global_assets( string $scope = 'all' ): void {
		...
		switch ( $scope ) {
			case 'css':
				\Apollo_Core\Global_Assets::enqueue_css();

				break;
			case 'js':
				\Apollo_Core\Global_Assets::enqueue_js();
...
if ( ! function_exists( 'apollo_is_using_cdn' ) ) {
	...
	function apollo_is_using_cdn(): bool {
		if ( class_exists( '\Apollo_Core\Global_Assets' ) ) {
			return \Apollo_Core\Global_Assets::is_using_cdn();
		}
...
if ( ! function_exists( 'apollo_set_use_cdn' ) ) {
	...
	function apollo_set_use_cdn( bool $use_cdn ): void {
		if ( class_exists( '\Apollo_Core\Global_Assets' ) ) {
			\Apollo_Core\Global_Assets::set_use_cdn( $use_cdn );
		}
	}
}
...
if ( ! function_exists( 'apollo_get_dj_data' ) ) {
	...
	function apollo_get_dj_data( int $dj_id ): array {
		...
		$post = get_post( $dj_id );
		if ( ! $post || 'event_dj' !== $post->post_type ) {
			return array();
		}
...
if ( ! function_exists( 'apollo_get_local_data' ) ) {
	...
	function apollo_get_local_data( int $local_id ): array {
		...
		if ( ! $post || 'event_local' !== $post->post_type ) {
			return array();
		}
...
add_action(
	...
	function ( $user_id, $memberships, $admin_id ) {
		...
		if ( in_array( 'dj_professional', $memberships, true ) || in_array( 'dj_amateur', $memberships, true ) ) {
			do_action( 'apollo_dj_membership_approved', $user_id, $memberships );
		}

		if ( in_array( 'event_producer_professional', $memberships, true ) || in_array( 'event_producer_active', $memberships, true ) ) {
