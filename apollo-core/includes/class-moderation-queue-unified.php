 ...
 * and connects them to the moderation system.
 *
 * @package Apollo_Core
 * @since 3.1.0
 */
...
class Apollo_Moderation_Queue_Unified {
	...
	public static function get_pending_count(): int {
		$cached = get_transient( 'apollo_pending_moderation_count' );
		if ( false !== $cached ) {
			return (int) $cached;
		}
		...
		set_transient( 'apollo_pending_moderation_count', $count, 5 * MINUTE_IN_SECONDS );

		return $count;
	}

	/**
	 * Get CPTs that require moderation
	 *
	 * @return array Array of post type slugs.
	 */
	public static function get_moderation_cpts(): array {
		$cpts = self::$moderation_cpts;

		// Allow filtering
		$cpts = apply_filters( 'apollo_moderation_cpts', $cpts );
	...
	public static function add_pending_count_badge(): void {
		...
		// Find the moderation menu item and add badge
		foreach ( $menu as $key => $item ) {
			if ( isset( $item[2] ) && 'apollo-moderation' === $item[2] ) {
				$menu[ $key ][0] = sprintf(
					...
					$count,
					sprintf(
						/* translators: %d: number of items pending moderation */
						_n( '%d item pending moderation', '%d items pending moderation', $count, 'apollo-core' ),
						$count
					)
				);
				break;
	...
	public static function clear_pending_cache(): void {
		delete_transient( 'apollo_pending_moderation_count' );
	}
	...
	public static function on_post_save( int $post_id, WP_Post $post, bool $update ): void {
		...
		if ( in_array( $post->post_status, array( 'pending', 'draft' ), true )
			...
			if ( ! get_post_meta( $post_id, '_apollo_submitted_for_moderation', true ) ) {
				update_post_meta( $post_id, '_apollo_submitted_for_moderation', current_time( 'mysql' ) );
				update_post_meta( $post_id, '_apollo_submitted_by', $post->post_author );
			}

			// Trigger action for notifications
			do_action( 'apollo_post_pending_moderation', $post_id, $post );
		}
	}
