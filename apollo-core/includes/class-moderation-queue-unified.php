 ...
 * Route: /apollo/v1/mod/em-fila, /mod/em-fila-count
 ...
 */
class Moderation_Queue_Unified {
	...
	public static function rest_get_unified_queue( WP_REST_Request $request ): WP_REST_Response {
		...
		foreach ( $query->posts as $post ) {
			...
			$items[] = array(
				'id'          => $post->ID,
				'title'       => ! empty( $post->post_title ) ? $post->post_title : __( '(sem título)', 'apollo-core' ),
				'type'        => $post->post_type,
				...
				'date'        => $post->post_date,
				'date_human'  => human_time_diff( strtotime( $post->post_date ), time() ) . ' ' . __( 'atrás', 'apollo-core' ),
				'edit_link'   => get_edit_post_link( $post->ID, 'raw' ),
				'thumbnail'   => get_the_post_thumbnail_url( $post->ID, 'thumbnail' ) ? get_the_post_thumbnail_url( $post->ID, 'thumbnail' ) : '',
				'source'      => ! empty( $source ) ? $source : 'wordpress',
