		...
		if ( empty( $keys['private'] ) || empty( $subscriptions ) ) {
		   ...
		   foreach ( $subscriptions as $id => $subscription ) {
			   ...
			   if ( $result === true ) {
				   ...
				   ++$failed;
				   if (!$error_message && is_string($result)) {
				...
				<h2><?php esc_html_e( 'Send Test Notification', 'apollo-core' ); ?></h2>
				   <form method="post" action="" enctype="multipart/form-data">
					   ...
					   <table class="form-table">
						   ...
						   <tr>
							   <th><label for="test_icon">Ícone</label></th>
							   <td>
								   <input type="url" id="test_icon" name="test_icon" class="regular-text" placeholder="URL do ícone (PNG, 192x192)">
								   <p class="description">Deixe em branco para usar o ícone padrão do site.</p>
							   </td>
						   </tr>
					   </table>
					   <p class="submit">
				   ...
				   if ( isset( $_POST['send_test_push'] ) && check_admin_referer( 'apollo_test_push' ) ) {
					   ...
					   $icon   = esc_url_raw( $_POST['test_icon'] ?? '' );
					   $options = array();
					   if ( $icon ) {
						   $options['icon'] = $icon;
					   }
					   $result = self::send_notification( $title, $body, '', $options );
					   if ( isset($result['error']) && $result['error'] ) {
						   echo '<div class="notice notice-error"><p>' . esc_html__('Erro ao enviar notificação: ', 'apollo-core') . esc_html($result['error']) . '</p></div>';
					   } else {
