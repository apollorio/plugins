...
function apollo_register_quiz_rest_routes() {
	// POST quiz/tentativa - Record a quiz attempt.
	register_rest_route(
		'apollo/v1',
		'quiz/tentativa',
		array(
	...
	register_rest_route(
		...
		array(
			...
			'permission_callback' => function ( $request ) {
				$rate = \Apollo\Core\Security\RestSecurity::rateLimit( $request, 10, '_quiz_stats' );
				if ( is_wp_error( $rate ) ) {
					return $rate;
				}
	...
	register_rest_route(
		...
		array(
			...
			'permission_callback' => function ( $request ) {
				$rate = \Apollo\Core\Security\RestSecurity::rateLimit( $request, 10, '_quiz_user' );
				if ( is_wp_error( $rate ) ) {
					return $rate;
				}
 ...
 */
function apollo_rest_quiz_attempt_permission( $request ) {
	$rate = \Apollo\Core\Security\RestSecurity::rateLimit( $request, 10, '_quiz_attempt' );
	if ( is_wp_error( $rate ) ) {
		return $rate;
	}
