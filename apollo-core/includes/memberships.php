...
namespace Apollo_Core {
	 ...
	 */
	function get_default_memberships(): array {
		return array(
	 ...
	 */
	function save_memberships( array $memberships ): bool {
		// Validate structure.
	 ...
	 */
	function get_user_membership( int $user_id ): string {
	 ...
	 * @return bool True on success, false on failure.
	 */
	function set_user_membership( int $user_id, string $membership_slug, ?int $actor_id = null ): bool {
	 ...
	 */
	function init_memberships_option() {
	 ...
	 * This should be called during plugin activation
	 */
	function assign_default_memberships() {
	 ...
	 * @param int $user_id User ID.
	 */
	function assign_membership_on_registration( $user_id ) {
		$membership = get_user_meta( $user_id, '_apollo_membership', true );
	 ...
	 */
	function get_membership_data( $slug ) {
		$memberships = apollo_get_memberships();
	 ...
	 */
	function membership_exists( $slug ) {
		$memberships = apollo_get_memberships();
	 ...
	 * @return bool True on success, false on failure.
	 */
	function delete_membership( $slug ) {
	 ...
	 */
	function export_memberships_json() {
	 ...
	 */
	function import_memberships_json( $json ) {
		...
		// Validate and save.
		$result = save_memberships( $data['memberships'] );
...
} // End namespace Apollo_Core.

namespace {
	/**
	 * Backward compatibility shim
	 */
	if ( ! function_exists( 'apollo_get_default_memberships' ) ) {
		function apollo_get_default_memberships(): array {
			return Apollo_Core\get_default_memberships();
		}
	}

	if ( ! function_exists( 'apollo_get_memberships' ) ) {
		function apollo_get_memberships(): array {
			return Apollo_Core\get_memberships();
		}
	}

	if ( ! function_exists( 'apollo_save_memberships' ) ) {
		function apollo_save_memberships( array $memberships ): bool {
			return Apollo_Core\save_memberships( $memberships );
		}
	}

	if ( ! function_exists( 'apollo_get_user_membership' ) ) {
		function apollo_get_user_membership( int $user_id ): string {
			return Apollo_Core\get_user_membership( $user_id );
		}
	}

	if ( ! function_exists( 'apollo_set_user_membership' ) ) {
		function apollo_set_user_membership( int $user_id, string $membership_slug, ?int $actor_id = null ): bool {
			return Apollo_Core\set_user_membership( $user_id, $membership_slug, $actor_id );
		}
	}

	if ( ! function_exists( 'apollo_init_memberships_option' ) ) {
		function apollo_init_memberships_option() {
			return Apollo_Core\init_memberships_option();
		}
	}

	if ( ! function_exists( 'apollo_assign_default_memberships' ) ) {
		function apollo_assign_default_memberships() {
			return Apollo_Core\assign_default_memberships();
		}
	}

	if ( ! function_exists( 'apollo_assign_membership_on_registration' ) ) {
		function apollo_assign_membership_on_registration( $user_id ) {
			return Apollo_Core\assign_membership_on_registration( $user_id );
		}
	}

	if ( ! function_exists( 'apollo_get_membership_data' ) ) {
		function apollo_get_membership_data( $slug ) {
			return Apollo_Core\get_membership_data( $slug );
		}
	}

	if ( ! function_exists( 'apollo_membership_exists' ) ) {
		function apollo_membership_exists( $slug ) {
			return Apollo_Core\membership_exists( $slug );
		}
	}

	if ( ! function_exists( 'apollo_delete_membership' ) ) {
		function apollo_delete_membership( $slug ) {
			return Apollo_Core\delete_membership( $slug );
		}
	}

	if ( ! function_exists( 'apollo_export_memberships_json' ) ) {
		function apollo_export_memberships_json() {
			return Apollo_Core\export_memberships_json();
		}
	}

	if ( ! function_exists( 'apollo_import_memberships_json' ) ) {
		function apollo_import_memberships_json( $json ) {
			return Apollo_Core\import_memberships_json( $json );
		}
	}
}
