	...
	>
		...
		<?php foreach ( $schema as $field ) : ?>
			<?php if ( $field['visible'] ) : ?>
				<?php
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Output is escaped within apollo_render_field.
				echo apollo_render_field( $field, $args['values'] );
				?>
			<?php endif; ?>
		<?php endforeach; ?>

		<div class="apollo-form-submit">
			<button type="submit" class="apollo-button apollo-button-primary" title="Enviar formulário">
...
function apollo_render_field( $field, $values = array() ) {
	...
	$field_title   = isset( $field['label'] ) ? $field['label'] : '';
		...
		switch ( $field['type'] ) {
			case 'textarea':
				...
				<textarea
					...
					title="<?php echo esc_attr( $field_title ); ?>"
			...
			case 'checkbox':
				...
				if ( empty( $options ) ) {
					// Single checkbox.
					?>
				<label class="apollo-checkbox-label" for="apollo-field-<?php echo esc_attr( $field['key'] ); ?>">
					<input
						type="checkbox"
						id="apollo-field-<?php echo esc_attr( $field['key'] ); ?>"
						name="<?php echo esc_attr( $field['key'] ); ?>"
				...
				<div class="apollo-checkbox-group">
					<?php foreach ( $options as $option_value => $option_label ) : ?>
						<?php $option_id = 'apollo-field-' . $field['key'] . '-' . sanitize_key( (string) $option_value ); ?>
						<label class="apollo-checkbox-label" for="<?php echo esc_attr( $option_id ); ?>">
							<input
								type="checkbox"
			...
			<div class="apollo-radio-group">
				<?php foreach ( $options as $option_value => $option_label ) : ?>
					<?php $option_id = 'apollo-field-' . $field['key'] . '-' . sanitize_key( (string) $option_value ); ?>
					<label class="apollo-radio-label" for="<?php echo esc_attr( $option_id ); ?>">
						<input
							type="radio"
							id="<?php echo esc_attr( $option_id ); ?>"
							name="<?php echo esc_attr( $field['key'] ); ?>"
...
function apollo_save_user_instagram_on_register( $user_id ) {
	// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified by WordPress core during user registration.
...
function apollo_save_user_instagram_on_profile_update( $user_id ) {
	// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified by WordPress core during profile update.
			...
			<td>
				<div style="display: flex; align-items: center;">
					...
					<input
						...
						class="regular-text"
						title="<?php esc_attr_e( 'Instagram ID', 'apollo-core' ); ?>"
						aria-required="false"
						pattern="[A-Za-z0-9_]{1,30}"
						maxlength="30"
					>
...
function apollo_display_user_instagram( $user_id ) {
	...
	$instagram_url = 'https://instagram.com/' . rawurlencode( $instagram_id );
