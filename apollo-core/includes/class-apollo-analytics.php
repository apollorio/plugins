...
class Analytics {
	...
	public static function init(): void {
		...
		add_action( 'apollo_analytics_cleanup_old_data', array( __CLASS__, 'run_data_cleanup' ) );
		if ( ! wp_next_scheduled( 'apollo_analytics_cleanup_old_data' ) ) {
			wp_schedule_event( strtotime( 'tomorrow 2:00am' ), 'daily', 'apollo_analytics_cleanup_old_data' );
		}
	...
	public static function ajax_track_pageview(): void {
		// Security: Verify nonce for all requests.
		check_ajax_referer( 'apollo_analytics_nonce', 'nonce' );

		// Rate limiting: Prevent abuse.
		if ( ! \Apollo_Analytics_Rate_Limiter::check_rate_limit( 'pageview', 50, HOUR_IN_SECONDS ) ) {
			wp_send_json_error( 'Rate limit exceeded' );
		}

		// GDPR: Check consent if required.
		if ( self::is_gdpr_consent_required() && ! self::has_analytics_consent() ) {
			wp_send_json_error( 'Analytics consent required' );
		}
		...
		$data = array(
			...
			'user_id'         => self::anonymize_user_id( absint( $_POST['user_id'] ?? 0 ) ),
			...
			'user_agent'      => self::anonymize_user_agent( sanitize_text_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ?? '' ) ) ),
	...
	public static function ajax_track_interaction(): void {
		...
		// Rate limiting: Prevent abuse.
		if ( ! \Apollo_Analytics_Rate_Limiter::check_rate_limit( 'interaction', 200, HOUR_IN_SECONDS ) ) {
			wp_send_json_error( 'Rate limit exceeded' );
		}
		...
		foreach ( $interactions as $interaction ) {
			$data = array(
				...
				'user_id'          => self::anonymize_user_id( absint( $interaction['user_id'] ?? 0 ) ),
	...
	public static function ajax_track_session_end(): void {
		...
		// Rate limiting: Prevent abuse.
		if ( ! \Apollo_Analytics_Rate_Limiter::check_rate_limit( 'session_end', 20, HOUR_IN_SECONDS ) ) {
			wp_send_json_error( 'Rate limit exceeded' );
		}
	...
	public static function ajax_track_heatmap(): void {
		...
		// Rate limiting: Stricter limits for heatmap (very sensitive data).
		if ( ! \Apollo_Analytics_Rate_Limiter::check_rate_limit( 'heatmap', 10, HOUR_IN_SECONDS ) ) {
			wp_send_json_error( 'Rate limit exceeded' );
		}

		// GDPR: Check consent if required - heatmap is particularly sensitive.
		if ( self::is_gdpr_consent_required() && ! self::has_analytics_consent() ) {
			wp_send_json_error( 'Analytics consent required' );
		}
	...
	/**
	 * Check if GDPR consent is required for analytics
	 */
	public static function is_gdpr_consent_required(): bool {
		return (bool) get_option( 'apollo_analytics_gdpr_consent_required', true );
	}
	...
	public static function has_analytics_consent(): bool {
		...
		if ( isset( $_COOKIE['apollo_consent_analytics'] ) && 'true' === $_COOKIE['apollo_consent_analytics'] ) {
			return true;
		}

		// For logged-in users, check user meta.
		if ( is_user_logged_in() ) {
			$user_consent = get_user_meta( get_current_user_id(), 'apollo_analytics_consent', true );
			if ( 'accepted' === $user_consent ) {
				return true;
			}
		}
	 ...
	 */
	private static function anonymize_user_id( int $user_id ): int {
		// For logged-in users, keep ID but hash it for external storage.
		// For anonymous users, return 0.
		return $user_id > 0 ? $user_id : 0;
	}

	/**
	 * Anonymize user agent string for privacy
	 */
	private static function anonymize_user_agent( string $user_agent ): string {
		// Remove potentially identifying parts while keeping useful analytics data.
		$user_agent = preg_replace( '/\([^)]*\)/', '(anonymized)', $user_agent );
		return sanitize_text_field( $user_agent );
	}

	/**
	 * Register GDPR privacy exporter
	 */
	public static function register_privacy_exporter( array $exporters ): array {
		$exporters['apollo-analytics'] = array(
			'exporter_friendly_name' => 'Apollo Analytics',
	...
	/**
	 * GDPR privacy data exporter
	 */
	public static function privacy_exporter( string $email_address, int $page = 1 ): array {
		...
		// Export pageview data.
		$pageviews = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}apollo_analytics_pageviews WHERE user_id = %d",
				$user->ID
			)
		);
		...
		if ( ! empty( $pageviews ) ) {
			$export_items[] = array(
				'group_id'    => 'apollo_analytics_pageviews',
				'group_label' => 'Apollo Analytics - Page Views',
				'item_id'     => 'pageviews-' . $user->ID,
				'data'        => array_map( function( $pv ) {
					return array(
						'name'  => 'Page View',
						'value' => sprintf(
							'URL: %s, Date: %s, Session: %s',
							$pv->page_url,
							$pv->created_at,
							$pv->session_id
						),
		...
		$interactions = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}apollo_analytics_interactions WHERE user_id = %d",
				$user->ID
			)
		);
		...
		if ( ! empty( $interactions ) ) {
			$export_items[] = array(
				'group_id'    => 'apollo_analytics_interactions',
				'group_label' => 'Apollo Analytics - Interactions',
				'item_id'     => 'interactions-' . $user->ID,
				'data'        => array_map( function( $int ) {
					return array(
						'name'  => 'Interaction',
						'value' => sprintf(
							'Type: %s, Element: %s, Date: %s',
	...
	/**
	 * Register GDPR privacy eraser
	 */
	public static function register_privacy_eraser( array $erasers ): array {
		$erasers['apollo-analytics'] = array(
			'eraser_friendly_name' => 'Apollo Analytics',
	...
	/**
	 * GDPR privacy data eraser
	 */
	public static function privacy_eraser( string $email_address, int $page = 1 ): array {
		$user = get_user_by( 'email', $email_address );
		if ( ! $user ) {
			return array(
				'items_removed'  => false,
		...
		// Remove pageview data.
		$pageviews_deleted = $wpdb->delete(
		...
		$interactions_deleted = $wpdb->delete(
		...
		if ( $pageviews_deleted || $interactions_deleted || $sessions_deleted ) {
			$items_removed = true;
		}
		...
		return array(
			...
			'items_retained' => false,
			'messages'       => array( 'Apollo Analytics data has been removed.' ),
			'done'           => true,
		);
	}
	...
	public static function cleanup_old_data(): void {
		$retention_days = absint( get_option( 'apollo_analytics_retention_days', 365 ) );
		if ( $retention_days <= 0 ) {
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}apollo_analytics_pageviews WHERE created_at < %s",
				$cutoff_date
			)
		);
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}apollo_analytics_interactions WHERE created_at < %s",
				$cutoff_date
			)
		);
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}apollo_analytics_sessions WHERE started_at < %s",
				$cutoff_date
			)
		);
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}apollo_analytics_heatmap WHERE stat_date < %s",
	...
	/**
	 * Run data cleanup (cron job handler)
	 */
	public static function run_data_cleanup(): void {
		$retention_days = absint( get_option( 'apollo_analytics_retention_days', 365 ) );
		if ( $retention_days > 0 ) {
			self::cleanup_old_data_by_days( $retention_days );
		}
	}
	...
	public static function run_daily_aggregation(): void {
		...
		self::cleanup_old_data_by_days( 90 );
	}
	 ...
	 */
	private static function cleanup_old_data_by_days( int $days ): void {
		global $wpdb;
	...
	/**
	 * Central tracking hub for cross-plugin analytics
	 *
	 * Unified tracking entry point for all Apollo plugins.
	 * Routes tracking data to appropriate storage methods based on event type.
	 *
	 * @param array $data {
	 *     @type string $type      Event type (see list below)
	 *     @type int    $user_id   User ID (0 for anonymous)
	 *     @type int    $post_id   Related post/event ID
	 *     @type string $plugin    Source plugin: 'core'|'events'|'social'|'rio'
	 *     @type array  $metadata  Additional context data
	 * }
	 *
	 * Supported Event Types:
	 * - Core: pageview, interaction, session_end, heatmap
	 * - Events: event_view, event_interest, event_bookmark, event_share, event_review,
	 *           event_reaction, event_ticket, event_lifecycle, events_listing,
	 *           dj_view, local_view
	 * - Social: social_interaction, social_reaction, user_page_view, hub_interaction,
	 *           classified_action, classified_view, social_post_action, wall_post_action,
	 *           group_action
	 * - Rio: pwa_navigation, pwa_install
	 *
	 * @return int|false Insert ID or false
	 */
	public static function track( array $data ): int|false {
		if ( ! self::is_tracking_enabled() ) {
			return false;
		}
		...
		/**
		 * Fires before tracking data is processed.
		 *
		 * @param array $data The tracking data.
		 */
		do_action( 'apollo_before_track_process', $data );

		// Allow other plugins to modify tracking data
		$data = apply_filters( 'apollo_before_track', $data );

		// Prepare metadata with source plugin
		$metadata = array_merge(
			array( 'source_plugin' => $data['plugin'] ),
			$data['metadata']
		);
		...
		switch ( $data['type'] ) {
			...
			// ========================================
			case 'pageview':
				$result = self::store_pageview( $data['user_id'], $data['post_id'], $metadata );
				break;

			case 'interaction':
				$result = self::store_interaction( $data['user_id'], $data['post_id'], $metadata );
				break;
			...
			case 'social_interaction':
				$action = $metadata['action'] ?? 'like';
				$target_type = $metadata['target_type'] ?? 'post';
				self::track_social_interaction( $action, $data['user_id'], $data['post_id'], $target_type );
				$result = true;
				break;

			case 'social_reaction':
			case 'user_page_view':
			case 'hub_interaction':
			case 'classified_action':
			case 'classified_view':
			case 'social_post_action':
			case 'wall_post_action':
			case 'group_action':
				$result = self::track_event( $data['type'], $data['user_id'], $data['post_id'], $metadata );
				break;

			// ========================================
			// EVENTS TRACKING TYPES
			// ========================================
			case 'event_view':
			case 'event_interest':
			case 'event_bookmark':
			...
			// ========================================
			// PWA/RIO TRACKING TYPES
			// ========================================
			case 'pwa_navigation':
			case 'pwa_install':
				$metadata['source_plugin'] = 'rio';
				$result = self::track_event( $data['type'], $data['user_id'], $data['post_id'], $metadata );
				break;

			// ========================================
			// DEFAULT: Custom event type
			// ========================================
			default:
				$result = self::track_event( $data['type'], $data['user_id'], $data['post_id'], $metadata );
				break;
		...
		/**
		 * Fires after tracking data is stored.
		 *
		 * @param array     $data   The tracking data.
		 * @param int|false $result The insert ID or false on failure.
		 */
		do_action( 'apollo_after_track', $data, $result );

		return $result;
	}

	/**
	 * Check rate limit using the Rate Limiter class
	 *
	 ...
	 */
	public static function check_rate_limit( string $action, int $limit = 100, int $window_seconds = HOUR_IN_SECONDS ): bool {
		if ( class_exists( 'Apollo_Analytics_Rate_Limiter' ) ) {
			return \Apollo_Analytics_Rate_Limiter::check_rate_limit( $action, $limit, $window_seconds );
		}
		return true;
	}
}