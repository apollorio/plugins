 ...
 *
 * Handles mod queue and approve/reject actions for CENA events.
 *
 * @package Apollo_Core
 * @since 3.1.0
 */
...
class Cena_Rio_Moderation {
	...
	public static function init(): void {
		...
		add_shortcode( 'apollo_cena_mod_queue', array( self::class, 'render_mod_queue' ) );

		// Handle POST actions.
		add_action( 'admin_post_apollo_cena_approve', array( self::class, 'handle_approve' ) );
		add_action( 'admin_post_apollo_cena_reject', array( self::class, 'handle_reject' ) );

		// AJAX handlers for front-end mod.
		add_action( 'wp_ajax_apollo_cena_approve', array( self::class, 'ajax_approve' ) );
		add_action( 'wp_ajax_apollo_cena_reject', array( self::class, 'ajax_reject' ) );

		// REST API endpoints.
		add_action( 'rest_api_init', array( self::class, 'register_rest_routes' ) );

		// Filter: Exclude non-approved CENA-RIO events from public calendar queries.
		add_action( 'pre_get_posts', array( self::class, 'filter_cena_rio_events' ), 20 );
	}
	 ...
	 * Events with status 'expected' or 'confirmed' are still in mod workflow.
	 *
	...
	public static function check_mod_permission(): bool {
		...
		if ( class_exists( '\Apollo_Core\Cena_Rio_Roles' ) ) {
			return \Apollo_Core\Cena_Rio_Roles::user_can_moderate();
		}
		if ( class_exists( 'Apollo_Cena_Rio_Roles' ) ) {
			return \Apollo_Cena_Rio_Roles::user_can_moderate();
		}
		// Fallback to capability check.
		return current_user_can( 'apollo_cena_moderate_events' ) || current_user_can( 'manage_options' );
	}

	/**
	 * REST API: Get mod queue
	 *
	 ...
	 */
	public static function render_mod_queue(): string {
		// Check permission using namespace-aware check.
		$can_moderate = false;
		if ( class_exists( '\Apollo_Core\Cena_Rio_Roles' ) ) {
			$can_moderate = \Apollo_Core\Cena_Rio_Roles::user_can_moderate();
		} elseif ( class_exists( 'Apollo_Cena_Rio_Roles' ) ) {
			$can_moderate = \Apollo_Cena_Rio_Roles::user_can_moderate();
		} else {
			$can_moderate = current_user_can( 'apollo_cena_moderate_events' ) || current_user_can( 'manage_options' );
		}
		...
		ob_start();
		?>
		<div class="apollo-cena-mod-queue" style="max-width:1200px;margin:0 auto">
			<div style="margin-bottom:24px">
				...
				<p style="color:#64748b;margin:0">
					<?php
					// translators: %d is the number of events awaiting approval.
					echo esc_html( sprintf( __( '%d evento(s) aguardando aprovação', 'apollo-core' ), count( $events ) ) );
					?>
				</p>
			</div>
			...
			<?php else : ?>
				<div class="mod-grid" style="display:flex;flex-direction:column;gap:16px">
								...
								<div style="flex:1;min-width:250px">
									...
									<div style="display:flex;flex-direction:column;gap:4px;font-size:14px;color:#64748b">
										<div style="display:flex;align-items:center;gap:8px">
											...
											<span><strong>Data:</strong> <?php echo esc_html( ! empty( $start_date ) ? $start_date : $date ); ?></span>
										</div>
										<?php if ( $venue ) : ?>
											<div style="display:flex;align-items:center;gap:8px">
