...
class Apollo_User_Moderation {
	...
	public static function set_mod_level( int $user_id, int $level, int $actor_id ): bool {
		...
		// Validate level.
		if ( ! in_array( $level, array( self::MOD_LEVEL_NONE, self::MOD_LEVEL_BASIC, self::MOD_LEVEL_ADVANCED, self::MOD_LEVEL_FULL ), true ) ) {
			return false;
		}
	...
	public static function check_ip_blocklist(): void {
		...
		foreach ( $blocklist as $entry ) {
			if ( $entry['hash'] === $ip_hash ) {
				wp_die(
					esc_html__( 'Seu acesso foi bloqueado. Entre em contato com o suporte se acredita que isso é um erro.', 'apollo-core' ),
	...
	public static function check_user_access(): void {
		...
		if ( self::STATUS_BANNED === $status ) {
			wp_logout();
			wp_die(
				esc_html__( 'Sua conta foi banida permanentemente. Entre em contato com o suporte para mais informações.', 'apollo-core' ),
	...
	public static function register_rest_routes(): void {
		...
		register_rest_route(
			'apollo/v1',
			'/moderation/user-status/(?P<user_id>\d+)',
			array(
				'methods'             => 'GET',
