 ...
 *
 * Handles user status management, suspension, and banning.
 *
 * Meta keys utilized:
 * - apollo_status: active, suspended, banned
 * - apollo_suspension_until: timestamp UTC of suspension end
 * - apollo_suspension_reason: suspension reason
 * - apollo_banned_at: ban timestamp
 * - apollo_banned_reason: ban reason
 * - apollo_mod_level: 0, 1, or 3 (moderator levels)
 *
 * @package Apollo_Core
 */
...
function apollo_suspend_user( int $user_id, int $actor_id, string $reason = '', int $duration = 0 ): bool {
	return Apollo_Core\User_Moderation::suspend_user( $user_id, $actor_id, $reason, $duration );
}
...
function apollo_unsuspend_user( int $user_id, int $actor_id = 0, string $reason = '' ): bool {
	return Apollo_Core\User_Moderation::unsuspend_user( $user_id, $actor_id, $reason );
}
...
function apollo_ban_user( int $user_id, int $actor_id, string $reason = '' ): bool {
	return Apollo_Core\User_Moderation::ban_user( $user_id, $actor_id, $reason );
}
...
function apollo_is_user_suspended( int $user_id ): bool {
	return Apollo_Core\User_Moderation::is_user_suspended( $user_id );
}
...
function apollo_is_user_banned( int $user_id ): bool {
	return Apollo_Core\User_Moderation::is_user_banned( $user_id );
}
...
function apollo_can_user_perform( int $user_id, string $capability = '' ): bool {
	return Apollo_Core\User_Moderation::can_user_perform( $user_id, $capability );
}
...
function apollo_get_mod_level( int $user_id ): int {
	return Apollo_Core\User_Moderation::get_mod_level( $user_id );
}
...
function apollo_can_moderate( int $user_id, string $action ): bool {
	return Apollo_Core\User_Moderation::can_moderate( $user_id, $action );
}
