...
final class User_Dashboard_Interesse {
	...
	private static function build_dashboard( int $uid, bool $compact = false ): string {
		$events       = class_exists( __NAMESPACE__ . '\\Interesse_Ranking' ) ? Interesse_Ranking::get_user_interested_events( $uid ) : array();
		$sounds       = class_exists( __NAMESPACE__ . '\\Interesse_Ranking' ) ? Interesse_Ranking::get_user_top_sounds( $uid, 10 ) : array();
...
<div class="tags">
			...
			foreach ( $bubbles as $b ) :
				?>
	<span class="tag"><?php echo esc_html( $b ); ?></span><?php endforeach; ?></div>
</div>
		<?php endif; ?>
...
<div class="tags">
			...
			foreach ( $communities as $c ) :
				?>
	<span class="tag"><?php echo esc_html( $c ); ?></span><?php endforeach; ?></div>
</div>
		<?php endif; ?>
...
<div class="tags">
			...
			foreach ( $nucleos as $n ) :
				?>
	<span class="tag"><?php echo esc_html( $n ); ?></span><?php endforeach; ?></div>
</div>
		<?php endif; ?>
</div>
</div>
...
.empty{color:var(--muted);font-style:italic;margin:0}
	...
	private static function get_upcoming_interested_events( int $uid, int $limit = 5 ): array {
		$eids = class_exists( __NAMESPACE__ . '\\Interesse_Ranking' ) ? Interesse_Ranking::get_user_interested_events( $uid ) : array();
		if ( empty( $eids ) ) {
			return array();
		}
		$args = array(
	...
	public static function render_sounds_section( int $uid ): void {
		...
		foreach ( $sounds as $i => $s ) {
			...
			echo '<li><span class="n">#' . ( $i + 1 ) . '</span><span class="t">' . esc_html( $s['name'] ) . '</span><span class="b" style="width:' . $pct . '%"></span></li>';}
...
add_action( 'init', array( 'Apollo_Core\\User_Dashboard_Interesse', 'init' ) );
