...
class Global_Assets {
	 ...
	 * Core CSS/JS comes from Apollo CDN: https://assets.apollo.rio.br/
	 *
	 * @return void
	 */
	public static function register_assets(): void {
		...
		// Apollo CDN Loader Script - main entry point
		wp_register_script(
			'apollo-cdn-loader',
		...
		// DEPRECATED: apollo-uni-css now points to CDN styles/index.css
		// Kept for backward compatibility - CDN script loads this automatically
		wp_register_style(
			'apollo-uni-css',
	...
	public static function enqueue_css( bool $force = false ): void {
		...
		// Local tokens and legacy compatibility CSS
		if ( wp_style_is( 'apollo-tokens', 'registered' ) ) {
			wp_enqueue_style( 'apollo-tokens' );
		}
		if ( wp_style_is( 'apollo-legacy-compat', 'registered' ) ) {
        ...
        ';
		if ( wp_style_is( 'apollo-tokens', 'enqueued' ) ) {
			wp_add_inline_style( 'apollo-tokens', $inline_css );
		}
