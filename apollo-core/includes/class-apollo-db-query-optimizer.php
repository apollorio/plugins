...
class DB_Query_Optimizer {
	...
	public static function optimize_user_queries( mixed $results, WP_User_Query $query ): mixed {
		...
		add_filter(
			...
			function ( $sql ) {
				...
				if ( false !== strpos( $sql, 'wp_users' ) ) {
					// Add FORCE INDEX hint if needed.
					$sql = str_replace(
						'FROM wp_users',
						'FROM wp_users FORCE INDEX (PRIMARY)',
						$sql
					);
				}
				return $sql;
	...
	public static function create_optimized_indexes(): void {
		...
		$indexes = array(
			...
			"CREATE INDEX IF NOT EXISTS idx_event_listing_status_date ON {$wpdb->posts} (post_type, post_status, post_date)",
			"CREATE INDEX IF NOT EXISTS idx_event_listing_modified ON {$wpdb->posts} (post_type, post_modified)",

			// User meta indexes for common queries.
			"CREATE INDEX IF NOT EXISTS idx_user_meta_views ON {$wpdb->usermeta} (user_id, meta_key(50)) WHERE meta_key = 'profile_views'",
			"CREATE INDEX IF NOT EXISTS idx_user_meta_last_active ON {$wpdb->usermeta} (user_id, meta_key(50)) WHERE meta_key = 'last_active'",
		);

		foreach ( $indexes as $index_sql ) {
			$wpdb->query( $wpdb->prepare( $index_sql ) );
		}
	}
