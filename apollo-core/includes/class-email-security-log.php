...
class EmailSecurityLog
{
    ...
    private const RATE_LIMIT_MAX = 50; // Max emails per user per hour
    ...
    public static function isRateLimited(int $user_id = 0, string $ip = ''): bool
    {
        ...
        if ((int) $count >= self::RATE_LIMIT_MAX) {
            ...
            self::log(self::TYPE_RATE_LIMITED, 'rate-limit@blocked', [
                'metadata' => [
                    'user_id' => $user_id,
                    'ip' => $ip,
                    'count' => $count,
    ...
    private static function detectSuspiciousActivity(string $type, string $recipient, string $ip): void
    {
        ...
        $table_name = $wpdb->prefix . self::TABLE_NAME;
        $window_start = gmdate('Y-m-d H:i:s', time() - 300); // Last 5 minutes

        // Check for rapid sending from same IP
        $rapid_count = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$table_name} 
        ...
        if ((int) $rapid_count > 10) {
            self::log(self::TYPE_SUSPICIOUS, $recipient, [
            ...
            if ((int) $rapid_count > 25) {
                self::notifyAdmins('Atividade suspeita de email detectada', [
                    'IP' => $ip,
    ...
    public static function getStats(string $period = 'today'): array
    {
        ...
        // By template
        $template_counts = $wpdb->get_results($wpdb->prepare(
        ...
        // Top recipients
        $stats['top_recipients'] = $wpdb->get_results($wpdb->prepare(
