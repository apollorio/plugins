	...
	if ( $is_blocked ) {
		...
		return new WP_Error(
			...
			sprintf(
				...
				! empty( $reason ) ? $reason : __( 'No reason provided', 'apollo-core' )
			)
		);
	}
...
function apollo_check_current_user_suspension() {
	...
	if ( $is_blocked ) {
		wp_logout();
		wp_safe_redirect( wp_login_url() . '?apollo_error=blocked' );
		exit;
	}
	...
	if ( $suspended_until && time() < intval( $suspended_until ) ) {
		wp_logout();
		wp_safe_redirect( wp_login_url() . '?apollo_error=suspended' );
		exit;
	}
...
function apollo_login_error_messages() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- This is a login page redirect parameter, not a form submission.
	if ( isset( $_GET['apollo_error'] ) ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only display of redirect parameter.
